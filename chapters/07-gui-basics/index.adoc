...
Recall from Chapter <<chapter:Repetition>> that we can record DNA as a
...
few built-in Java classes. Chapter <<chapter:Constructing Graphical User
Interfaces>> introduces more advanced tools for constructing complex
GUIs.
...
Chapter <<chapter:Constructing_Graphical_User_Interfaces>> discusses the
...
discuss frames in greater detail in Section <<syntax:GUIs_in_Java>>.
...
Example <<example:Yes-No_dialog>> automatically generates two buttons
...
Section <<syntax:Java_basics>>, but we give two more examples here to
...
In Example <<example:Input_dialog>> the user is required to enter a
...
Unlike Example <<example:Input_dialog>>, the return value from
...
identical to the code for Example <<example:Input_dialog_with_a_list>>.
...
Chapter <<chapter:Synchronization>>.
...
Chapter <<chapter:Constructing_Graphical_User_Interfaces>>, we will also
...
5.  Modify the program in Example <<example:Yes-No_dialog>> such that it
...
6.  Modify the program in Example <<example:Yes-No_dialog>> such that it
...
has focus.
10. <<exercise:randomizeCapitalsExercise>> Section <<solution:Three card
...
Exceptions are covered in Chapter <<chapter:Exceptions>>.* +
