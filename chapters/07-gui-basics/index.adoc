...
Let's dissect <<SimpleDialogProgram>>. Line <<importSwing>> is
...
<<OddEvenTestProgram>> begins by declaring a random number
...
<<CapitalQuizProgram>> performs these tasks. In this program,
...
line <<multipleOptionsLine>> in <<CapitalQuizProgram>>. The
first one functions like the `null` used in
<<OddEvenTestProgram>>, specifying that the default frame
...
[.text-center]
.A dialog with a custom icon generated by <<CustomIconDialogProgram>>.
...
[[inputDialogFigure]]
[.text-center]
.(a) A dialog requesting input as text. (b) A dialog requesting a selection from a list of choices. These dialogs are generated by <<ChemistryQuizOneProgram>> and <<ChemistryQuizTwoProgram>>, respectively.
...
in <<CapitalQuizProgram>> so that the button labeled "`Baku`"
has focus.
