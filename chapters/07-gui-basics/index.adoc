...
Recall from <<Repetition>> that we can record DNA as a
...
few built-in Java classes. <<Constructing Graphical User Interfaces>> introduces more advanced tools for constructing complex
GUIs.
...
<<Constructing Graphical User Interfaces>> discusses the
...
dialog is shown in <<yes-noDialogFigure>>(a). A second dialog is
...
<<yes-noDialogFigure>>(b) and (c). Note that a call to
...
<<selectCapitalDialogFigure>> shows the dialog created.
...
<<customIconDialogFigure>>. Dialogs illustrated in earlier
...
<<chemistryQuizFigure>>(a). This method returns a `String` named
...
<<chemistryQuizFigure>>(b). The four elements are contained in a
drop down list.
...
The GUIs we will create in <<Constructing Graphical User Interfaces>>, however, will be more than dialogs. They will be fully
...
<<Constructing Graphical User Interfaces>>, we will also
...
constants).
4.  [[exercise7.4]] In Program <<program:OddEvenTest>>,
...
5.  [[exercise7.5]] Modify the program in <<example:Yes-No_dialog>> such that it
...
6.  [[exercise7.6]] Modify the program in <<example:Yes-No_dialog>> such that it
...
dialog.
9.  [[exercise7.9]] Modify line <<multipleOptionsLine>>
in Program <<program:CapitalQuiz>> so that the button labeled "`Baku`"
has focus.
10. [[exercise7.10]] <<solution:Three card
...
Exceptions are covered in <<Exceptions>>.* +
