...
and making it function properly. <<Simple Graphical User Interfaces>> introduced a way to make simple GUIs, but those GUIs came
...
created in <<example:Empty_frame>>.
...
[source, java, linenums]
[[program:FrameWithPanel, FrameWithPanel]]
----
...
In the `main()` method in <<FrameWithPanel>>, we first
...
As discussed in <<Interfaces>>, an interface is a set of
...
[source, java, linenums]
[[program:FrameWithPanelAndActions, FrameWithPanelAndActions]]
----
...
[source, java, linenums]
[[program:AlternateActionListener, AlternateActionListener]]
----
...
[source, java, linenums]
[[program:SimpleMouseEvents, SimpleMouseEvents]]
----
...
****
<<exercise15.8, Exercise 15.8>>
****

===== Mouse adapter

Creating a `MouseListener` requires all five methods in the interface to
be implemented. In some cases, as in <<example:Mouse_listener>>,
...
[source, java, linenums]
[[program:SimpleMouseAdapter, SimpleMouseAdapter]]
----
...
like <<soundGameFigures>>(b).

.An animal sound game.
[source, java, linenums]
[[program:SoundGame, SoundGame]]
----
...
****
<<exercise15.11, Exercise 15.11>> +
<<exercise15.12, Exercise 15.12>>
****
...
[source, java, linenums]
[[program:IconExample, IconExample]]
----
...
<<labelExampleFigure>>(b) if the alignment instructions at
...
[source, java, linenums]
[[program:LabelExample, LabelExample]]
----
...
****
<<exercise15.18, Exercise 15.18>> +
<<exercise15.19, Exercise 15.19>>
****
...
<<animalIdentifierFigure>>. Class `AnimalIdentifier` declares
...
[source, java, linenums]
[[program:AnimalIdentifier, AnimalIdentifier]]
----
include::programs/AnimalIdentifier.java[]
----

Inside the constructor, we create a panel with a
2 × 2 `GridLayout`. Then, we create four image icons,
...
[source, java, linenums]
[[program:BorderLayoutExample, BorderLayoutExample]]
----
...
[source, java, linenums]
[[program:CalculatorLayout, CalculatorLayout]]
----
...
****
<<exercise15.4, Exercise 15.4>> +
<<exercise15.21, Exercise 15.21>>
****
...
[source, java, linenums]
[[program:ProblemGenerator, ProblemGenerator]]
----
...
<<MathTutorFigure>>. Class `MathTutor` begins by creating only
...
<<rainbowAppletFigure>>. Program <<program:RainbowApplet>>
defines the applet.
...
[source, java, linenums]
[[program:RainbowApplet, RainbowApplet]]
----
...
<<Synchronization>> will become important. However, only
...
[source, java, linenums]
[[program:UnresponsiveGUI, UnresponsiveGUI]]
----
...
****
<<exercise15.23, Exercise 15.23>> +
<<exercise15.24, Exercise 15.24>>
****
...
what kind of objects will be kept in the list. <<Dynamic Data Structures>> covers generics in some depth. The reason we need
...
2.  [[exercise15.2]] Note that both `ActionListener`
...
10. [[exercise15.10]] Write a Java program that
creates a GUI containing a label with a picture of yourself.
11. [[exercise15.11]] Extend the `SoundGame` class
developed in <<example:Sound_game>> to include sounds for
...
12. [[exercise15.12]] In <<example:Sound
...
<<exercise15.9, Exercise 15.9>> and <<exercise15.11, Exercise 15.11>>, you may need to download sounds from
the Internet.
14. [[exercise15.14]] Modify the applet from
<<solution:Math_applet>> to display the problem number the user
...
the GUI.
15. [[exercise15.15]] Remove the `actionPerformed()` and
`itemStateChanged()` methods from the `MathTutorApplet` class given in
<<solution:Math_applet>>. Move the code from these methods into
...
`pack()` method?
20. [[exercise15.20]] Create a GUI with a frame that uses
...
21. [[exercise15.21]] Write a Java program that creates
...
bar to a `JFrame`?
22. [[exercise15.22]] Use the example from the book to
create a suitable HTML file that embeds the `RainbowApplet` class from
<<example:RainbowApplet>> and links to the Java source code. Now
...
24. [[exercise15.24]] Program <<program:UnresponsiveGUI>> is
...
of random numbers, similar to the work done in <<example:Array
summation>>. How many sines do you need to compute to make the GUI
unresponsive for 5 seconds?
25. [[exercise15.25]] Take the computationally expensive loop
from <<exercise15.24, Exercise 15.24>> and use it to replace
