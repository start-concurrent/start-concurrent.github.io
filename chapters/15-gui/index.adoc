...
created in Example REF=example:Empty frame.
...
be implemented. In some cases, as in Example REF=example:Mouse listener,
...
.A GUI demonstrating `BorderLayout` generated by Program REF=program:BorderLayoutExample.
...
.GUI generated by Program REF=program:CalculatorLayout.
...
.The `RainbowApplet` as seen through a web browser. Clicking on the link displays the Java source code of the applet in Program REF=program:RainbowApplet.
...
.GUI generated by Program REF=program:UnresponsiveGUI.
...
Program REF=program:FrameWithPanelAndActions, why have we declared the
...
Program REF=program:SimpleMouseEvents by implementing the
...
Program REF=program:IconExample such that clicking the icon-decorated
...
developed in Example REF=example:Sound game to include sounds for
...
12. <<exercise:SoundGameProperStopExercise>> In
Example REF=example:Sound game we stopped both the chirp and the bark
sounds because the action listener corresponding to the "`Stop Playing`"
button does not know which sound is playing. Modify
Program REF=program:SoundGame so that only the sound that is playing is
stopped. You may need to declare another variable to keep track of which
is playing.
13. <<exercise:animalIdentifierExercise>> Modify
Program REF=program:AnimalIdentifier so that it plays a sound associated
...
Exercises REF=exercise:SoundGameExtension and
REF=exercise:iconAndSoundExercise, you may need to download sounds from
the Internet.
14. <<exercise:addSessionCountExercise>> Modify the applet from
Section REF=solution:Math applet to display the problem number the user
...
Section REF=solution:Math applet. Move the code from these methods into
...
Program REF=program:FrameWithPanelAndActions exchange the first two
...
Program REF=program:FrameWithPanel by adding a second panel named
...
line REF=disableResizableLine from
...
Program REF=program:FlowLayoutExample by deleting the lines that set the
frame size. What does the resulting GUI look like? Now add the following
code just before line REF=disableResizableLine:
+
....
demo.pack();
....
...
Example REF=example:RainbowApplet and links to the Java source code. Now
...
+
*Experiments*
23. <<exercise:unresponsive GUI>> Recall that
Program REF=program:UnresponsiveGUI is unresponsive because the event
dispatch thread goes to sleep for 5 seconds (5,000 milliseconds) on
line REF=unresponsive sleep. Experiment with this value to determine
what is a reasonable amount of time for the EDT to be blocked before the
GUI feels unresponsive.
24. <<exercise:busy GUI>> Program REF=program:UnresponsiveGUI is
...
calculations or doing slow I/O operations. Replace line REF=unresponsive
...
of random numbers, similar to the work done in Example REF=example:Array
summation. How many sines do you need to compute to make the GUI
unresponsive for 5 seconds?
25. <<exercise:busy workers>> Take the computationally expensive loop
from Exercise REF=exercise:busy GUI and use it to replace
line REF=responsive sleep in Program REF=program:WorkerGUI, the
