...
<<EmptyFrame>> creates and displays an empty frame with
...
In <<AlternateActionListenerProgram>> class
...
<<LabelExampleProgram>>. You will see the GUI shown in
...
24. [[exercise15.24]] <<UnresponsiveGUIProgram>> is
...
line <<responsive_sleep>> in <<WorkerGUIProgram>>, the
