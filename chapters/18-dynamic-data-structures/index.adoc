...
Exercise REF=exercise:dynamicArrayTestExercise explores the need to
...
properly implemented). Exercise REF=exercise:implementDASMethods has you
...
of `String` objects. In Example REF=example:Postfix computation, we
...
Example REF=example:Postfix computation.
...
created. In the case of Example REF=example:Postfix computation, we need
...
beginning of the chapter. As in Example REF=example:Postfix computation,
...
Example REF=example:Postfix computation for our stack. All that remains
...
evaluator program from Example REF=example:Postfix computation. A more
...
is the `HashMap` from Example REF=example:Maps.
...
Program REF=program:ReadIntoFixedArray is an exception generated?
4.  <<exercise:postIncrementExercise>> In
Program REF=program:ReadAndGrowArray, is it possible to post-increment
...
Program REF=program:UseDynamicArray is, on average, only three-quarters
full.
...
Program REF=program:DynamicArrayStack.
8.  Based on queue implementation in
Program REF=program:LinkedListQueue, draw a picture of the linked list
structure after each of the following statements.
+
....
...
9.  <<exercise:Shrink dynamic array>> Implement a version of
...
10. Consider Program REF=program:LinkedListWithTail which defines the
...
11. In Section REF=subsection:Linked lists, we use two kinds of linked
...
(Program REF=program:DynamicArray). To test the `set()` method, you can
...
`parseDouble()` method from Example REF=example:Parsing a number to
compute the equivalent `double` values of operands, rewrite the solution
from Section REF=solution:Infix conversion to convert your terms into
