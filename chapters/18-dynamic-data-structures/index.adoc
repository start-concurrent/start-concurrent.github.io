...
Chapter <<chapter:Classes>> and examine in much greater depth here.
...
solve the nesting expression problem in Section <<solution:Nested
...
problem from Chapter <<chapter:Classes>> is not actually a dynamic data
...
Section <<syntax:Abstract_data_types_(ADT)>>, but there are many other
useful ADTs.
...
Program <<program:ReadIntoFixedArray>> allocates an array of 10 strings
...
.Program to read names into an array, sort, and print. If there are more than 10 lines in the input, an exception is generated.
[source, java, linenums]
[[program:ReadIntoFixedArray, ReadIntoFixedArray]]
----
include::programs/ReadIntoFixedArray.java[]
----
...
Program <<program:ReadAndGrowArray>> begins like the previous program by
allocating a fixed array. However, it now catches the
`ArrayOutOfBoundsException` at line <<exceptionRAGA>> if it tries to
...
.Read names into an array, enlarging the array as necessary.
[source, java, linenums]
[[program:ReadAndGrowArray, ReadAndGrowArray]]
----
include::programs/ReadAndGrowArray.java[]
----

Note that it was necessary to refactor the code in
Program <<program:ReadIntoFixedArray>> slightly: Add the `name` variable
...
the `names` array. Figure <<figure:dynamicproblems>> gives an example of
this problem.
...
same version of the array. Program <<program:DynamicArray>> is a simple
...
.A class to manage a dynamic array. This array grows by doubling when more space is needed.
[source, java, linenums]
[[program:DynamicArray, DynamicArray]]
----
include::programs/DynamicArray.java[]
----

Note that the `set()` and `sort()` methods are both `synchronized` in
case this class is used by multiple threads simultaneously.
Exercise <<exercise:dynamicArrayTestExercise>> explores the need to
synchronize these methods in the presence of multiple threads.

Program <<program:UseDynamicArray>> illustrates how to modify and extend
Program <<program:ReadIntoFixedArray>> to use this new class. Since the
...
.A program that uses the `DynamicArray` class to store input read from a file.
[source, java, linenums]
[[program:UseDynamicArray, UseDynamicArray]]
----
include::programs/UseDynamicArray.java[]
----
...
on average, the array will only be three-quarters full
...
refresher on static nested classes, refer to Section <<advanced:Nested
classes>>.
...
.A basic implementation of a linked list class to hold `String` objects.
[source, java, linenums]
[[program:LinkedList, LinkedList]]
----
include::programs/LinkedList.java[]
----

Program <<program:UseLinkedList>> is a re-implementation of the
...
.A program that uses the `LinkedList` class to store input read from a file.
[source, java, linenums]
[[program:UseLinkedList, UseLinkedList]]
----
include::programs/UseLinkedList.java[]
----
...
`String` read in, has a `next` value of `null`. Figure <<figure:linked
...
Program <<program:LinkedListWithTail>> adds a _tail pointer_ called
...
[source, java, linenums]
[[program:LinkedListWithTail, LinkedListWithTail]]
----
include::programs/LinkedListWithTail.java[]
----
...
Program <<program:SortedLinkedList>> is a version of a linked list that
...
.A linked list class in which calling the `add()` method inserts each value in sorted order.
[source, java, linenums]
[[program:SortedLinkedList, SortedLinkedList]]
----
include::programs/SortedLinkedList.java[]
----
...
Chapter <<chapter:Classes>>. Recall that a stack data structure behaves
...
infix conversion problem in Section <<solution:Infix_conversion>>, but
...
.An interface specifying the stack ADT.
[source, java, linenums]
[[program:Stack, Stack]]
----
include::programs/Stack.java[]
----
...
Program <<program:LinkedListStack>>.

.A class to implement a stack ADT using a linked list.
[source, java, linenums]
[[program:LinkedListStack, LinkedListStack]]
----
include::programs/LinkedListStack.java[]
----
...
expressions problem in Section <<solution:Nested_expressions>> merely
...
Like the dynamic array example of Program <<program:UseDynamicArray>>,
Program <<program:DynamicArrayStack>> implements a stack of `String`
values using a dynamic array data structure.

.Program illustrating a stack ADT partially implemented using a dynamic array.
[source, java, linenums]
[[program:DynamicArrayStack, DynamicArrayStack]]
----
include::programs/DynamicArrayStack.java[]
----

This stack implementation using a dynamic array omits the `top()` and
`isEmpty()` methods (causing a compiler error in
Program <<program:DynamicArrayStack>> until the `Stack` interface is
properly implemented). Exercise <<exercise:implementDASMethods>> has you
provide implementations of these methods.
...
Section <<solution:Infix_conversion>>, we give the solution to this
...
[source, java, linenums]
[[program:TermStack, TermStack]]
----
include::programs/TermStack.java[]
----

This class gives a linked list implementation of a stack. In fact, it is
virtually identical to Program <<program:LinkedListStack>> with the
substitution of `Term` for `String`.

.Program to evaluate a postfix expression.
[source, java, linenums]
[[program:PostfixCalculator, PostfixCalculator]]
----
include::programs/PostfixCalculator.java[]
----
...
.An interface specifying the queue ADT.
[source, java, linenums]
[[program:Queue, Queue]]
----
include::programs/Queue.java[]
----

===== Linked list implementation

Program <<program:LinkedListQueue>> shows an implementation of the queue
...
.Program illustrating a queue ADT implemented using a linked list.
[source, java, linenums]
[[program:LinkedListQueue, LinkedListQueue]]
----
include::programs/LinkedListQueue.java[]
----
...
of `String` objects. In Example <<example:Postfix_computation>>, we
create the stack class `TermStack` to hold `Term` objects, but
...
Example <<example:Postfix_computation>>.
...
Program <<program:ObjectStack>> is an implementation of a stack ADT with
an underlying data type of `Object`.

.A class that implements a stack of `Object` references.
[source, java, linenums]
[[program:ObjectStack, ObjectStack]]
----
include::programs/ObjectStack.java[]
----
...
created. In the case of Example <<example:Postfix_computation>>, we need
a stack that can hold `Term` objects instead of `String` objects, and a
generic class allows us to create a stack of any reference type.
...
Program <<program:GenericLinkedList>> defines a generic version of the
...
[source, java, linenums]
[[program:GenericLinkedList, GenericLinkedList]]
----
include::programs/GenericLinkedList.java[]
----
...
Program <<program:UseGenericLinkedList>> uses the generic class
`GenericLinkedList` parameterized by `String` to re-implement
Program <<program:UseLinkedList>>.

.Program that uses the generic class `GenericLinkedList` to create and use a linked list of Strings.
[source, java, linenums]
[[program:UseGenericLinkedList, UseGenericLinkedList]]
----
include::programs/UseGenericLinkedList.java[]
----
...
Section <<subsection:The_for-each_loop>>. The `ArrayDeque`, `ArrayList`,
...
Program <<program:VectorExample>> illustrates a use of the `Vector`
...
.A simple program to illustrate the use of the Vector class.
[source, java, linenums]
[[program:VectorExample, VectorExample]]
----
include::programs/VectorExample.java[]
----

Output from a typical run of Program <<program:VectorExample>> is shown
below:
...
Program <<program:HashMapExample>> reads a sequence of lines containing
...
.A program that illustrates using a `HashMap` dictionary to store a set of names and ages.
[source, java, linenums]
[[program:HashMapExample, HashMapExample]]
----
include::programs/HashMapExample.java[]
----
...
beginning of the chapter. As in Example <<example:Postfix_computation>>,
...
[source, java, linenums]
----
include::programs/Term.java[lines=1..4]
----
...
[source, java, linenums]
----
include::programs/Term.java[lines=6..14]
----
...
[source, java, linenums]
----
include::programs/Term.java[lines=16..18]
----
...
[source, java, linenums]
----
include::programs/Term.java[lines=20..34]
----
...
Example <<example:Postfix_computation>> for our stack. All that remains
is the client code that parses the input.

[source, java, linenums]
----
include::programs/InfixToPostfix.java[lines=1..9]
----
...
[source, java, linenums]
----
include::programs/InfixToPostfix.java[lines=10..33]
----
...
[source, java, linenums]
----
include::programs/InfixToPostfix.java[lines=35..41]
----
...
evaluator program from Example <<example:Postfix_computation>>. A more
complex program that did both the conversion and the calculation might
want to store everything in `Term` objects instead of outputting a
`String` and then recreating `Term` objects.
...
Program <<program:UseLinkedListQueue>> is a simple multi-threaded
program to test (and break!) the thread safety of the queue
implementation in Program <<program:LinkedListQueue>>. This program
(<<program:UseLinkedListQueue>>) creates a queue and stores a reference
...
.Program to test the queue implementation, including its thread safety.
[source, java, linenums]
[[program:UseLinkedListQueue, UseLinkedListQueue]]
----
include::programs/UseLinkedListQueue.java[]
----
...
line <<linePutQueue>> in Program <<program:LinkedListQueue>>). Suppose
...
variables to the new `Node` object `temp` at line <<lineNewLLN>> and
...
Chapter <<chapter:Synchronization>>, this mutual exclusion can be
...
.A synchronized version of the queue class that allows thread-safe use.
[source, java, linenums]
[[program:LinkedListQueueTS, LinkedListQueueTS]]
----
include::programs/LinkedListQueueTS.java[]
----

With both `enqueue()` and `dequeue()` methods synchronized as in
Program <<program:LinkedListQueueTS>>, a typical output generated by the
...
As we mentioned in Section <<concurrency:Objects>>, some libraries are
...
is the `HashMap` from Example <<example:Maps>>.

As we mentioned in Chapter <<chapter:Interfaces>>, an interface cannot
...
Program <<program:ArrayListExample>> is an example of synchronizing
...
(shared) class variable `list` at line <<aleSync>>.

.Example of thread-safe use of an `ArrayList`.
[source, java, linenums]
[[program:ArrayListExample, ArrayListExample]]
----
include::programs/ArrayListExample.java[]
----
...
data structures.
2.  In which situations is it better to use a dynamic array? In which
situations is it better to use a linked list? Explain why in each case.
3.  <<exercise:exceptionGeneratedExercise>> On which line in
Program <<program:ReadIntoFixedArray>> is an exception generated?
4.  <<exercise:postIncrementExercise>> In
Program <<program:ReadAndGrowArray>>, is it possible to post-increment
`n` inside the `try` clause rather than at the bottom of the `while`
loop?
5.  <<exercise:proveThreeQuartersFull>> Explain why the `names` array in
Program <<program:UseDynamicArray>> is, on average, only three-quarters
full.
6.  Based on the stack implementation in
Program <<program:LinkedListStack>>, draw a picture of the linked list
...
----
    ...
    stack.push("world");
----
7.  <<exercise:implementDASMethods>> Implement the methods `top()` and
`isEmpty()` for the dynamic array implementation of the stack in
Program <<program:DynamicArrayStack>>.
8.  Based on queue implementation in
Program <<program:LinkedListQueue>>, draw a picture of the linked list
structure after each of the following statements.
+
[source,java]
----
...
9.  <<exercise:Shrink_dynamic_array>> Implement a version of
`DynamicArray` that shrinks the size of its internal storage array to
...
are added to the dynamic array at once and then removed.
10. Consider Program <<program:LinkedListWithTail>> which defines the
...
11. In Section <<subsection:Linked_lists>>, we use two kinds of linked
...
+
The algorithm for a bubble sort is described in Section <<problem:Sort
...
Chapter <<chapter:Recursion>>.
12. <<exercise:dynamicArrayTestExercise>> Create JUnit test cases to
verify that the `synchronized` keywords are needed on the `set()` and
`sort()` methods of the `DynamicArray` example
(Program <<program:DynamicArray>>). To test the `set()` method, you can
...
`sort()` method, create two threads that sort the same large random
...
13. To make an infix calculator that can handle floating point values or
...
`parseDouble()` method from Example <<example:Parsing_a_number>> to
compute the equivalent `double` values of operands, rewrite the solution
from Section <<solution:Infix_conversion>> to convert your terms into
postfix ordering and then calculate the answer.
14. Re-implement the solution to the infix conversion problem given in
Section <<solution:Infix_conversion>> so that it uses `GenericStack`
with a type parameter of `Term` instead of `TermStack`.
15. Interfaces can also be generic. Consider the following generic
version of `Queue`.
+
[source,java]
----
