...
encoding that intuition into a computer program takes effort.
...
<<Classes>> and examine in much greater depth here.
...
solve the nesting expression problem in <<Nested expressions>>, but we explore stacks in this chapter as one of many
...
problem from <<Classes>> is not actually a dynamic data
...
<<exercise18.12, Exercise 18.12>> explores the need to
synchronize these methods in the presence of multiple threads.

****
<<exercise18.9, Exercise 18.9>> +
<<exercise18.12, Exercise 18.12>>
****
...
<<Classes>>. Recall that a stack data structure behaves
...
<<The for-each loop>>. The `ArrayDeque`, `ArrayList`,
...
<<Synchronization>>, this mutual exclusion can be
...
As we mentioned in <<Interfaces>>, an interface cannot
...
1. [[exercise18.1]] Explain the difference between static data structures and dynamic
data structures.
2.  [[exercise18.2]] In which situations is it better to use a dynamic array? In which
situations is it better to use a linked list? Explain why in each case.
3.  [[exercise18.3]] On which line in
...
loop?
5.  [[exercise18.5]] Explain why the `names` array in
Program <<program:UseDynamicArray>> is, on average, only three-quarters
full.
6. [[exercise18.6]] Based on the stack implementation in
...
9.  [[exercise18.9]] Implement a version of
...
10. [[exercise18.10]] Consider Program <<program:LinkedListWithTail>> which defines the
...
11. [[exercise18.11]] In <<Linked lists>>, we use two kinds of linked
...
+
The algorithm for a bubble sort is described in <<Problem: Sort it out>>. The idea is to make repeated passes through a list, swapping
...
<<Recursion>>.
12. [[exercise18.12]] Create JUnit test cases to
...
13. [[exercise18.13]] To make an infix calculator that can handle floating point values or
...
from <<Solution: Infix conversion>> to convert your terms into
postfix ordering and then calculate the answer.
14. [[exercise18.14]] Re-implement the solution to the infix conversion problem given in
<<solution:Infix_conversion>> so that it uses `GenericStack`
with a type parameter of `Term` instead of `TermStack`.
15. [[exercise18.15]] Interfaces can also be generic. Consider the following generic
version of `Queue`.
+
[source,java]
----
