...
<<chapter:Classes>> and examine in much greater depth here.
...
solve the nesting expression problem in <<solution:Nested
...
problem from <<chapter:Classes>> is not actually a dynamic data
...
<<syntax:Abstract_data_types_(ADT)>>, but there are many other
useful ADTs.
...
the `names` array. <<figure-dynamicproblems>> gives an example of
this problem.
...
refresher on static nested classes, refer to <<advanced:Nested
classes>>.
...
`String` read in, has a `next` value of `null`. <<figure-linked_list_classes>> shows a visualization of the contents of this
...
<<chapter:Classes>>. Recall that a stack data structure behaves
...
infix conversion problem in <<solution:Infix_conversion>>, but
...
expressions problem in <<solution:Nested_expressions>> merely
...
<<solution:Infix_conversion>>, we give the solution to this
...
of `String` objects. In <<example:Postfix_computation>>, we
...
created. In the case of <<example:Postfix_computation>>, we need
...
<<subsection:The_for-each_loop>>. The `ArrayDeque`, `ArrayList`,
...
beginning of the chapter. As in <<example:Postfix_computation>>,
...
<<example:Postfix_computation>> for our stack. All that remains
...
evaluator program from <<example:Postfix_computation>>. A more
...
<<chapter:Synchronization>>, this mutual exclusion can be
...
is the `HashMap` from <<example:Maps>>.

As we mentioned in <<chapter:Interfaces>>, an interface cannot
...
11. In <<subsection:Linked_lists>>, we use two kinds of linked
...
+
The algorithm for a bubble sort is described in <<problem:Sort
