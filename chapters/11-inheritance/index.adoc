...
In <<Selection>>, we talked extensively about Boolean
...
problem from <<Interfaces>>, multiple inheritance would
...
interfaces from <<Interfaces>>, which specified the
...
We introduced access modifiers in <<Access modifiers>>, but inheritance gives them new meaning. Recall that the
...
As we discussed in <<Classes>>, every class has a
...
of inheritance and polymorphism in <<Polymorphism>> and
...
<<Classes>> and then move onto an inheritance hierarchy
of polygons.

.Graduate student roster
====

The `Student` class we create in <<Classes>> is useful
...
discuss the `Graphics` class in <<Constructing Graphical User Interfaces>>. If you compile and run this code, please note that in
...
1. [[exercise11.1]] Give three advantages of using inheritance instead of copying and
pasting code from a parent class. Are there any disadvantages to using
inheritance?
2. [[exercise11.2]] Consider classes `Radish` and `Carrot` which both extend class
...
3. [[exercise11.3]] In the context of inheritance, the keyword `super` can be used for
two different purposes. What are they?
4. [[exercise11.4]] Consider the following class definitions.
+
[source,java]
----
public class A {
...
----
5. [[exercise11.5]] Beginning Java programmers often confuse package-private access (no
...
6. [[exercise11.6]] What are the similarities and differences between overloading a
method and overriding a method?
7. [[exercise11.7]] What is field hiding? How can software bugs arise from this
functionality in Java?
8. [[exercise11.8]] Give reasons why the designers of Java decided not to allow multiple
inheritance. Would you have made the same decision? Why or why not?
9. [[exercise11.9]] Draw a class hierarchy establishing a sensible relationship between
...
10. [[exercise11.10]] Create an `InternationalStudent` class that extends `Student`. It
...
fields.
11. [[exercise11.11]] Add `Pentagon` and `Hexagon` classes that extend the `Polygon`
...
12. [[exercise11.12]] The inheritance design of our solution to the Boolean circuits
...
extra classes.
13. [[exercise11.13]] Re-implement the object hierarchy in the solution to the sort it out
problem from <<Interfaces>>. This time, let the `Cat`,
...
14. [[exercise11.14]] Design a celestial body simulator. You will
...
+
*Experiments*
15. [[exercise11.15]] Inheritance is a powerful technique, but it comes with some overhead
...
<<File I/O>> to write a program to create all these
...
<<Concurrent Programming>> to accurately time these
