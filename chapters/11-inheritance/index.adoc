...
In <<chapter:Selection>>, we talked extensively about Boolean
...
subject in <<chapter:Polymorphism>>.
...
problem from <<chapter:Interfaces>>, multiple inheritance would
...
interfaces from <<chapter:Interfaces>>, which specified the
...
We introduced access modifiers in <<subsection:Access_modifiers>>, but inheritance gives them new meaning. Recall that the
...
As we discussed in <<chapter:Classes>>, every class has a
...
of inheritance and polymorphism in <<chapter:Polymorphism>> and
how to build lists and other data structures using `Object` references
in <<chapter:Dynamic_Data_Structures>>.
...
<<chapter:Classes>> and then move onto an inheritance hierarchy
of polygons.

.Graduate student roster
====

The `Student` class we create in <<chapter:Classes>> is useful
...
discuss the `Graphics` class in <<chapter:Constructing_Graphical_User_Interfaces>>. If you compile and run this code, please note that in
...
<<syntax:Threads_in_Java>>. Extending the `Thread` class to make
...
with exceptions in <<subsection:Inheritance_and_exceptions>>.
...
problem given in <<solution:Boolean_circuits>> makes adding new
...
problem from <<chapter:Interfaces>>. This time, let the `Cat`,
...
<<chapter:File_I/O>> to write a program to create all these
...
<<chapter:Concurrent_Programming>> to accurately time these
