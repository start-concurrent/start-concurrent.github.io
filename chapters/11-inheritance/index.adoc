...
subject in Chapter <<chapter:Polymorphism>>.
...
[source, java, linenums]
[[program:Flounder, Flounder]]
----
include::programs/Flounder.java[]
----
...
problem from Chapter <<chapter:Interfaces>>, multiple inheritance would
...
interfaces from Chapter <<chapter:Interfaces>>, which specified the
...
We introduced access modifiers in Section <<subsection:Access_modifiers>>, but inheritance gives them new meaning. Recall that the
...
As we discussed in Chapter <<chapter:Classes>>, every class has a
...
[source, java, linenums]
[[program:LazyFish, LazyFish]]
----
include::programs/LazyFish.java[]
----
...
[source, java, linenums]
[[program:FastFish, FastFish]]
----
include::programs/FastFish.java[]
----
...
|`finalize()` |Perform cleanup when an object is garbage collected. Similar to a destructor in C++. Rarely used.
...
of inheritance and polymorphism in Chapter <<chapter:Polymorphism>> and
how to build lists and other data structures using `Object` references
in Chapter <<chapter:Dynamic_Data_Structures>>.
...
Chapter <<chapter:Classes>> and then move onto an inheritance hierarchy
of polygons.

.Graduate student roster
====

The `Student` class we create in Chapter <<chapter:Classes>> is useful
...
[source, java, linenums]
[[program:GraduateStudent, GraduateStudent]]
----
include::programs/GraduateStudent.java[]
----
...
[source, java, linenums]
----
include::programs/Polygon.java[lines=1..8]
----
...
[source, java, linenums]
----
include::programs/Polygon.java[lines=10..25]
----
...
discuss the `Graphics` class in Chapter <<chapter:Constructing_Graphical_User_Interfaces>>. If you compile and run this code, please note that in
...
[source, java, linenums]
----
include::programs/Triangle.java[lines=1..14]
----
...
[source, java, linenums]
----
include::programs/Triangle.java[lines=16..26]
----
...
[source, java, linenums]
[[program:Rectangle, Rectangle]]
----
include::programs/Rectangle.java[]
----
...
[source, java, linenums]
[[program:UnaryOperator, UnaryOperator]]
----
include::programs/UnaryOperator.java[]
----
...
[source, java, linenums]
[[program:BinaryOperator, BinaryOperator]]
----
include::programs/BinaryOperator.java[]
----
...
[source, java, linenums]
----
include::programs/BooleanCircuit.java[lines=1..9]
----
...
[source, java, linenums]
----
include::programs/BooleanCircuit.java[lines=11..30]
----
...
[source, java, linenums]
----
include::programs/BooleanCircuit.java[lines=32..49]
----
...
[source, java, linenums]
----
include::programs/BooleanCircuit.java[lines=51..56]
----
...
Section <<syntax:Threads_in_Java>>. Extending the `Thread` class to make
your own customized threads of execution is an alternative to
implementing the `Runnable` interface mentioned in
Section <<concurrency:Interfaces>> and is discussed in greater detail in
...
with exceptions in Section <<subsection:Inheritance_and_exceptions>>.
...
inheritance?
2.  Consider classes `Radish` and `Carrot` which both extend class
...
3.  In the context of inheritance, the keyword `super` can be used for
...
----
5.  Beginning Java programmers often confuse package-private access (no
...
6.  What are the similarities and differences between overloading a
method and overriding a method?
7.  What is field hiding? How can software bugs arise from this
functionality in Java?
8.  Give reasons why the designers of Java decided not to allow multiple
inheritance. Would you have made the same decision? Why or why not?
9.  Draw a class hierarchy establishing a sensible relationship between
...
10. Create an `InternationalStudent` class that extends `Student`. It
should include `String` fields for country of origin and also visa
status. It should include mutator and accessor methods for these two new
fields.
11. Add `Pentagon` and `Hexagon` classes that extend the `Polygon`
class. The constructor for each class should take an latexmath:[x],
...
12. The inheritance design of our solution to the Boolean circuits
problem given in Section <<solution:Boolean_circuits>> makes adding new
...
extra classes.
13. Re-implement the object hierarchy in the solution to the sort it out
problem from Chapter <<chapter:Interfaces>>. This time, let the `Cat`,
`Dog`, and `Person` classes extend the `Creature` class defined below.
+
[source, java, linenums]
[[program:Creature, Creature]]
----
include::programs/Creature.java[]
----

+
Refactor your code so that the `Cat`, `Dog`, and `Person` classes are as
short as possible. How many lines of code do you save?
14. <<exercise:celestial>> Design a celestial body simulator. You will
need to create a class containing fields for the latexmath:[x],
latexmath:[y], and latexmath:[z] locations, latexmath:[x],
latexmath:[y], and latexmath:[z] velocities, radii, and masses of
each object. For each time step of length latexmath:[t], you must do
the following.
a.  Compute the sum of forces exerted on each body by every other body.
...
b.  Compute the latexmath:[x], latexmath:[y], and latexmath:[z]
...
c.  Update the latexmath:[x], latexmath:[y], and latexmath:[z]
...
+
*Experiments*
15. Inheritance is a powerful technique, but it comes with some overhead
...
Chapter <<chapter:File_I/O>> to write a program to create all these
...
Chapter <<chapter:Concurrent_Programming>> to accurately time these
