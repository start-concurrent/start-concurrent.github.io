...
7.  Refer to the sort given in Section REF=solution:Sort it out as the
...
functionality.
8.  <<exercise:parallel bubble sort>> Again refer to the sort from
Section REF=solution:Sort it out. The goal now is to parallelize the
...
9.  <<exercise:use atomics>> Re-implement the original
`SynchronizedAccount` class from Example REF=example:Bank account using
...
Exercise REF=exercise:parallel bubble sort, time it against the
...
expect?
11. <<exercise:test atomics>> Take Program REF=program:NoRaceCondition
