...
In Chapter <<chapter:Synchronization>>, we introduced the
...
In Chapter <<chapter:Inheritance>> you were exposed to concepts in
...
discussed in Chapter <<chapter:Dynamic_Data_Structures>>.)
...
System.out.println( sale.discount );
----
2.  What are the differences and similarities between abstract classes
and interfaces?
3.  Assume that the `Corn`, `Carrot`, and `Potato` classes are all
...
----
4.  How many different meanings of the keyword `final` are there in
Java, and what does each mean?
5.  Assume that `Quicksand` is a subclass of `Danger`.
+
...
6.  Implement a program to assess income tax on normal employees,
...
7.  Refer to the sort given in Section <<solution:Sort_it_out>> as the
...
functionality.
8.  <<exercise:parallel_bubble_sort>> Again refer to the sort from
Section <<solution:Sort_it_out>>. The goal now is to parallelize the
...
9.  <<exercise:use_atomics>> Re-implement the original
`SynchronizedAccount` class from Example <<example:Bank_account>> using
...
+
*Experiments*
10. Once you have implemented the sort in parallel from
Exercise <<exercise:parallel_bubble_sort>>, time it against the
...
expect?
11. <<exercise:test_atomics>> Take Program <<program:NoRaceCondition>>
