...
In <<chapter:Synchronization>>, we introduced the
...
In <<chapter:Inheritance>> you were exposed to concepts in
...
discussed in <<chapter:Dynamic_Data_Structures>>.)
...
7.  Refer to the sort given in <<solution:Sort_it_out>> as the
...
<<solution:Sort_it_out>>. The goal now is to parallelize the
