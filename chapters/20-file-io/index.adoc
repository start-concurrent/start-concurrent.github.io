...
[[program:BaseballPlayer, BaseballPlayer]]
----
...
.Program to read a list of radiuses from a text file and output their areas to another file.
[source, java, linenums]
[[program:AreaFromRadiusText, AreaFromRadiusText]]
----
...
.Program to read a list of radiuses from a binary file and output their areas to another file.
[source, java, linenums]
[[program:AreaFromRadiusBinary, AreaFromRadiusBinary]]
----
...
.Program that spawns threads that print odd and even numbers to a file concurrently.
[source, java, linenums]
[[program:ConcurrentFileAccess, ConcurrentFileAccess]]
----
...
call `out.flush()` after line <<output_data_concurrently>>, we can flush
...
1. [[exercise20.1]] What is the difference between volatile and non-volatile memory?
Which is often associated with files and why?
2. [[exercise20.1]] What is the difference between text and binary files? What are the
pros and cons of using each?
3. [[exercise20.3]] Define compression ratio to be the size of the uncompressed data in
...
4. [[exercise20.4]] What is serialization in Java? What do you have to do to serialize
an object?
5. [[exercise20.5]] What kinds of objects cannot be serialized?
+
*Programming Practice*
6. [[exercise20.6]] Implement the RLE bitmap compression program using only
`FileInputStream` and `FileOutputStream` for file input and output.
7.  [[exercise20.7]] Re-implement the maze solving program from <<Solution: Maze of doom>> to ask the user for a file instead of reading from standard
input.
8. [[exercise20.8]] An HTML file contains many tags such as `<p>`, which marks the
...
9.  [[exercise20.9]] Write a program that prompts the user for an
...
described in <<example:Binary_search_tree>>. Then, traverse the
tree, printing all the words found (and the number of times found) to
the screen in alphabetical order.
10. [[exercise20.10]] Expand the program from <<exercise20.9, Exercise 20.9>> so that it
...
(http://www.gutenberg.org/) and a dictionary file from an open source
spell checker or from a Scrabble word list.
11. [[exercise20.11]] Files can become corrupted when they are transferred over a network.
...
a.  Add up the values of all the bytes, storing this sum in a `long`
variable
b.  Set latexmath:[sum = sum\mod 2^{32}]
c.  Let latexmath:[r = (sum\mod 2^{16}) + (sum \div 2^{16})]
d.  Let latexmath:[s = (r\mod 2^{16}) + (r \div 2^{16})]
e.  The final checksum is latexmath:[s]
+
...
+
*Experiments*
12. [[exercise20.12]] Write the RLE bitmap compression program in parallel so that a file
