...
7.  Re-implement the maze solving program from Section REF=solution:Maze
...
9.  <<exercise:word list>> Write a program that prompts the user for an
...
described in Example REF=example:Binary search tree. Then, traverse the
tree, printing all the words found (and the number of times found) to
the screen in alphabetical order.
10. Expand the program from Exercise REF=exercise:word list so that it
