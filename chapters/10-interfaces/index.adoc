...
[[section-Problem_Sort_it_out]]
=== Problem: Sort it out

Defining classes as we did in <<chapter:Classes>> provides us
...
<<chapter:Inheritance>>, we will talk about hierarchical
...
If you have not read <<advanced:Nested_classes>>, you may want
...
We’ll see in <<chapter:Inheritance>> that the `Dog`, `Cat`, and
...
cover generics in <<chapter:Dynamic_Data_Structures>>.

=== Concurrency: Interfaces

As we discussed in <<concepts:Making_a_promise>>, implementing
...
In <<chapter:Synchronization>>, we will discuss a key way to
...
the solution from <<solution:Sort_it_out>>. Design and include
...
13. Refer to the sort given as a solution in <<solution:Sort it
...
<<chapter:Concurrent_Programming>>, refer to the simple bubble
sort from <<problem:Sort_it_out>>. The goal is now to
