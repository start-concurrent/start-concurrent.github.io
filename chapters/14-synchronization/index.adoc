...
threads execute. As explained in <<concepts:Thread_scheduling>>,
...
recall the threadstate diagram from <<chapter:Concurrent
...
In <<chapter:Concurrent_Programming>> we suggest that you use
...
<<subsection:deadlock>>. Unlike that example, we include a
...
<<concepts:Thread_interaction>>. We can rewrite the `run()`
method as follows.
+
[source,java]
----
...
11. <<exercise:fair_bank_account>> In <<example:Bank_account>>
we used the class `SynchronizedAccount` to solve a bank account problem.
As we mention in <<section:starvation_and_livelock>>, depositing
...
philosophers problem given in <<solution:dining_philosophers>>
