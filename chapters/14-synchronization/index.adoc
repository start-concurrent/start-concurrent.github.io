...
Exercise REF=exercise:interleavings two snippets of code could run
...
==== Begin Example: Producer/consumer
...
==== Begin Example: Deadlock philosophers
...
implementation in Example REF=example:Bank account for the bank account.
...
==== Begin Example: Livelock party preparations
...
implementation of matrix multiply given in Example REF=example:Matrix
multiplication and use the `synchronized` keyword to lock on the matrix
itself.
...
1.  <<exercise:synchronized keyword>> What is the purpose of the
`synchronized` keyword? How does it work?
2.  <<exercise:constructor leak>> The language specification for Java
...
way?
3.  <<exercise:wait or notify without synchronized>> If you call
...
6.  <<exercise:race condition>> What is a race condition? Give a real
life example of one.
7.  <<exercise:per object lock>> Let’s reexamine the code that
increments a variable with several threads from
Section REF=concepts:Thread interaction. We can rewrite the `run()`
method as follows.
+
....
...
8.  <<exercise:deadlock conditions>> Examine our deadlock example from
Example REF=example:Deadlock sum. Explain why this example fulfills all
four conditions for deadlock. Be specific about which threads and which
resources are needed to show each condition.
9.  <<exercise:priority inversion>> What is priority inversion? Why can
a low priority thread holding a lock be particularly problematic?
+
*Programming Practice*
10. <<exercise:better producer/consumer>> In
Example REF=example:Producer/consumer the `Buffer` class used to
...
11. <<exercise:fair bank account>> In Example REF=example:Bank account
we used the class `SynchronizedAccount` to solve a bank account problem.
As we mention in Section REF=section:starvation and livelock, depositing
...
12. <<exercise:philosophers with hunger>> The solution to the dining
philosophers problem given in Section REF=solution:dining philosophers
...
+
*Experiments*
13. <<exercise:time lock acquisition>> Critical sections can slow down
...
system.
14. <<exercise:thread time on CPU>> Design a program which
...
15. <<exercise:priority inversion experiment>> Create an experiment to
