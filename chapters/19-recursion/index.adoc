...
The evil mastermind from <<Concurrent Programming>> has
...
<<Syntax: Recursive data structures>>. <<recursiveTrees>>
...
the stack abstract data type in <<Dynamic Data Structures>>. A similar structure is used to control the flow of control
...
****
<<exercise19.6, Exercise 19.6>> +
<<exercise19.8, Exercise 19.8>>
****
...
The linked list examples from <<Dynamic Data Structures>> are recursive data structures, since a linked list node is
...
****
<<exercise19.9, Exercise 19.9>> +
<<exercise19.10, Exercise 19.10>>
****
...
****
<<exercise19.11, Exercise 19.11>> +
<<exercise19.12, Exercise 19.12>>
****
...
in <<Dynamic Data Structures>>, we want our data
...
1.  [[exercise19.1]] <<example:Multiplication_defined_recursively>> gave a mathematical recursive definition for
...
2.  [[exercise19.2]] In principle, every problem that can be solved with an iterative
...
3.  [[exercise19.3]] Consider the first (non-memoized)
...
4.  [[exercise19.4]] In the recursive `solve()` method in the `MazeSolver` program given
...
5.  [[exercise19.5]] <<exercise8.11, Exercise 8.11>>
from <<Methods>> challenges you to write a method to
...
----
6.  [[exercise19.6]] The efficient implementation of
...
7.  [[exercise19.7]] Write an implementation of
...
\end{cases}]
8.  [[exercise19.8]] <<example:Fibonacci_implemented_recursively>> shows two implementations that can be used to
...
exponentation from <<exercise19.7, Exercise 19.7>> to
...
9.  [[exercise19.9]] <<example:Recursive_linked_list_size>> shows a way to calculate the size of a linked list
...
10. [[exercise19.10]] Expand the previous exercise
...
written.
11. [[exercise19.11]] Create a recursive `find()` method (and a
...
12. [[exercise19.12]] The height of a binary tree is defined as
...
its two subtrees.
13. [[exercise19.13]] Create an `interface` that
describes a tree ADT. Modify the programs in <<example:Binary_search_tree>> to use this `interface`.
+
*Experiments*
14. [[exercise19.14]] Write an iterative version of the factorial function and compare its
...
15. [[exercise19.15]] Write a program that generates four arrays
...
16. [[exercise19.16]] Investigate the performance of using recursion to compute Fibonacci
...
recursive solution.)
17. [[exercise19.17]] <<exercise6.9, Exercise 6.9>> from <<Arrays>> explains how binary search can be used to search for a value in a sorted
