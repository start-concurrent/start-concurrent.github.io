...
.A variant of Program REF=program:Tree that stores`Integer` values instead of `String` values.
[source, java, linenums]
----
...
1.  <<exercise:recursive addition>> Example REF=example:Multiplication
...
3.  <<exercise:inefficient Fibonacci>> Consider the first (non-memoized)
recursive version of the Fibonacci method given in
Example REF=example:Fibonacci implemented recursively. How many times is
...
in Section REF=solution:Maze of doom, the current location in the maze
...
5.  <<exercise:recursive palindromes>> Exercise REF=exercise:palindromes
from Chapter REF=chapter:Methods challenges you to write a method to
...
....
6.  <<exercise:Fibonacci without array>> The efficient implementation of
Fibonacci from Example REF=example:Fibonacci implemented recursively
...
7.  <<exercise:efficient exponentiation>> Write an implementation of
...
Example REF=example:Exponentiation except when latexmath:[n] is odd.
...
and odd}
\end{cases}]
8.  <<exercise:efficient Fibonacci>> Example REF=example:Fibonacci
...
exponentation from Exercise REF=exercise:efficient exponentiation to
...
9.  <<exercise:print linked list>> Example REF=example:Recursive linked
...
10. <<exercise:reverse print linked list>> Expand the previous exercise
...
written.
11. <<exercise:tree find>> Create a recursive `find()` method (and a
non-recursive proxy method to call it) for the `Tree` class given in
Program REF=program:Tree. Its operation is similar to the `add()`
...
12. <<exercise:tree height>> The height of a binary tree is defined as
...
describes a tree ADT. Modify the programs in Example REF=example:Binary
...
15. <<exercise:sort timing>> Write a program that generates four arrays
...
Example REF=example:Selection sort, the bubble sort algorithm given in
Section REF=problem:Sort it out, and the merge sort algorithm given in
Example REF=example:Merge sort, respectively. Use the
...
recursive solution.)
17. Exercise REF=exercise:binary search from Chapter REF=chapter:Arrays
