...
determine your current age given in Section <<subsection:What_is_recursion>>.
...
\end{cases}]
8.  <<exercise:efficient_Fibonacci>> Example <<example:Fibonacci_implemented_recursively>> shows two implementations that can be used to
...
9.  <<exercise:print_linked_list>> Example <<example:Recursive_linked_list_size>> shows a way to calculate the size of a linked list
...
describes a tree ADT. Modify the programs in Example <<example:Binary_search_tree>> to use this `interface`.
+
*Experiments*
...
Section <<section:Problem:_Sort_it_out>>, and the merge sort algorithm given in
