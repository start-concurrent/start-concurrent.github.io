...
The evil mastermind from <<chapter:Concurrent_Programming>> has
...
<<syntax:Recursive_data_structures>>. Figure <<recursiveTrees>>
...
the stack abstract data type in <<chapter:Dynamic Data
...
The linked list examples from <<chapter:Dynamic Data
...
in <<chapter:Dynamic_Data_Structures>>, we want our data
...
1.  <<exercise:recursive_addition>> <<example:Multiplication_defined_recursively>> gave a mathematical recursive definition for
latexmath:[x \times y]. Give a similar recursive definition for
latexmath:[x + y]. The structure is similar to the recursion to
determine your current age given in <<subsection:What_is_recursion>>.
...
<<example:Fibonacci_implemented_recursively>>. How many times is
...
in <<solution:Maze_of_doom>>, the current location in the maze
...
from <<chapter:Methods>> challenges you to write a method to
...
Fibonacci from <<example:Fibonacci_implemented_recursively>>
...
<<example:Exponentiation>> except when latexmath:[n] is odd.
...
\end{cases}]
8.  <<exercise:efficient_Fibonacci>> <<example:Fibonacci_implemented_recursively>> shows two implementations that can be used to
...
9.  <<exercise:print_linked_list>> <<example:Recursive_linked_list_size>> shows a way to calculate the size of a linked list
...
describes a tree ADT. Modify the programs in <<example:Binary_search_tree>> to use this `interface`.
+
*Experiments*
...
<<example:Selection_sort>>, the bubble sort algorithm given in
<<section:Problem:_Sort_it_out>>, and the merge sort algorithm given in
<<example:Merge_sort>>, respectively. Use the
...
recursive solution.)
17. Exercise <<exercise:binary_search>> from <<chapter:Arrays>>
