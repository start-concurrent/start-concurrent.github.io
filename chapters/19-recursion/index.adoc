...
The evil mastermind from Chapter <<chapter:Concurrent_Programming>> has
...
Section <<syntax:Recursive_data_structures>>. Figure <<recursiveTrees>>
...
==== Begin Example: Multiplication defined recursively
...
==== Begin Example: Factorial defined recursively
...
==== Begin Example: Fibonacci defined recursively
...
the stack abstract data type in Chapter <<chapter:Dynamic Data
...
Figure <<figure:recursive_calls>> shows the stack frames being pushed
...
image::recursivecalls.png[]


Figure <<figure:recursive_returns>> shows the stack frames popping off
...
==== Begin Example: Factorial implemented recursively
...
==== Begin Example: Fibonacci implemented recursively
...
Figure <<figure:hanoi>> shows an example of the puzzle. The goal is to
...
.A recursive solution to the Tower of Hanoi with four disks and poles named `'A'`, `'B'`, and `'C'`.
[source, java, linenums]
----
include::programs/TowerOfHanoi.java[]
----
...
The linked list examples from Chapter <<chapter:Dynamic Data
...
==== Begin Example: Recursive linked list size

How would you get the size of a linked list? The implementation in
Program <<program:LinkedList>> keeps track of its size as it grows, but
...
Program <<program:IterativeListSize>> counts the elements in this way.

.Linked list implementation whose `size()` method counts its elements iteratively.
[source, java, linenums]
----
include::programs/IterativeListSize.java[]
----
...
Program <<program:RecursiveListSize>> counts the elements in a linked
...
[source, java, linenums]
----
include::programs/RecursiveListSize.java[]
----
...
Figure <<figure:tree_visualization>> shows a visualization of a tree. In
...
and print them out in alphabetical order. Program <<program:Tree>> shows
...
.A class that implements a simple binary search tree ADT for creating a sorted list of strings.
[source, java, linenums]
----
include::programs/Tree.java[]
----

Figure <<figure:tree_classes>> shows a visualization of the contents of
...
order. Program <<program:ReadAndSortStrings>> gives an example of this
process using a `Tree` object for storage.

.A program to read `String` values, store them in a binary search tree, and print the results in sorted order.
[source, java, linenums]
----
include::programs/ReadAndSortStrings.java[]
----
...
==== Begin Example: Binary search tree to hold integers

Consider Program <<program:IntegerTree>>, which implements a tree that
...
.A variant of Program <<program:Tree>> that stores`Integer` values instead of `String` values.
[source, java, linenums]
----
include::programs/IntegerTree.java[]
----
...
in Chapter <<chapter:Dynamic_Data_Structures>>, we want our data
structures, recursive or otherwise, to hold any type. In this way, we
can reuse code.

==== Begin Example: Defining a generic binary search tree

Program <<program:GenericTree>> defines a generic version of the `Tree`
...
.A class that implements a generic tree.
[source, java, linenums]
----
include::programs/GenericTree.java[]
----
...
==== Begin Example: Using a generic class

Program <<program:ReadAndSortGenerics>> uses the generic tree class to
...
.Program to create two trees with different underlying types.
[source, java, linenums]
----
include::programs/ReadAndSortGenerics.java[]
----
...
[source, java, linenums]
----
include::programs/MazeSolver.java[lines=1..14]
----
...
[source, java, linenums]
----
include::programs/MazeSolver.java[lines=16..29]
----
...
[source, java, linenums]
----
include::programs/MazeSolver.java[lines=31..37]
----
...
[source, java, linenums]
----
include::programs/MazeSolver.java[lines=39..57]
----
...
(line <<mazeRRecursive>>). If any of those four neighbors returns
...
==== Begin Example: Futures to sum square roots
...
[source, java, linenums]
----
include::programs/RootSummer.java[]
----
...
[source, java, linenums]
----
include::programs/RootFutures.java[lines=1..14]
----
...
[source, java, linenums]
----
include::programs/RootFutures.java[lines=16..22]
----
...
[source, java, linenums]
----
include::programs/RootFutures.java[lines=24..40]
----
...
1.  <<exercise:recursive_addition>> Example <<example:Multiplication
defined recursively>> gave a mathematical recursive definition for
latexmath:[x \times y]. Give a similar recursive definition for
latexmath:[x + y]. The structure is similar to the recursion to
determine your current age given in Section <<subsection:What is
recursion?>>.
2.  In principle, every problem that can be solved with an iterative
...
3.  <<exercise:inefficient_Fibonacci>> Consider the first (non-memoized)
recursive version of the Fibonacci method given in
Example <<example:Fibonacci_implemented_recursively>>. How many times is
`fibonacci()` called with argument latexmath:[1] to compute
`fibonacci(n)`? Instrument your program and count the number of calls
for latexmath:[n = 2, 3, 4 \ldots 20].
4.  In the recursive `solve()` method in the `MazeSolver` program given
in Section <<solution:Maze_of_doom>>, the current location in the maze
...
5.  <<exercise:recursive_palindromes>> Exercise <<exercise:palindromes>>
from Chapter <<chapter:Methods>> challenges you to write a method to
determine whether a `String` contains a palindrome. Recall that a
palindrome (if punctuation and spaces are ignored) can be described as a
`String` in which the first and last characters are equal, and all the
characters in between form a palindrome. Write a recursive method to
test if a `String` is a palindrome, using the following signature.
+
....
...
....
6.  <<exercise:Fibonacci_without_array>> The efficient implementation of
Fibonacci from Example <<example:Fibonacci_implemented_recursively>>
...
7.  <<exercise:efficient_exponentiation>> Write an implementation of
fast exponentiation that works for even and odd latexmath:[n]. This
implementation is exactly the same as the one given at the end of
Example <<example:Exponentiation>> except when latexmath:[n] is odd.
...
and odd}
\end{cases}]
8.  <<exercise:efficient_Fibonacci>> Example <<example:Fibonacci
implemented recursively>> shows two implementations that can be used to
find the latexmath:[n]^th^ Fibonacci number. With a little bit of
math, it is possible to show that there is a closed-form equation that
gives the latexmath:[n]^th^ Fibonacci number latexmath:[F_n] where
latexmath:[F_0 = F_1 = 1].
+
...
exponentation from Exercise <<exercise:efficient_exponentiation>> to
make an implementation that finds the latexmath:[n]^th^ Fibonacci
number *very* quickly.
+
...
9.  <<exercise:print_linked_list>> Example <<example:Recursive linked
list size>> shows a way to calculate the size of a linked list
...
10. <<exercise:reverse_print_linked_list>> Expand the previous exercise
to add another method called `reversePrint()` that prints out the values
in the linked list the *opposite* order that they appear. It should take
only a slight modification of the `print()` method you have already
written.
11. <<exercise:tree_find>> Create a recursive `find()` method (and a
non-recursive proxy method to call it) for the `Tree` class given in
Program <<program:Tree>>. Its operation is similar to the `add()`
method. If the subtree it is examining is empty (`null`), it should
return `false`. If the value it is looking for is at the root of the
...
12. <<exercise:tree_height>> The height of a binary tree is defined as
...
Program <<program:Tree>>. The base case is an empty tree (a `null`
...
its two subtrees.
13. <<exercise:setInterfaceExercise>> Create an `interface` that
describes a tree ADT. Modify the programs in Example <<example:Binary
search tree>> to use this `interface`.
+
*Experiments*
14. Write an iterative version of the factorial function and compare its
...
15. <<exercise:sort_timing>> Write a program that generates four arrays
...
Example <<example:Selection_sort>>, the bubble sort algorithm given in
Section <<problem:Sort_it_out>>, and the merge sort algorithm given in
Example <<example:Merge_sort>>, respectively. Use the
...
16. Investigate the performance of using recursion to compute Fibonacci
...
recursive solution.)
17. Exercise <<exercise:binary_search>> from Chapter <<chapter:Arrays>>
...
`System.currentTimeMillis()` or `System.nanoTime()` methods to time each
process. Was the iterative or recursive approach faster? By how much?
