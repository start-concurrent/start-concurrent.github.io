...
<<subsection:class_variables>> that class variables can be
...
they are created and used, we discuss them in <<advanced:Local
...
is discussed in <<chapter:Exceptions>>. Some critics of Java
...
<<chapter:Concurrent_Programming>>, we discuss how to create
...
In <<chapter:Synchronization>>, we discuss how to prevent this
...
and manipulate `int` values. In <<chapter:Dynamic Data
...
9.  In <<solution:Nested_expressions>>, we gave a definition of
...
14. Re-implement the solution from <<solution:Nested
