...
.Example of a class encapsulating the attributes of a human being.
...
[source, java, linenums]
[[program:SymbolStack, SymbolStack]]
----
...
.Program to record the number of lives a cat has left. Each `Cat` object starts with 9, but loses one each time it uses its curiosity. If no more lives remain, an error message is output.
[source, java, linenums]
[[program:Cat, Cat]]
----
