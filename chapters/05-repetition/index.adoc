...
In <<chapter:Selection>>, you learned tools that allow you to do
...
.Binary Conversion
====
As we discussed in <<chapter:Computer_Basics>>, binary numbers
...
topics are discussed further in <<concepts:Splitting_up_work>>.
...
In <<chapter:Concurrent_Programming>>, we will introduce tools
...
This sines example is similar to <<example:Array_summation>>
which we will cover in in <<chapter:Concurrent_Programming>>. As
...
2.  <<exercise:primalityroot>> In <<example:Primality_testing>>,
...
problem given in <<solution:DNA_searching>>, we used two `for`
...
<<solution:DNA_searching>> using `JOptionPane` to generate GUIs
...
13. In <<subsection:common_pitfalls>>, one of the common loop
...
an `int`?
14. In <<example:Primality_testing>>, we gave three programs to
...
`PrimalityTester2`?
15. <<exercise:primes_less_than>> In <<example:Parallelism
...
knowledge of primes from <<example:Primality_testing>>) to write
