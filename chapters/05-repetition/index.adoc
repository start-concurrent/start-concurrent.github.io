...
In Chapter <<chapter:Selection>>, you learned tools that allow you to do
...
condition remains `true`. Figure <<figure:while>> shows the pattern of
...
.Binary Conversion
====
As we discussed in Chapter <<chapter:Computer_Basics>>, binary numbers
...
.This program outputs a binary representation of a decimal number.
[source, java, linenums]
[[program:DecimalToBinary, DecimalToBinary]]
----
include::programs/DecimalToBinary.java[]
----
...
semicolons. Figure <<figure:for>> shows the pattern of execution for
`for` loops.
...
.This program gives a naive approach for testing if a number is prime.
[source, java, linenums]
[[program:PrimalityTester0, PrimalityTester0]]
----
include::programs/PrimalityTester0.java[]
----
...
.This program gives a slightly cleverer approach for testing if a number is prime.
[source, java, linenums]
[[program:PrimalityTester1, PrimalityTester1]]
----
include::programs/PrimalityTester1.java[]
----
...
.This program gives a much faster approach for testing if a number is prime.
[source, java, linenums]
[[program:PrimalityTester2, PrimalityTester2]]
----
include::programs/PrimalityTester2.java[]
----
...
.This program finds the reverse complement of a DNA sequence.
[source, java, linenums]
[[program:ReverseComplement, ReverseComplement]]
----
include::programs/ReverseComplement.java[]
----
...
.This program more cleverly finds the reverse complement of a DNA sequence.
[source, java, linenums]
[[program:CleverReverseComplement, CleverReverseComplement]]
----
include::programs/CleverReverseComplement.java[]
----
====
...
Figure <<figure:do-while>> shows the pattern of execution for `do-while`
loops.
...
.This program prints out triangular numbers.
[source, java, linenums]
[[program:TriangularNumbers, TriangularNumbers]]
----
include::programs/TriangularNumbers.java[]
----
...
.This program prints out triangular numbers more cleverly.
[source, java, linenums]
[[program:CleverTriangularNumbers, CleverTriangularNumbers]]
----
include::programs/CleverTriangularNumbers.java[]
----
...
[source, java, linenums]
----
include::programs/DNASearch.java[lines=1..8]
----
...
[source, java, linenums]
----
include::programs/DNASearch.java[lines=10..22]
----
...
[source, java, linenums]
----
include::programs/DNASearch.java[lines=24..36]
----
...
[source, java, linenums]
----
include::programs/DNASearch.java[lines=38..49]
----
...
[source, java, linenums]
----
include::programs/DNASearch.java[lines=51..56]
----
...
topics are discussed further in Section <<concepts:Splitting_up_work>>.
...
In Chapter <<chapter:Concurrent_Programming>>, we will introduce tools
...
.Program to add the sines of all integers in a range specified by the user.
[source, java, linenums]
[[program:SumSines, SumSines]]
----
include::programs/SumSines.java[]
----
...
This sines example is similar to Example <<example:Array_summation>>
which we will cover in in Chapter <<chapter:Concurrent_Programming>>. As
...
2.  <<exercise:primalityroot>> In Example <<example:Primality_testing>>,
...
----
3.  How many different DNA sequences of length latexmath:[n] are
there?
4.  There are three different errors in the following loop intended to
...
6.  Write a program that converts base 10 numbers into base 3 numbers.
...
F, in order, to represent digits larger than 9.
7.  The greatest common divisor (GCD) of two integers is the largest
...
8.  <<exercise:String_indexOf>> In the solution to the DNA searching
problem given in Section <<solution:DNA_searching>>, we used two `for`
...
9.  Write a program that reads a number latexmath:[n] from a user and
then prints all possible DNA sequences of length latexmath:[n]. Be
...
10. Re-implement the solution to the DNA searching program given in
Section <<solution:DNA_searching>> using `JOptionPane` to generate GUIs
for input and output.
+
*Experiments*
11. Using a `for` loop, record the Monty Hall simulation so that you can
...
12. Write three nested `for` loops, each of which run 1,000 times.
...
are on a Unix or Linux system, the `time` command. Feel free to increase
...
you may have to wait longer than you want.
13. In Section <<subsection:common_pitfalls>>, one of the common loop
...
an `int`?
14. In Example <<example:Primality_testing>>, we gave three programs to
...
`PrimalityTester2`?
15. <<exercise:primes_less_than>> In Example <<example:Parallelism
...
knowledge of primes from Example <<example:Primality_testing>>) to write
