...
This sines example is similar to Example REF=example:Array summation
which we will cover in in Chapter REF=chapter:Concurrent Programming. As
...
2.  <<exercise:primalityroot>> In Example REF=example:Primality testing,
...
8.  <<exercise:String indexOf>> In the solution to the DNA searching
problem given in Section REF=solution:DNA searching, we used two `for`
...
13. In Section REF=subsection:common pitfalls, one of the common loop
...
an `int`?
14. In Example REF=example:Primality testing, we gave three programs to
...
`PrimalityTester2`?
15. <<exercise:primes less than>> In Example REF=example:Parallelism
...
knowledge of primes from Example REF=example:Primality testing) to write
