...
simulation. In Chapter <<chapter:Constructing_Graphical_User_Interfaces>> you will learn how to make a graphical user interface (GUI)
...
Chapter <<chapter:Concurrent_Programming>>. The key item of importance
...
in Chapter <<chapter:Dynamic_Data_Structures>>. Below we give a short
...
the same summation is discussed in Section <<subsection:The_for-each_loop>>.
...
going to talk about it in depth in Chapter <<chapter:File_I/O>>, but for
...
.This program sums a list of numbers given as input.
[source, java, linenums]
[[program:Summer, Summer]]
----
...
.This program sums a list of numbers given as input without prompting the user.
[source, java, linenums]
[[program:QuietSummer, QuietSummer]]
----
include::programs/QuietSummer.java[]
----
...
[source, java, linenums]
----
include::programs/SelectionSort.java[lines=1..11]
----
...
[source, java, linenums]
----
include::programs/SelectionSort.java[lines=12..20]
----
...
[source, java, linenums]
----
include::programs/SelectionSort.java[lines=21..25]
----
...
multiple times would be tedious. When we get to Chapter <<chapter:File
...
[source, java, linenums]
----
include::programs/WordCount.java[lines=1..11]
----
...
[source, java, linenums]
----
include::programs/WordCount.java[lines=12..20]
----
...
[source, java, linenums]
----
include::programs/WordCount.java[lines=21..24]
----
...
Example <<example:King_of_the_hill>> covered how to find the maximum and
...
latexmath:[n] be the number of data points, label each data point
latexmath:[x_i], where latexmath:[1 \leq i \leq n], and let
latexmath:[\bar{x}] be the mean of all the data points. Then, the
...
[source, java, linenums]
----
include::programs/Statistics.java[lines=1..9]
----
...
[source, java, linenums]
----
include::programs/Statistics.java[lines=10..19]
----
...
[source, java, linenums]
----
include::programs/Statistics.java[lines=20..23]
----
...
[source, java, linenums]
----
include::programs/Statistics.java[lines=24..28]
----
...
[source, java, linenums]
----
include::programs/Statistics.java[lines=29..46]
----
...
chess, we will need an 8 &times; 8 two-dimensional array. We can represent
...
Section <<concepts:Types>>, but the only reference type we have
...
[source, java, linenums]
[[program:MatrixRotate, MatrixRotate]]
----
...
[source, java, linenums]
----
include::programs/TicTacToe.java[lines=1..13]
----
...
[source, java, linenums]
----
include::programs/TicTacToe.java[lines=15..35]
----
...
[source, java, linenums]
----
include::programs/TicTacToe.java[lines=36..62 *FIX: Tic Tac Toe partial program]
----

listing (lines 36-62) not available.*
...
In Chapter <<chapter:Repetition>> we describe three loops: `while`
...
discuss interfaces in Chapter <<chapter:Interfaces>> and dynamic data
structures in Chapters <<chapter:Dynamic_Data_Structures>> and
<<chapter:Recursion>>.
...
[.center%header,width=100%,cols="<.<45%l,<.<55%"]
|=======================================================================
|Method |Purpose
...
[source, java, linenums]
----
include::programs/Life.java[lines=1..11]
----
...
[source, java, linenums]
----
include::programs/Life.java[lines=12..33]
----
...
[source, java, linenums]
----
include::programs/Life.java[lines=34..48]
----
...
exceptions in general in Chapter <<chapter:Exceptions>> and give more
...
Chapter <<chapter:Concurrent_Programming>>, we will explain how to
...
.Here is a short program that reads the length of an array and the number of threads from the user and then prints out the amount of work for each one. You should be able to adapt the ideas in it to your own multi-threaded programs in Chapter <<chapter:Concurrent Programming>>.
[source, java, linenums]
[[program:AssigningWork, AssigningWork]]
----
...
7.  <<exercise:naive_array_split>> Consider the naive method of dividing
an array of length latexmath:[n] among latexmath:[k] threads that
was discussed in Section <<concurrency:Arrays>>: Each thread gets
...
8.  <<exercise:remove_punctuation>> In Example <<example:Word_search>>,
our code did not count `ship,` as an occurrence of `ship` because of the
comma.
+
Rewrite the code from Example <<example:Word_search>> to remove
...
9.  <<exercise:binary_search>> In Example <<example:Word_search>>, we
...
+
Rewrite the code from Example <<example:Word_search>> to use binary
search, after applying selection sort from Example <<example:Selection
...
10. In Example <<example:Statistics>>, we gave a program that finds the
...
11. <<exercise:Connect_Four>> We used the example of Tic Tac Toe in
Example <<example:Tic_Tac_Toe>> because a more complex game would have
...
Chapter <<chapter:Constructing_Graphical_User_Interfaces>>, adapt the
solution to Conway's Game of Life from Section <<solution:Conway's Game
of Life>> to display on a graphical user interface. You can use a
...
search improvement from Exercise <<exercise:binary_search>>. Use the OS
...
Example <<example:Selection_sort>> running on each of these input files
