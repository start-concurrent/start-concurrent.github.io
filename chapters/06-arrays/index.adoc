...
simulation. In <<chapter:Constructing_Graphical_User_Interfaces>> you will learn how to make a graphical user interface (GUI)
...
<<chapter:Concurrent_Programming>>. The key item of importance
...
in <<chapter:Dynamic_Data_Structures>>. Below we give a short
...
going to talk about it in depth in <<chapter:File_I/O>>, but for
...
multiple times would be tedious. When we get to <<chapter:File
...
In <<chapter:Repetition>> we describe three loops: `while`
...
discuss interfaces in <<chapter:Interfaces>> and dynamic data
structures in <<chapter:Dynamic_Data_Structures>> and
<<chapter:Recursion>>.
...
exceptions in general in <<chapter:Exceptions>> and give more
information about the `Thread.sleep()` method in <<chapter:Concurrent_Programming>>.

=== Concurrency: Arrays

Arrays are critical to concurrent programming in Java. In
<<chapter:Concurrent_Programming>>, we will explain how to
...
.Here is a short program that reads the length of an array and the number of threads from the user and then prints out the amount of work for each one. You should be able to adapt the ideas in it to your own multi-threaded programs in <<chapter:Concurrent Programming>>.
[source, java, linenums]
...
8.  <<exercise:remove_punctuation>> In <<example:Word_search>>,
...
9.  <<exercise:binary_search>> In <<example:Word_search>>, we
...
10. In <<example:Statistics>>, we gave a program that finds the
...
<<chapter:Constructing_Graphical_User_Interfaces>>, adapt the
