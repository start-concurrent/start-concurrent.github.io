...
simulation. In <<Constructing Graphical User Interfaces>> you will learn how to make a graphical user interface (GUI)
...
<<Concurrent Programming>>. The key item of importance
...
in <<Dynamic Data Structures>>. Below we give a short
...
the same summation is discussed in <<subsection:The_for-each_loop>>.
...
going to talk about it in depth in <<File I/O>>, but for
...
multiple times would be tedious. When we get to <<File I/O>>, we will talk about ways to read from multiple files at the same
...
<<Concepts: Types>>, but the only reference type we have
...
First, our code creates a 7 × 7 array of type
...
discuss interfaces in <<Interfaces>> and dynamic data
structures in <<Dynamic Data Structures>> and
<<Recursion>>.
...
exceptions in general in <<Exceptions>> and give more
information about the `Thread.sleep()` method in <<Concurrent Programming>>.

=== Concurrency: Arrays

Arrays are critical to concurrent programming in Java. In
<<Concurrent Programming>>, we will explain how to
...
.Here is a short program that reads the length of an array and the number of threads from the user and then prints out the amount of work for each one. You should be able to adapt the ideas in it to your own multi-threaded programs in <<Concurrent Programming>>.
[source, java, linenums]
...
1. [[exercise6.1]] Why can't an array be used to hold an arbitrarily long list of
numbers entered by a user? What are strategies that can be used to
overcome this problem?
2. [[exercise6.2]] In future chapters, we will introduce a data structure called a
...
3. [[exercise6.3]] Given the following code:
+
[source,java]
----
...
executed?
4. [[exercise6.4]] What error will be caused by the following code, and why?
+
[source,java]
----
...
----
5. [[exercise6.5]] An array of length latexmath:[n] in Java typically takes
...
6.  [[exercise6.6]] Our original table of city distances allocates
...
7.  [[exercise6.7]] Consider the naive method of dividing
...
8.  [[exercise6.8]] In <<example:Word_search>>,
our code did not count `ship,` as an occurrence of `ship` because of the
comma.
+
Rewrite the code from <<example:Word_search>> to remove
...
9.  [[exercise6.9]] In <<example:Word_search>>, we
...
+
Rewrite the code from <<example:Word_search>> to use binary
search, after applying selection sort from <<example:Selection
...
10. [[exercise6.10]] In <<example:Statistics>>, we gave a program that finds the
...
11. [[exercise6.11]] We used the example of Tic Tac Toe in
...
12. [[exercise6.12]] Once you have mastered the material in
<<Constructing Graphical User Interfaces>>, adapt the
...
+
*Experiments*
13. [[exercise6.13]] Creating arrays with longer and longer lengths requires more
...
memory being used.
14. [[exercise6.14]] Run the implementation of the word search program using the binary
search improvement from <<exercise6.9, Exercise 6.9>>. Use the OS
...
15. [[exercise6.15]] Generate input files consisting of latexmath:[1,000],
latexmath:[10,000], and latexmath:[100,000] random `int` values.
Time our implementation of selection sort from
<<example:Selection_sort>> running on each of these input files
