...
Enter your first name:       pass:quotes[*Barry*]
Enter your last name:        pass:quotes[*Wittman*]
...
marked the input in *bold* so that it's clear what the user enters. In this program, the names, the tuition, the
...
9.8, and so on, it ought to be declared as a `double` or a
`float`.
...
[[program:CompoundInterestCalculator, CompoundInterestCalculator]]
----
...
[source, java, linenums]
[[program:DragonAttributes, DragonAttributes]]
----
...
`System.out.format()` as described in Subsection <<subsection:Primitive_types>>, we print out the yearly cost, four year cost, monthly loan
...
[source, java, linenums]
[[program:SplitExpression, SplitExpression]]
----
...
+
[source, java, linenums]
[[program:UninitializedString, UninitializedString]]
----
