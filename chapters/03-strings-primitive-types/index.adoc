...
image::typesInJavaFigure.svg[opts=inline,width=100%]
...
Section REF=syntax:Data representation.
...
image::differentObjectsFigure.svg[opts=inline,width=100%]
...
image::splitExpressionFigure.svg[opts=inline,width=100%]
...
`int` and `long`?
2.  <<exercise:Downcast>> In Example REF=example:Downcast error, the sum
