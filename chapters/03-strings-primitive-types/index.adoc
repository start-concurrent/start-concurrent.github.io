...
`Double.MIN\_VALUE - 1` evaluates to `-1.0`.
...
expression `2*Double.MAX\_VALUE` results in `Double.POSITIVE_INFINITY`
to indicate that the maximum representable value has been exceeded.
However, the expression `Double.MAX\_VALUE + 1` results in
`Double.MAX\_VALUE`. Why doesn’t the second case yield
