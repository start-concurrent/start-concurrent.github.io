[[chapter:Primitive_Types_and_Strings]]
...
will talk about in Section <<syntax:Types_in_Java>>, but you cannot
...
As shown in Figure <<typesInJavaFigure>>(a), there are two kinds of
...
operators in great detail in Section <<syntax:Types_in_Java>>.
...
object has and what operations it can perform.
...
==== Begin Pitfall: Multiple declarations
...
from each type are given in Table <<rangePrimitiveTypes>>.

.Ranges for primitive integer types in Java.
[%autowidth,cols="<l,^,>,^,<",options="header",]
|=======================================================================
|*Type* |*Bytes* | |*Range* |
|byte|1 |-128 |to |127
...
which we will discuss in Chapter <<chapter:Arrays>>. However, too narrow
...
==== Begin Example: Integer variables
...
of commas to write three million as `3_000_000`. To use this syntax,
...
precision discussed in Chapter <<chapter:Computer_Basics>>, Java cannot
...
==== Begin Example: Floating point declarations
...
As discussed in Chapter <<chapter:Computer_Basics>>, integer types
...
==== Begin Example: Floating point accuracy
...
==== Begin Pitfall: Unexpected integer division
...
[%autowidth,cols="^.^l,^.^l,^.^l",options="header",]
|===================================
...
==== Begin Pitfall: Weak type checking with assignment shortcuts
...
==== Begin Pitfall: Increment confusion
...
Chapter <<chapter:Computer_Basics>>. These operators perform bitwise
...
Section <<syntax:Data_representation>>.

==== Begin Example: Binary operators in Java
...
==== Begin Example: Upcast with integers
...
==== Begin Example: Upcast from integers to floating point
...
==== Begin Example: Downcast from double to int
...
==== Begin Example: Conversion from double to float
...
storage. From Chapter <<chapter:Computer_Basics>>, you know that you can
...
==== Begin Example: Printing single characters
...
Chapters <<chapter:Selection>>, <<chapter:Repetition>>, and beyond.
...
[%autowidth,cols="^l,<,<l",options="header",]
|============================================
...
==== Begin Example: Boolean variables
...
[.text-center]
.Because they are primitive, `int` variables store values, not references.
...
==== Begin Example: String assignment
...
As you have seen in Chapter <<chapter:Problem_Solving_and_Programming>>,
...
In Chapter <<chapter:Problem_Solving_and_Programming>>, you saw that we
...
==== Begin Example: String concatenation
...
it further in Chapter <<chapter:Arrays>>. (It may help if you think of
...
==== Begin Example: Examining the char value at an index
...
Figure <<differentObjectsFigure>>(a), which shows three different
...
As shown in Figure <<differentObjectsFigure>>(b), this assignment points
...
Table <<mathFunctionsTable>> lists a few of the methods available. For a
...
|exp()|double population = 250 * Math.exp(0.03 * time);|Compute latexmath:[e^x], where latexmath:[x] is the
argument.
|log()|double digits = Math.log(1000000);|Compute the natural logarithm of the argument.
|pow()|double money = principal * Math.pow(1.0 + rate, time);|Compute latexmath:[a^b], where latexmath:[a] and latexmath:[b]
are the first and second arguments.
3+^.^d|Miscellaneous
|random()|double percent = Math.random();|Generate a random number latexmath:[x] where
...
==== Begin Example: Math library usage
...
.Program to compute interest earned and new balance.
[source, java, linenums]
----
include::programs/CompoundInterestCalculator.java[]
----
...
==== Begin Example: Dragon attribute generation

Suppose you are designing a video in which the hero must fight a dragon
with random attributes. Program <<program:DragonAttributes>> generates
random values for the age, height, gender, and hit points of the dragon.

.Program to set attributes of a randomly generated dragon for a video game.
[source, java, linenums]
----
include::programs/DragonAttributes.java[]
----

Note that we begin by importing `java.util.*` to include all the classes
in the `java.util` package, including `Random`. At
line <<createRandomNumberGeneratorLine>>, we create an object `random`
of type `Random`. At line <<generateAgeLine>>, we use it to generate a
...
Program <<program:DragonAttributes>> changes every time we run the
...
Program <<program:GetInputGUI>>, where we did the conversion as follows.
...
==== Begin Example: String to numeric conversion
...
how to work with exceptions in Chapter <<chapter:Exceptions>>.
...
As you recall from Chapter <<chapter:Computer_Basics>>, integer
...
in Chapter <<chapter:Dynamic_Data_Structures>>, where they are used to
...
[source, java, linenums]
----
include::programs/CollegeCosts.java[lines=1..7]
----
...
[source, java, linenums]
----
include::programs/CollegeCosts.java[lines=9..22]
----
...
[source, java, linenums]
----
include::programs/CollegeCosts.java[lines=24..31]
----
...
[source, java, linenums]
----
include::programs/CollegeCosts.java[lines=33..46]
----


=== Concurrency: Expressions

In Section <<concurrency:Solving_problems_in_parallel>>, we introduced
...
==== Begin Example: Split expression
...
Figure <<splitExpressionFigure>>, we show two ways of evaluating the
expression `f(a,b)*g(c)`. Figure <<splitExpressionFigure>>(a) shows
...
Figure <<splitExpressionFigure>>(b) shows evaluation of the expression
...
Figure <<splitExpressionFigure>>(b). Program <<program:SplitExpression>>
illustrates this concurrent approach.

In Program <<program:SplitExpression>>, we create two objects named
`fThread` and `gThread` at lines <<createFLine>> and <<createGLine>>,
...
.Program for concurrent evaluation of an expression.
[source, java, linenums]
----
include::programs/SplitExpression.java[]
----

Once the thread objects have been created, we start the threads at
lines <<startFComputationLine>> and <<startGComputationLine>>. Every
object whose type is `Thread` (or a child of `Thread`, which we will
discuss in Chapter <<chapter:Inheritance>>) has a `start()` method which
...
Chapter <<chapter:Exceptions>>, but, for now, you only need to know that
...
Program <<program:SplitExpression>> resumes at
line <<getResultFromFLine>>, where we obtain the result of the
...
program). Finally, at line <<computeFinalValueLine>>, these two computed
...
<<chapter:Methods>>, <<chapter:Classes>>, and <<chapter:Inheritance>>.

If you don’t understand all the elements of
Program <<program:SplitExpression>>, don’t despair. We’re trying to give
...
Chapter <<chapter:Concurrent_Programming>>.
...
carefully, if it can be done at all. Chapter <<chapter:Synchronization>>
...
`int` and `long`?
2.  <<exercise:Downcast>> In Example <<example:Downcast_error>>, the sum
...
3.  <<exercise:floatFloatExampleExercise>> The following three
...
....
4.  <<exercise:logicalOperatorExercise>> Consider the following variable
declarations.
+
....
int x = 3, y = 4, z = -9;
...
5.  <<exercise:bitwiselOperatorExercise>> Evaluate the following
...
f.  `5 << 2`
g.  `5 >>> 2`
6.  <<exercise:overflowUnderflowExercise>> Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Byte.MIN_VALUE - 1`
b.  `Byte.MAX_VALUE + 1`
c.  `Integer.MIN_VALUE - 1`
7.  <<exercise:floatingPointOverflowExercise>> Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Float.MAX_VALUE + 1`
...
8.  <<exercise:overFlowDoubleExercise>> When evaluated in Java, the
...
9.  <<exercise:conversionExercise>> Explain what is printed when the
...
System.out.println("" + 15 + 20);
....
10. <<exercise:typeValueDeterminationExercise>> For each of the
...
j.  `(int)(15.5 * 4.0)`
k.  `Math.round(3.154)`
11. <<exercise:splitExpressionEvaluationExercise>> For each of the
following expressions, determine the maximum amount of concurrency that
can be achieved. Using a diagram similar to
Figure <<splitExpressionFigure>>(b), show how the computation of each
...
b.  `f(a * g(b))`
c.  `f(g(a)) + f(b) + f(c)`
12. <<exercise:referenceTypes>> Answer the following questions about
...
Java.
13. <<exercise:objectComparsonExercise>> Consider the following
...
14. <<exercise:unicode>> Characters `'a'` and `'A'` have Unicode values
`\u0061` and `\u0041`, respectively. Give the representation of these
two characters as 16-bit unsigned binary integers.
15. <<exercise:representation>> Assuming that each character occupies 16
...
many websites such as http://www.asciitable.com/.
16. <<exercise:stringLength>> What is the output from the following
...
....
17. <<exercise:emptyStringsExercise>> What is the output from the
...
18. <<exercise:uninitializedStringObjectExercise>> Try compiling the
following program and observe the error reported by the compiler.
+
[source, java, linenums]
----
include::programs/UninitializedString.java[]
----

+
Now initialize the `greeting` object and rerun the program. Why does the
program compile now?
19. <<exercise:stringToNumericConversionProgramExercise>> Write a Java
...
output you see?
20. Convert the college cost calculator solution given in
Section <<solution:College_cost_calculator>> to use +
