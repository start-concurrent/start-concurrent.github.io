...
will talk about in <<Syntax: Types in Java>>, but you cannot
...
<<Selection>>, <<Repetition>>, and beyond.
...
As you have seen in <<Problem Solving and Programming>>,
...
In <<Problem Solving and Programming>>, you saw that we
...
how to work with exceptions in <<Exceptions>>.
...
****
<<exercise3.7, Exercise 3.7>> +
<<exercise3.8, Exercise 3.8>>
****
...
1.  [[exercise3.1]] What is the difference
between the set of integers from mathematics and the sets defined by
`int` and `long`?
2.  [[exercise3.2]] In <<example:Downcast_error>>, the sum
...
3.  [[exercise3.3]] The following three
...
----
4.  [[exercise3.4]] Consider the following variable
declarations.
+
[source, java]
----
...
5.  [[exercise3.5]] Evaluate the following
...
f.  `5 << 2`
g.  `5 >>> 2`
6.  [[exercise3.6]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Byte.MIN_VALUE - 1`
b.  `Byte.MAX_VALUE + 1`
c.  `Integer.MIN_VALUE - 1`
7.  [[exercise3.7]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Float.MAX_VALUE + 1`
...
8.  [[exercise3.8]] When evaluated in Java, the
...
9.  [[exercise3.9]] Explain what is printed when the
...
System.out.println("" + 15 + 20);
----
10. [[exercise3.10]] For each of the
...
j.  `(int)(15.5 * 4.0)`
k.  `Math.round(3.154)`
11. [[exercise3.11]] For each of the
...
b.  `f(a * g(b))`
c.  `f(g(a)) + f(b) + f(c)`
12. [[exercise3.12]] Answer the following questions about
...
Java.
13. [[exercise3.13]] Consider the following
...
14. [[exercise3.14]] Characters `'a'` and `'A'` have Unicode values
`\u0061` and `\u0041`, respectively. Give the representation of these
two characters as 16-bit unsigned binary integers.
15. [[exercise3.15]] Assuming that each character occupies 16
...
16. [[exercise3.16]] What is the output from the following
...
----
17. [[exercise3.17]] What is the output from the
...
18. [[exercise3.18]] Try compiling the
