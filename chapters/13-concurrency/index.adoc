...
Consider dividing the tasks in Example REF=example:Video game tasks into
...
Figure REF=figure:tasks in threads(a).

Tasks to evaluate different subexpressions in Example REF=example:Math
expression tasks can also be divided into two threads as shown in
Figure REF=figure:tasks in threads(b). In many problems there are
...
Examples REF=example:Array summation preview and REF=example:Matrix
multiplication preview use multiple identical tasks. But these tasks
operate on different data. Nevertheless, in Example REF=example:Array
...
Example REF=example:Matrix multiplication preview, the two tasks can
...
image::task-thread-packaging.svg[opts=inline,scaledwidth=100%,pdfwidth=100%,width=100%]
...
math expression problem in Example REF=example:Math expression threads.
...
finish, we can use the threads defined in Example REF=example:Thread
...
Recall the concurrent program in Example REF=example:Math expression
threads to evaluate a simple mathematical expression. This program uses
two threads. We executed this multi-threaded program on an iMac computer
with an Intel Core 2 Duo running at 2.16 Ghz. The execution time was
measured at 1,660,000 nanoseconds. We also wrote a simple sequential
program to evaluate the same expression. It took 4,100 nanoseconds to
execute this program on the same computer. Plugging in these values for
latexmath:[t_c] and latexmath:[t_s], we can find the speedup.
...
In Example REF=example:Array summation preview, we introduced the
...
In Example REF=example:Matrix multiplication preview, we discussed the
...
Exercise REF=exercise:NumberedThread shows that the pattern of thread
...
image::thread-states.svg[opts=inline,scaledwidth=100%,pdfwidth=100%,width=100%]
...
1.  <<exercise:thread methods>> The `start()`, `run()`, and `join()`
methods are essential parts of the process of using threads in Java.
Explain the purpose of each method.
2.  <<exercise:Thread vs. Runnable>> What is the difference between
extending the `Thread` class and implementing the `Runnable` interface?
When should you use one over the other?
3.  <<exercise:sleep and yield>> How do the `Thread.sleep()` method and
the `Thread.yield()` method each affect thread scheduling?
4.  <<exercise:math expression>> Consider the expression in
Example REF=example:Math expression tasks. Suppose that the multiply and
...
quadcore processor. Can the tasks in Examples REF=example:Video game
tasks and REF=example:Math expression tasks be further subdivided to
improve performance on four cores? Why or why not?
6.  <<exercise:speedup>> Consider the definition of speedup from
Section REF=examples:Concurrency and speedup. Let’s assume you have a
...
7.  <<exercise:speedup bounds>> In which situations can speedup be
...
9.  <<exercise:task parallelization>> Consider the following table of
tasks:
+
+
[cols="<,^,^,<",options="header",]
...
Example REF=example:Array summation using polling instead of `join()`
...
with the technique.
12. <<exercise:echo effect>> Composers often work with multiple tracks
...
13. <<exercise:sleep timer>> Write a program which takes a number of
...
14. <<exercise:concurrent approximation>> As you know,
latexmath:[\pi\approx
...
16. <<exercise:thread overhead>> Thread creation overhead is an
...
17. <<exercise:matrix multiply>> Create serial and concurrent
implementations of matrix multiplication like those described in
Example REF=example:Matrix multiplication.
...
Example REF=example:Array of threads which creates several
...
the array summing problem solved in Example REF=example:Array summation.
...
Example REF=example:Array summation seems to use concurrency
...
image::treesummation.svg[opts=inline,scaledwidth=100%,pdfwidth=100%,width=100%]
