...
that problem will be addressed in <<chapter:Synchronization>>.
...
Consider dividing the tasks in <<example:Video_game_tasks>> into
...
Tasks to evaluate different subexpressions in <<example:Math
...
operate on different data. Nevertheless, in <<example:Array
...
<<example:Matrix_multiplication_preview>>, the two tasks can
...
math expression problem in <<example:Math_expression_threads>>.
...
finish, we can use the threads defined in <<example:Thread
...
Recall the concurrent program in <<example:Math expression
...
In <<example:Array_summation_preview>>, we introduced the
...
In <<example:Matrix_multiplication_preview>>, we discussed the
...
<<syntax:Thread_states>>, and through the `wait()`, method which
we will discuss in <<chapter:Synchronization>>.
...
<<example:Math_expression_tasks>>. Suppose that the multiply and
...
<<examples:Concurrency_and_speedup>>. Let's assume you have a
...
<<example:Array_summation>> using polling instead of `join()`
...
<<example:Array_of_threads>> which creates several
...
the array summing problem solved in <<example:Array_summation>>.
...
<<example:Array_summation>> seems to use concurrency
