[[chapter:Concurrent_Programming]]
...
that problem will be addressed in Chapter <<chapter:Synchronization>>.
...
==== Begin Example: Video game tasks
...
Figure <<figure:video_game_tasks>>(a) and (b) show how the tasks in this
...
==== Begin Example: Math expression tasks
...
image::mathematical-expression-evaluation.png[]


Figure <<figure:math_evaluation>> shows how this expression can be
...
==== Begin Example: Array summation preview
...
This strategy is illustrated in Figure <<figure:array_decomposition>>.
...
image::arrayDecomposition.png[]


==== Begin Example: Matrix multiplication preview
...
.Data decomposition to multiply two latexmath:[4\times 4] matrices. The two cores perform the same multiplication tasks but on different data from matrix latexmath:[A]. The two cores compute the top and bottom two rows of latexmath:[C], respectively.
image::matrixDecomposition.png[]
...
task on each core. As shown in Figure <<figure:matrix_decomposition>>,
...
Consider dividing the tasks in Example <<example:Video_game_tasks>> into
...
Figure <<figure:tasks_in_threads>>(a).

Tasks to evaluate different subexpressions in Example <<example:Math
expression tasks>> can also be divided into two threads as shown in
Figure <<figure:tasks_in_threads>>(b). In many problems there are
...
Examples <<example:Array_summation_preview>> and <<example:Matrix
multiplication preview>> use multiple identical tasks. But these tasks
operate on different data. Nevertheless, in Example <<example:Array
...
Example <<example:Matrix_multiplication_preview>>, the two tasks can
again be assigned to two distinct threads that operate on different
parts of the input matrix latexmath:[A] to generate the corresponding
portions of the output matrix latexmath:[C].
...
image::task-thread-packaging.png[]
...
each evaluating a separate subexpression as in Figure <<figure:tasks in
threads>>(b).

==== Begin Example: Thread samples
...
subexpressions in Figure <<figure:tasks_in_threads>>(b) and save the
computed values.

[source, java, linenums]
----
include::programs/Thread1.java[]
----


[source, java, linenums]
----
include::programs/Thread2.java[]
----
...
math expression problem in Example <<example:Math_expression_threads>>.
...
==== Begin Example: Math expression threads

Now that we have the syntax to start threads and wait for them to
finish, we can use the threads defined in Example <<example:Thread
...
[source, java, linenums]
----
include::programs/MathExpression.java[]
----
...
concurrent Java programming. In Figure <<figure:tasks_in_threads>>, it
...
so would make the execution match Figure <<figure:tasks_in_threads>>
...
Figure <<figure:thread_execution>> shows the execution of `thread1` and
...
==== Begin Example: Array of threads
...
[source, java, linenums]
----
include::programs/NumberedThread.java[]
----
...
==== Begin Example: Measuring execution time
...
==== Begin Example: Math expression speedup

Recall the concurrent program in Example <<example:Math expression
...
these values for latexmath:[t_c] and latexmath:[t_s], we can find
the speedup.
...
==== Begin Example: Array summation

In Example <<example:Array_summation_preview>>, we introduced the
...
[source, java, linenums]
----
include::programs/SumThread.java[lines=1..14]
----
...
[source, java, linenums]
----
include::programs/SumThread.java[lines=16..21]
----
...
[source, java, linenums]
----
include::programs/SumThread.java[lines=23..39]
----
...
Section <<concurrency:Arrays>> which describes the fair division of work
...
[source, java, linenums]
----
include::programs/SumThread.java[lines=41..53]
----
...
In Example <<example:Matrix_multiplication_preview>>, we discussed the
...
two square matrices latexmath:[A] and latexmath:[B] and compute the
resultant matrix latexmath:[C]. If these matrices have latexmath:[n]
rows and latexmath:[n] columns, the value at the latexmath:[i]^th^
row and latexmath:[j]^th^ column of latexmath:[C] is
...
[source, java, linenums]
----
include::programs/MatrixThread.java[]
----
...
Section <<concurrency:Arrays>> to allocate rows to each thread fairly.
...
Section <<syntax:Thread_states>>, and through the `wait()`, method which
we will discuss in Chapter <<chapter:Synchronization>>.
...
Exercise <<exercise:NumberedThread>> shows that the pattern of thread
...
Figure <<figure:thread_states>> shows the lifecycle of a thread. A
...
==== Begin Example: Military marching
...
[source, java, linenums]
----
include::programs/LeftThread.java[]
----
...
[source, java, linenums]
----
include::programs/RightThread.java[]
----
...
[source, java, linenums]
----
include::programs/MilitaryMarching.java[]
----
...
[source, java, linenums]
----
include::programs/LeftYieldThread.java[]
----


[source, java, linenums]
----
include::programs/RightYieldThread.java[]
----
...
[source, java, linenums]
----
include::programs/LeftPollingThread.java[]
----


[source, java, linenums]
----
include::programs/RightPollingThread.java[]
----
...
[source, java, linenums]
----
include::programs/FactorThread.java[]
----
...
Section <<concurrency:Arrays>> to divide the work fairly. Because we
...
2.  <<exercise:Thread_vs._Runnable>> What is the difference between
extending the `Thread` class and implementing the `Runnable` interface?
When should you use one over the other?
3.  <<exercise:sleep_and_yield>> How do the `Thread.sleep()` method and
the `Thread.yield()` method each affect thread scheduling?
4.  <<exercise:math_expression>> Consider the expression in
Example <<example:Math_expression_tasks>>. Suppose that the multiply and
...
in Figure <<figure:math_evaluation>>(a) and (b).
5.  <<exercise:multicore-exercise>> Suppose that a computer has one
quadcore processor. Can the tasks in Examples <<example:Video game
tasks>> and <<example:Math_expression_tasks>> be further subdivided to
improve performance on four cores? Why or why not?
6.  <<exercise:speedup>> Consider the definition of speedup from
Section <<examples:Concurrency_and_speedup>>. Let’s assume you have a
...
thread does not need to communicate after it has been created.
7.  <<exercise:speedup_bounds>> In which situations can speedup be
smaller than the number of processors? Is it ever possible for speedup
to be greater than the number of processors?
8.  <<exercise:Amdahl>> Amdahl’s Law is a mathematical description of
...
parallel program is latexmath:[\frac{1}{1 - P}] where latexmath:[P]
...
8? What implications does Amdahl’s Law have?
9.  <<exercise:task_parallelization>> Consider the following table of
tasks:
+
[cols="<,^,^,<",options="header",]
|===========================================
...
10. <<exercise:interleavings>> Consider the following code snippet.
+
....
x = 13;
x = x * 10;
...
11. <<exercise:polling>> Re-implement the array summing problem from
Example <<example:Array_summation>> using polling instead of `join()`
...
with the technique.
12. <<exercise:echo_effect>> Composers often work with multiple tracks
...
13. <<exercise:sleep_timer>> Write a program which takes a number of
...
14. <<exercise:concurrent_approximation>> As you know,
latexmath:[\pi\approx
...
+
*Experiments*
15. <<exercise:time-measurement>> Use the `currentTimeMillis()` method
...
16. <<exercise:thread_overhead>> Thread creation overhead is an
...
17. <<exercise:matrix_multiply>> Create serial and concurrent
implementations of matrix multiplication like those described in
Example <<example:Matrix_multiplication>>.
...
18. <<exercise:NumberedThread>> Repeatedly run the code in
Example <<example:Array_of_threads>> which creates several
...
19. <<exercise:ArraySum>> Create serial and parallel implementations of
the array summing problem solved in Example <<example:Array_summation>>.
...
first?
20. <<exercise:TreeSum>> The solution to the array summing problem in
Example <<example:Array_summation>> seems to use concurrency
...
be a static field. Figure <<figure:tree_summation>> illustrates this
process.
+
