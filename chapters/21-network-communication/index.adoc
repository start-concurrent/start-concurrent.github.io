...
from <<chapter:Constructing_Graphical_User_Interfaces>>, you can
...
Even in <<example:Chat_client_and_server>>, it was convenient to
...
8.  Recall the client and server from <<subsection:receiving_and_sending_data>> that, respectively, send 100 `int` values and sum them.
...
program given in <<example:Chat_client_and_server>>. Use a
...
implementation from <<solution:Web_server>>. Implement a similar
