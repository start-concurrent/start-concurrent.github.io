...
Even in Example REF=example:Chat client and server, it was convenient to
...
8.  Recall the client and server from Section REF=subsection:receiving
and sending data that, respectively, send 100 `int` values and sum them.
Rewrite these fragments to send and receive the `int` values in text
rather than binary format.
9.  <<exercise:chat GUI>> Add a GUI based on `JFrame` for the chat
program given in Example REF=example:Chat client and server. Use a
...
10. <<exercise:multi-threaded web server>> Study the web server
implementation from Section REF=solution:Web server. Implement a similar
...
11. <<exercise:thread pool web server>> One of the weaknesses of the web
...
Exercise REF=exercise:multi-threaded web server. Can you design an
