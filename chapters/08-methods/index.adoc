...
solution to Conway’s Game of Life from <<chapter:Arrays>>. If we
...
<<chapter:Classes>>. Unlike the static methods we have discussed
...
<<subsection:class_variables>>. This problem becomes worse with
...
problem. In <<chapter:Synchronization>> we talk about the much
...
12. Re-implement the solution from <<solution:Three_card_poker>>
...
three card version we discussed in <<problem:Three_card_poker>>.
