...
solution to Conway's Game of Life from <<Arrays>>. If we
...
.A program that keeps track of the number of times the `record()` method is called.
[source, java, linenums]
[[program:Bookkeeper, Bookkeeper]]
----
include::programs/Bookkeeper.java[]
----
...
.A program that calculates the attraction due to gravity between two masses.
[source, java, linenums]
[[program:Gravity, Gravity]]
----
include::programs/Gravity.java[]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=1..11]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=13..29]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=31..39]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=41..48]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=50..63]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=65..77]
----
...
[source, java, linenums]
----
include::programs/ThreeCardPoker.java[lines=79..100]
----
...
<<Classes>>. Unlike the static methods we have discussed
...
.This program implements an LCG similar to one sometimes used in the function `rand()` used in the C language.
[source, java, linenums]
[[program:UnsafeRandom, UnsafeRandom]]
----
include::programs/UnsafeRandom.java[]
----
...
.This program implements the same LCG safely by requiring the caller to supply the previous random number.
[source, java, linenums]
[[program:SafeRandom, SafeRandom]]
----
include::programs/SafeRandom.java[]
----
...
problem. In <<Synchronization>> we talk about the much
...
[source, java, linenums]
[[program:AlwaysEven, AlwaysEven]]
----
include::programs/AlwaysEven.java[]
----
...
1. [[exercise8.1]] Describe three advantages of dividing long segments of code into
static methods.
2. [[exercise8.2]] Can you think of any disadvantages of dividing code into methods?
Are there situations where using a method is unwise?
3. [[exercise8.3]] If you wanted to declare a static method that would compute the
mean, median, and standard deviation of an input array of `double`
values, how would you return those three answers?
4. [[exercise8.4]] Consider the following method definition.
+
[source,java]
----
...
while( x < 128 )
    twice(x);
----
5. [[exercise8.5]] Consider the following signatures of two overloaded methods.
+
[source,java]
----
...
6. [[exercise8.6]] The following class generates a sequence of even numbers. Each time
...
7. [[exercise8.7]] Write a static method called `cube()` that takes a single `double`
value as a parameter and returns its value cubed. Do not use the
`Math.pow()` method.
8. [[exercise8.8]] Implement a static method that takes a single `int` value as a
...
9.  [[exercise8.9]] Write a static method that takes an array of `int` values as a
...
10. [[exercise8.10]] Write a static method that finds the
...
|=============================
|latexmath:[n] |Return Value |latexmath:[n] |Return Value
|1 |0 |16 |4
|2 |1 |100 |6
|4 |2 |512 |9
|8 |3 |1000 |9
|10 |3 |1024 |10
|=============================
11. [[exercise8.11]] Write a method that tests palindromes like
the method from <<example:Palindrome_testing>> but also ignores
punctuation and spaces. Thus, `"A man, a plan, a canal: Panama"` should
be counted as a palindrome by this new method.
12. [[exercise8.12]] Re-implement the solution from <<Solution: Three card poker>>
so that it uses a GUI constructed with `JOptionPane` to display the hand
and the winnings.
13. [[exercise8.13]] Five card poker is a much more common version of poker than the
...
a.  All five cards belong to the same suit and have ranks in sequential
...
it is a tie.
b.  Four of the five cards have the same rank. If two people have four
of a kind, the higher rank set of four wins.
c.  Three of the five cards have the same rank and the other two share
another rank. If two people have a full house, the higher ranked set of
three wins.
d.  All five cards have the same suit. If two people have flushes, the
...
e.  All five cards have ranks in sequential order (with either ace high
or low). If two people both have straight flushes, the higher ranked one
wins. If they both have the same ranks, it is a tie.
f.  Three of the cards have the same rank. If two people have three of a
kind, the higher ranked set of three wins.
g.  A pair of cards have the same rank and another pair of cards share
...
it is a tie.
h.  A pair of cards has the same rank. If two people have pairs, the
...
i.  If none of the other cases hold, the high card determines the value
...
+
*Experiments*
14. [[exercise8.14]] In terms of time, there is a small overhead associated with calling
