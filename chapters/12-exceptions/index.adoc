...
<<chapter:Arrays>>, we used the `Thread.sleep()` method to put
...
In <<chapter:Arrays>>, there were no executable statements in
...
demonstrate in <<chapter:File_I/O>>
...
Exercise <<exercise:celestial>> from <<chapter:Inheritance>>.
