...
1.  <<exercise:exceptions vs. error codes>> What are the advantages of
using exceptions instead of returning error codes?
2.  <<exercise:final and finally>> The keywords `final` and `finally`,
as well as the `Object` method `finalize()`, are sometimes confused.
What is the purpose of each one?
3.  <<exercise:throws keyword>> What is the difference between the
`throw` keyword and the `throws` keyword?
4.  <<exercise:catch or specify>> Explain the catch or specify
requirement of Java.
5.  <<exercise:checked vs. unchecked>> What must be done differently
...
6.  <<exercise:whole program in try>> For every program you write, you
...
8.  <<exercise:checked example>> Consider the following two classes.
+
....
...
9.  <<exercise:finally returns>> What value will the following method
...
....
10. <<exercise:specific exceptions first>> Why will the following
...
....
11. <<exercise:multiple exceptions>> Consider the following fragment of
Java.
+
....
try {
...
12. The <<exercise:calculator exceptions>> `NumberFormatException`
...
13. <<exercise:celestial exceptions>> Refer to
Exercise REF=exercise:celestial from Chapter REF=chapter:Inheritance.
...
14. <<exercise:password checker>> Users often log onto systems by
...
+
*Experiments*
15. Throwing <<exercise:exception performance>> and catching exceptions
