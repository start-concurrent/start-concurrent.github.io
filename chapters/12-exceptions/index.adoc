...
Chapter <<chapter:Arrays>>, we used the `Thread.sleep()` method to put
...
In Chapter <<chapter:Arrays>>, there were no executable statements in
...
demonstrate in Chapter <<chapter:File_I/O>>
...
2.  <<exercise:final_and_finally>> The keywords `final` and `finally`,
as well as the `Object` method `finalize()`, are sometimes confused.
What is the purpose of each one?
3.  <<exercise:throws_keyword>> What is the difference between the
`throw` keyword and the `throws` keyword?
4.  <<exercise:catch_or_specify>> Explain the catch or specify
requirement of Java.
5.  <<exercise:checked_vs._unchecked>> What must be done differently
...
6.  <<exercise:whole_program_in_try>> For every program you write, you
...
7.  Why did the designers of Java choose to make `NullPointerException`
and +
...
8.  <<exercise:checked_example>> Consider the following two classes.
+
[source,java]
----
...
9.  <<exercise:finally_returns>> What value will the following method
...
----
10. <<exercise:specific_exceptions_first>> Why will the following
...
----
11. <<exercise:multiple_exceptions>> Consider the following fragment of
Java.
+
[source,java]
----
...
12. The <<exercise:calculator_exceptions>> `NumberFormatException`
exception is thrown whenever the +
...
13. <<exercise:celestial_exceptions>> Refer to
Exercise <<exercise:celestial>> from Chapter <<chapter:Inheritance>>.
...
14. <<exercise:password_checker>> Users often log onto systems by
...
+
*Experiments*
15. Throwing <<exercise:exception_performance>> and catching exceptions
