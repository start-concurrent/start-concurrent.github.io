...
image::flowchart.svg[opts=inline,width=100%]
...
Example REF=example:Leap year uses three `if` statements and three
...
11. Re-implement both parts from Exercise REF=exercise:coins using
...
Example REF=example:Ordinal numbers to give the correct suffixes (always
...
13. At the bottom of Section REF=subsection:switch statements, we use a
