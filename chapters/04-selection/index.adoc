...
<<chapter:Methods>> and <<chapter:Classes>>. What we're
...
<<chapter:Primitive_Types_and_Strings>>, but
...
in great detail in <<chapter:Synchronization>>. Remember, one of
...
1. [[exercise4.1]] Given that latexmath:[x], latexmath:[y], and latexmath:[z] are
propositions in Boolean logic, make a truth table for the expression
latexmath:[(\lnot(x \land \lnot y)
\oplus \lnot z)].
2. [[exercise4.2]] What is the value of the Boolean expression latexmath:[\lnot((T
\oplus F) \land \lnot(F \lor T))]?
3. [[exercise4.3]] The calculation to determine the leap year given in
<<example:Leap_year>> uses three `if` statements and three
...
and `&&`.
4.  [[exercise4.4]] The XOR operator (`^`) is useful for combining `boolean` values, but
it can be replaced with a more commonly used *relational* operator in
Java. Which one?
5. [[exercise4.5]] De Morgan's laws are the following, which show that the process of
...
6. [[exercise4.6]] Use De Morgan's laws given above to rewrite the following statement
...
----
7. [[exercise4.7]] Consider the following fragment of code.
+
[source,java]
----
int x = 5;
...
`y > 10 & (x = 10) > 5`? Why?
8. [[exercise4.8]] Consider the following fragment of code.
+
[source,java]
----
int a = 7;
...
9. [[exercise4.9]] This problem has two parts.
...
10. [[exercise4.10]] Write programs that:
...
11. [[exercise4.11]] Re-implement both parts from <<exercise4.10, Exercise 4.10>> using `switch` statements instead of `if` statements.
12. [[exercise4.12]] Expand the program given in
<<example:Ordinal_numbers>> to give the correct suffixes (always
...
13. [[exercise4.14]] At the bottom of <<subsection:switch_statements>>, we use a
...
statements instead.
14. [[exercise4.14]] Every member of your secret club has an ID number. These ID numbers
...
correctly.
15. [[exercise4.15]] According to the North American Numbering Plan (NANP) used by the
...
large. (23 or more!)
16. [[exercise4.16]] Re-implement the solution to the Monty Hall program given in
<<solution:Monty_Hall>> using `JOptionPane` to generate GUIs for
input and output.
