...
from Chapter <<chapter:Primitive_Types_and_Strings>>. With an object of
...
Chapters <<chapter:Methods>> and <<chapter:Classes>>. What we’re
...
Figure <<figure:if>> shows this pattern of conditional execution
...
Figure <<figure:else>> shows the pattern of conditional execution
...
==== Begin Pitfall: Misleading indentation
...
Chapter <<chapter:Primitive_Types_and_Strings>>, but
Table <<table:relational_operators>> below shows them again in a
...
==== Begin Pitfall: Assignment instead of equality
...
Figure <<figure:flowchart>>.

.Program to navigate the possible choices in the decision tree.
[source, java, linenums]
----
...
==== Begin Example: Days in the month
...
.This program computes the number of days in a given month.
[source, java, linenums]
----
...
.This program appends the appropriate suffix to a numeral to make it an ordinal.
[source, java, linenums]
----
...
.This program determines a Chinese zodiac animal based on birth year.
[source, java, linenums]
----
...
.This program determines Western zodiac signs based on birth month and day.
[source, java, linenums]
----
...
in great detail in Chapter <<chapter:Synchronization>>. Remember, one of
...
\oplus \lnot z)].
2.  What is the value of the Boolean expression latexmath:[\lnot((T
\oplus F) \land \lnot(F \lor T))]?
3.  The calculation to determine the leap year given in
Example <<example:Leap_year>> uses three `if` statements and three
...
and `\&\&`.
4.  The XOR operator (`^`) is useful for combining `boolean` values, but
it can be replaced with a more commonly used *relational* operator in
Java. Which one?
5.  De Morgan’s laws are the following, which show that the process of
...
6.  Use De Morgan’s laws given above to rewrite the following statement
...
9.  a.  Write a program that reads in two `double` values and prints the
...
10. <<exercise:coins>> Write programs that:
...
11. Re-implement both parts from Exercise <<exercise:coins>> using
`switch` statements instead of `if` statements. Note: You cannot use
`switch` statements for (b) unless you are using Java 7 or later.
12. <<exercise:ordinals>> Expand the program given in
Example <<example:Ordinal_numbers>> to give the correct suffixes (always
...
13. At the bottom of Section <<subsection:switch_statements>>, we use a
...
statements instead.
14. Every member of your secret club has an ID number. These ID numbers
...
correctly.
15. According to the North American Numbering Plan (NANP) used by the
...
large. (23 or more!)
16. Re-implement the solution to the Monty Hall program given in
Section <<solution:Monty_Hall>> using `JOptionPane` to generate GUIs for
input and output.
