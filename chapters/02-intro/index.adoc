[[chapter:Problem_Solving_and_Programming]]
...
implementing them in Java, but Chapter <<chapter:Testing_and_Debugging>>
...
==== Begin Example: Rising costs of fixing an error
...
==== Begin Example: Ball bouncing problem and plan
...
latexmath:[k^{\mathrm{th}}] time. See Figure <<bouncingBallFigure>>
...
+
.A ball is dropped from height latexmath:[h]. The ball rises to height latexmath:[h c] after the first bounce and to latexmath:[hc^2] after the second.
...
generates an output. Figure <<subProblemRelationFigure>> shows how these
...
image::subProblemRelation.png[]


Before we can continue on to Step 3, we need to learn some Java.
Section <<syntax:Java_basics>> introduces you to the Java you’ll need to
solve this problem.
...
Chapter <<chapter:Testing_and_Debugging>>, in which we briefly discuss
...
depth in Chapter <<chapter:Methods>>.
...
==== Begin Example: Command line input

Subproblem 1 requires us to get the height, coefficient of restitution,
and number of bounces from the user. Program <<program:GetInputCLI>>
...
==== Begin Example: Input for distance computation
...
Program <<program:Distance>> solves each of these subproblems in order,
using the command-line input and output tools we have just discussed.

.Program to compute the distance a moving object travels.
[source, java, linenums]
----
include::programs/Distance.java[]
----
...
Starting at line <<solutionSubProblemOneScanner>>, we solve subproblem
...
At line <<solutionSubProblemTwoScanner>>, we compute the distance
...
Chapter <<chapter:Simple_Graphical_User_Interfaces>>. Then, we go into
GUIs in much more depth in Chapter <<chapter:Constructing Graphical User
Interfaces>>.
...
exactly like the previous program. Figure <<figure:showMessageDialog>>
...
`showMessageDialog()`. Figure <<figure:showInputDialog>> shows the
...
change. We’ll discuss types more deeply in Chapter <<chapter:Primitive
Types and Strings>>.
...
==== Begin Example: GUI input

We can change the solution given in Program <<program:GetInputCLI>> to
use the GUI-based input tools in `JOptionPane`.
Program <<program:GetInputCLI>> this equivalent GUI-based Java program.

.Program to get the height, coefficient of restitution, and number of bounces using a GUI.
[source, java, linenums]
----
include::programs/GetInputGUI.java[]
----

The code in this program is very similar to
Program <<program:GetInputCLI>> until line <<inputDialogLineOne>>. At
...
Chapter <<chapter:Primitive_Types_and_Strings>>. Here are examples of
...
.Program to compute height of a ball after bounces.
[source, java, linenums]
----
include::programs/ComputeHeight.java[]
----

Program <<program:ComputeHeight>> is only focusing on subproblem 2, but,
...
.Program to display height of a ball using the command line.
[source, java, linenums]
----
include::programs/DisplayHeightCLI.java[]
----

Program <<program:DisplayHeightCLI>> is only focusing on subproblem 3,
...
.Program to display height of a ball using a GUI.
[source, java, linenums]
----
include::programs/DisplayHeightGUI.java[]
----
...
The problem solving steps given in Section <<concepts:Developing
...
In Example <<example:Ball_bouncing_problem_and_plan>> we made sure that
...
In Program <<program:GetInputCLI>>, we implemented subproblem 1, reading
the input from the command line. In Program <<program:ComputeHeight>>,
we implemented subproblem 2, computing the height of the final bounce.
In Program <<program:DisplayHeightCLI>>, we implemented subproblem 3,
...
[source, java, linenums]
----
include::programs/BouncingBallCLI.java[lines=1..28]
----
...
[source, java, linenums]
----
include::programs/BouncingBallCLI.java[lines=30..31]
----
...
[source, java, linenums]
----
include::programs/BouncingBallCLI.java[lines=33..39]
----
...
Programs <<program:GetInputCLI>>, <<program:ComputeHeight>>, and
<<program:DisplayHeightGUI>>. The final program is below. It only
...
[source, java, linenums]
----
include::programs/BouncingBallGUI.java[]
----
...
further in Chapter <<chapter:Testing_and_Debugging>>.
...
Figure <<documentTranslationFigure>>(a), the Lellaraps might attack
before you finish.

In order to finish the work faster, you hire a second translator whose
skills in Lellarap are as good as yours. As shown in
Figure <<documentTranslationFigure>>(b), you divide the document into
...
.(a) Translation by one translator. Time latexmath:[t_s] gives the sequential time taken. (b) Translation by translators A and B. Times latexmath:[t_1], latexmath:[t_2], latexmath:[t_3], and latexmath:[t_4] give the times needed to do each component of the concurrent approach.
image::documentTranslationFigure.png[]
...
Figure <<documentTranslationFigure>>(b), it would be a _concurrent
...
different tasks in Figure <<documentTranslationFigure>> with their
...
indicated in Figure <<documentTranslationFigure>>(b).
Exercise <<exercise:translationTimingExercise>> asks you to calculate
these times for the sequential and concurrent approaches.
...
==== Begin Example: Task decomposition

Let’s expand the problem given in Example <<example:Domain
decomposition>>. R^3^ robots can do more than just measure area. In
...
2.  <<exercise:doubleTypeExercise>> In Example <<example:Input for
...
3.  What is the purpose of the statement at
line <<createScannerGetInputDataObject>> in
Program <<program:GetInputCLI>>?
4.  Explain the difference between a declaration and an assignment
statement.
5.  <<exercise:statementCategorizationExercise>> Is the following
statement from line <<sampleDeclarationAndExecutableStatement>> of
Program <<program:BouncingBallGUI>> a declaration, an assignment, or a
...
String response = JOptionPane.showInputDialog(
    ...
    JOptionPane.QUESTION_MESSAGE);
....
6.  <<exercise:textOutputGraphicalOutputComparison>> When would you
prefer using the `JOptionPane` class for output over
`System.out.print()`? When might you prefer `System.out.print()` to
using `JOptionPane`?
7.  <<exercise:keywordLabeling>> Review
Program <<program:BouncingBallGUI>> and identify all the Java keywords
used in it.
8.  <<exercise:importStatementExercise>> Try to recompile
Program <<program:BouncingBallGUI>> after removing the `import`
statement at the top. Read and explain the error message generated by
the compiler.
9.  <<exercise:concurrentParallelExercise>> Explain the difference
...
10. <<exercise:translationTimingExercise>> Refer to
Figure <<documentTranslationFigure>>. Suppose that you and your
...
11. <<exercise:homeAreaCalculationAnalogyExercise>> In
Example <<example:Domain_decomposition>>, what aspect of a multicore
system do the robots represent?
12. <<exercise:homeAreaCalculationExerciseDifferentHomeStructures>> In
Example <<example:Domain_decomposition>>, suppose that you have two R^3^
...
13. <<exercise:scannerPractice>> Write a program that prompts the user
...
`System.out.println()`.
14. <<exercise:scannerAverage>> Expand the program from
Exercise <<exercise:scannerPractice>> so that it finds the average of
...
15. Rewrite your solution to Exercise <<exercise:scannerAverage>> so
that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
16. <<exercise:getInputDataExercise>> Copy and paste
Program <<program:GetInputCLI>> into the Java IDE you prefer. Compile
...
17. Rewrite your solution to Exercise <<exercise:getInputDataExercise>>
so that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
18. <<exercise:distanceComputationExercise>> In Example <<example:Input
for distance computation>>, we assumed that the speed is given in miles
per hour and the time in hours. Change Program <<program:Distance>> to
...
19. <<exercise:stringInputExercise>> A program can use both a command
...
20. Use the complete software development cycle to write a program that
reads in the lengths of two legs of a right triangle and computes the
length of its hypotenuse.
a.  Make sure you understand the problem. How can you apply the
Pythagorean formula (latexmath:[a^2 + b^2 = c^2]) to solve it?
b.  Design a solution by listing the steps you will need to take to read
in the appropriate values, find the answer, and then output it.
c.  Implement the steps as a Java program.
d.  Test the solution with several known values. For example, a right
...
errors?
e.  Consider what other features your program should have. If your
