...
image::bouncingBall.svg[opts=inline,width=100%]
...
image::subProblemRelation.svg[opts=inline,width=100%]
...
In Example REF=example:Ball bouncing problem and plan we made sure that
...
image::documentTranslationFigure.svg[opts=inline,width=100%]
...
indicated in Figure REF=documentTranslationFigure(b).
Exercise REF=exercise:translationTimingExercise asks you to calculate
...
image::floorPlan.svg[opts=inline,width=100%]
...
Let’s expand the problem given in Example REF=example:Domain
...
2.  <<exercise:doubleTypeExercise>> In Example REF=example:Input for
distance computation, we declared all variables to be of type `double`.
How would the program behave differently if we had declared all the
...
statement from line REF=sampleDeclarationAndExecutableStatement of
...
Program REF=program:BouncingBallGUI and identify all the Java keywords
used in it.
8.  <<exercise:importStatementExercise>> Try to recompile
Program REF=program:BouncingBallGUI after removing the `import`
...
Figure REF=documentTranslationFigure. Suppose that you and your
...
Example REF=example:Domain decomposition, what aspect of a multicore
system do the robots represent?
12. <<exercise:homeAreaCalculationExerciseDifferentHomeStructures>> In
Example REF=example:Domain decomposition, suppose that you have two R^3^
...
Exercise REF=exercise:scannerPractice so that it finds the average of
...
15. Rewrite your solution to Exercise REF=exercise:scannerAverage so
that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
16. <<exercise:getInputDataExercise>> Copy and paste
Program REF=program:GetInputCLI into the Java IDE you prefer. Compile
...
17. Rewrite your solution to Exercise REF=exercise:getInputDataExercise
so that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
18. <<exercise:distanceComputationExercise>> In
Example REF=example:Input for distance computation, we assumed that the
speed is given in miles per hour and the time in hours. Change
Program REF=program:Distance to compute the distance traveled by a
moving object given its speed in miles per hour but time in seconds. You
will need to perform a conversion from seconds to hours before you can
find the distance.
