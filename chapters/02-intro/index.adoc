...
implementing them in Java, but <<Testing and Debugging>>
...
latexmath:[k^{\mathrm{th}}] time. See <<bouncingBallFigure>>
...
<<Testing and Debugging>>, in which we briefly discuss
...
<<Simple Graphical User Interfaces>>. Then, we go into
GUIs in much more depth in <<Constructing Graphical User Interfaces>>.
...
exactly like the previous program. <<showMessageDialogFigure>>
...
change. We'll discuss types more deeply in <<Primitive Types and Strings>>.
...
<<Primitive Types and Strings>>. Here are examples of
...
In <<example-Ball_bouncing_problem>> we made sure that
...
further in <<Testing and Debugging>>.
...
Let's expand the problem given in <<Domain decomposition>>. R^3^ robots can do more than just measure area. In
...
2.  [[exercise2.2]] In <<Input for distance computation>>, we declared all variables to be of type
...
11. [[exercise2.11]] In
<<Domain decomposition>>, what aspect of a multicore
system do the robots represent?
12. [[exercise2.12]] In
<<Domain decomposition>>, suppose that you have two R^3^
...
<<exercise2.13,Exercise 2.13>> so that it finds the average of
...
15. Rewrite your solution to <<exercise2.14,Exercise 2.14>> so
...
17. Rewrite your solution to <<exercise2.16,Exercise 2.16>>
so that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
18. [[exercise2.18]] In <<Input for distance computation>>, we assumed that the speed is given in miles
