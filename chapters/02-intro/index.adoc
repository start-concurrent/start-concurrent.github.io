...
implementing them in Java, but <<chapter:Testing_and_Debugging>>
...
<<syntax:Java_basics>> introduces you to the Java you'll need to
solve this problem.
...
<<chapter:Testing_and_Debugging>>, in which we briefly discuss
...
<<chapter:Simple_Graphical_User_Interfaces>>. Then, we go into
GUIs in much more depth in <<chapter:Constructing_Graphical_User_Interfaces>>.
...
change. We'll discuss types more deeply in <<chapter:Primitive
Types and Strings>>.
...
<<chapter:Primitive_Types_and_Strings>>. Here are examples of
...
The problem solving steps given in <<concepts:Developing
...
In <<example:Ball_bouncing_problem_and_plan>> we made sure that
...
further in <<chapter:Testing_and_Debugging>>.
...
Let's expand the problem given in <<example:Domain
...
2.  [[exercise2.2]] In <<example:Input for
...
11. [[exercise2.11]] In
<<example:Domain_decomposition>>, what aspect of a multicore
system do the robots represent?
12. [[exercise2.12]] In
<<example:Domain_decomposition>>, suppose that you have two R^3^
...
`System.out.print()`.
18. [[exercise2.18]] In <<example:Input
