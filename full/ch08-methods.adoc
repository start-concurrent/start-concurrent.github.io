...
. [[ex8-1]] Describe three advantages of dividing long segments of code into
static methods.
. [[ex8-2]] Can you think of any disadvantages of dividing code into methods?
Are there situations where using a method is unwise?
. [[ex8-3]] If you wanted to declare a static method that would compute the
mean, median, and standard deviation of an input array of `double`
values, how would you return those three answers?
. [[ex8-4]] Consider the following method definition.
+
[source,java]
----
...
while( x < 128 )
    twice(x);
----
. [[ex8-5]] Consider the following signatures of two overloaded methods.
+
[source,java]
----
...
. [[ex8-6]] The following class generates a sequence of even numbers. Each time
...
[start=7]
. [[ex8-7]] Write a static method called `cube()` that takes a single `double`
value as a parameter and returns its value cubed. Do not use the
`Math.pow()` method.
. [[ex8-8]] Implement a static method that takes a single `int` value as a
...
.  [[ex8-9]] Write a static method that takes an array of `int` values as a
...
. [[ex8-10]] Write a static method that finds the
...
|10 |3 |1024 |10
|=============================
. [[ex8-11]] Write a method that tests palindromes like
...
. [[ex8-12]] Re-implement the solution from <<Solution: Three card poker>>
so that it uses a GUI constructed with `JOptionPane` to display the hand
and the winnings.
. [[ex8-13]] Five card poker is a much more common version of poker than the
...
*Experiments*
[start=14]
. [[ex8-14]] In terms of time, there is a small overhead associated with calling
