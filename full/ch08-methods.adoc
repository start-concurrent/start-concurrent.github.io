...
Suprisingly, the tools we introduce in this chapter do not allow you to solve a
...
as `Math.sqrt()`. The expression stem:[f(x) = \sqrt{x}] _f_ ( _x_ ) = √[overline]#_x_#. defines
what stem:[f(x)] does in the same way that the code inside of
`Math.sqrt()` does. When you want to use stem:[f(x)] for a
specific value, you can set stem:[y = f(5)] in the same way that
...
In the statement stem:[f(5)], the value of stem:[x] is
replaced by stem:[5]. In the same way, some variable inside of
...
given by the equation stem:[F =
\frac{Gm_1m_2}{r^2}], where stem:[m_1] is the mass of one
object, stem:[m_2] is the mass of another, stem:[r] is the
distance between their centers, and stem:[G] is the gravitational
constant,
stem:[6.673 \times 10^{-11}\mbox{ N}\cdot\mbox{m}^2\cdot\mbox{kg}^{-2}].
...
Given two points in 3D space stem:[(x_1, y_1, z_1)] and
stem:[(x_2,
y_2, z_2)], we can compute the Euclidean distance between them with the
equation: stem:[d = \sqrt{(x_1 - x_2)^2 + (y_1 - y_2)^2 + (z_1
- z_2)^2}] _d_ = √[overline]#(_x_₁ - _x_₂)² + (_y_₁ - _y_₂)² + (_z_₁
- _z_₂)²# The following method applies this equation directly.
...
uniformly generated random number in the range stem:[[0,1)], the
final shuffled deck should be any one of the stem:[52!] possible
...
stem:[x_i=(ax_{i-1}+b)\mod m], deriving the next number from the
...
stem:[\lfloor\log_2(n)\rfloor] of an integer stem:[n].
Note that if stem:[\log_2 n = x], it is also true that
stem:[n = 2^x]. In other words, the stem:[\log_2] operator
tells you what power of stem:[2] a number is. One way to define
the stem:[\log_2 n] is the number of times you have to divide
stem:[n] by stem:[2] to get stem:[1]. Use this
...
various input values of stem:[n].
+
[.center%autowidth%header,cols="4*^"]
|=============================
|stem:[n] |Return Value |stem:[n] |Return Value
|1 |0 |16 |4
|2 |1 |100 |6
|4 |2 |512 |9
|8 |3 |1000 |9
...
. In terms of time, there is a small overhead associated with calling
