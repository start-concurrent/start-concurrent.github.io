...
----
. [[ex5-3]] How many different DNA sequences of length latexmath:[n] are
there?
. [[ex5-4]] There are three different errors in the following loop intended to
...
----
. [[ex5-5]]  Consider the following code containing nested `for` loops.
+
[source,java]
----
Scanner in = new Scanner(System.in);
...
[start=6]
.  [[ex5-6]] Write a program that converts base 10 numbers into base 3 numbers.
...
.  [[ex5-7]] The greatest common divisor (GCD) of two integers is the largest
...
.  [[ex5-8]] In the solution to the DNA searching
...
.  [[ex5-9]] Write a program that reads a number latexmath:[n] from a user and
...
. [[ex5-10]] Re-implement the solution to the DNA searching program given in
<<Solution: DNA searching>> using `JOptionPane` to generate GUIs
for input and output.

*Experiments*
[start=11]
. [[ex5-11]] Using a `for` loop, record the Monty Hall simulation so that you can
...
. [[ex5-12]] Write three nested `for` loops, each of which run 1,000 times.
...
. [[ex5-13]] In <<Common pitfalls>>, one of the common loop
...
an `int`?
. [[ex5-14]] In <<primalityTestingExample>>, we gave three programs to
...
`PrimalityTester2`?
. [[ex5-15]] In <<parallelismWithoutThreadsExample>>, we ran four programs at the same time to solve a
