...
the condition? The task is taking a step towards the door. The
...
we can do much better. Note that if a number stem:[n] is divisible by 2, then it's also divisible by
stem:[\frac{n}{2}]. So, if a number is *not* divisible by 2, it's not divisible by any
number larger than stem:[\frac{n}{2}]. If it's not divisible by 2 or 3, then it's not divisible by any number
larger than stem:[\frac{n}{3}]. If it's not divisible by 2 or 3
or 4, it's not divisible by any number larger than
stem:[\frac{n}{4}], and so on. Thus, we don't have to check all
the way up to stem:[n - 1]. If we're checking to see if
...
_triangular numbers_. The stem:[i]^th^ triangular number is the sum
...
but not including stem:[n] or alternately start at 1 and go up to and
...
subsequence it's looking for. Afterwards, it prints out the total number of
