...
To convert a number stem:[n] to binary, we first find the largest
power of 2 that is not larger than stem:[n]. Then, we begin a
repetitive process that stops when the power of 2 under consideration is
0. If 2 raised to the current power is bigger than stem:[n], we
print out a 0 because that power is too big for stem:[n].
Otherwise, we print out a 1, subtract 2 raised to that power from
stem:[n], and move on to the next smaller power of 2. This
process will print a 0 for every power of 2 that is not in
stem:[n] and a 1 for every one that is, giving exactly the
...
stem:[2] from stem:[2^0] up to stem:[2^9]. As you
...
latest letter found so far. To get the stem:[i]^th^ `char` from a
...
write some code to determine if a number stem:[n] is prime. To do
so, we can simply divide stem:[n] by all the numbers between 2
and stem:[n - 1]. If none of the numbers divide it evenly, it
...
stem:[n] is divisible by 2, then it is also divisible by
stem:[\frac{n}{2}]. So, if a number is *not* divisible by 2, it
is not divisible by any number larger than stem:[\frac{n}{2}]. If
it is not divisible by 2 or 3, then it is not divisible by any number
larger than stem:[\frac{n}{3}]. If it is not divisible by 2 or 3
or 4, it is not divisible by any number larger than
stem:[\frac{n}{4}], and so on. Thus, we do not have to check all
the way up to stem:[n - 1]. If we are checking to see if
stem:[n] is divisible by stem:[x] and learning that
stem:[n] is not divisible by anything larger than
stem:[\frac{n}{x}], the point where stem:[x = \frac{n}{x}]
is when stem:[x = \sqrt{n}]. Thus, we only need to search up to
stem:[\sqrt{n}], which will save even more time.
...
triangular numbers. The stem:[i]^th^ triangular number is the sum
of the first stem:[i] integers. They are called triangular
...
We can use nested loops to print out the first stem:[n]
triangular numbers, where stem:[n] is specified by the user.
...
stem:[i] to it, as stem:[i] increases.
...
If you want to iterate stem:[n] times, start at 0 and go up to
but not including stem:[n] or start at 1 and go up to and
including stem:[n]. To keep loop headers consistent, some
...
In <<ch13-concurrency#ch13-concurrency>>, we will introduce tools
...
which we will cover in in <<ch13-concurrency#ch13-concurrency>>. As
...
----
. How many different DNA sequences of length stem:[n] are
there?
...
.  Write a program that reads a number stem:[n] from a user and
then prints all possible DNA sequences of length stem:[n]. Be
