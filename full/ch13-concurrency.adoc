...
for parameters stem:[a] and stem:[K] at a given value of
stem:[t].
stem:[2\cdot K\cdot a\cdot t\cdot e^{-a\cdot t^2}] We can
divide the expression into two terms:
stem:[2\cdot K\cdot a\cdot t] and stem:[e^{-a\cdot t^2}].
...
Suppose that we want to apply function stem:[f()] to each element
of an array stem:[A] and sum the results. Mathematically, we want
to compute the following sum.

stem:[S=\sum_{i=1}^N f\left(a(i)\right)]

In this formula, stem:[a(i)] is the stem:[i]^th^ element
of array stem:[A]. We want to perform the task of applying
function stem:[f()] to each element in the array and summing the
...
task on half of the array. Let stem:[S_1] and stem:[S_2]
denote the sums computed by core 1 and core 2, respectively.

stem:[S_1 =\sum_{i=1}^{\lfloor \frac{N}{2}\rfloor } f\left(a(i)\right)\hspace{.75in}
S_2 =\sum_{i=\lfloor \frac{N}{2} \rfloor +1}^{N} f\left(a(i)\right)]

Assuming that stem:[N] is even, both cores process exactly the
same amount of data. For odd stem:[N], one of the cores processes
...
After stem:[S_1] and stem:[S_2] have been computed, one of
the cores can add these two numbers together to get stem:[S].
...
multiply two square matrices stem:[A] and stem:[B], which
are both stem:[n\times n] matrices. The product matrix
stem:[C] will also be stem:[n\times n]. A sequential
program will compute each element of matrix stem:[C] one at a
time. However, a concurrent program can compute more than one element of
stem:[C] simultaneously using multiple cores.

[[figure-matrix_decomposition]]
[.text-center]
.Data decomposition to multiply two stem:[4\times 4] matrices. The two cores perform the same multiplication tasks but on different data from matrix stem:[A]. The two cores compute the top and bottom two rows of stem:[C], respectively.
image::matrixDecomposition.svg[scaledwidth=75%,pdfwidth=75%,width=75%]


In this problem, the task is to multiply matrices stem:[A] and
stem:[B]. Through domain decomposition, we can replicate this
task on each core. As shown in <<figure-matrix_decomposition>>,
each core computes only a portion of stem:[C]. For example, if
stem:[A] and stem:[B] are stem:[4\times 4]
matrices, we can ask one core to compute the product of the first two
rows of stem:[A] with all four columns of stem:[B] to
generate the first two rows of stem:[C]. The second core computes
the remaining two rows of stem:[C]. Both cores can access
matrices stem:[A] and stem:[B].
...
<<arraySummationPreviewExample>> and <<matrixMultiplicationPreviewExample>> use multiple identical tasks. But these tasks
operate on different data. Nevertheless, in <<arraySummationPreviewExample>>, the two tasks can be assigned two threads that
...
parts of the input matrix stem:[A] to generate the corresponding
portions of the output matrix stem:[C].
...
finish, we can use the threads defined in <<threadSamplesExample>> with a main thread to make our first complete concurrent
...
the multiple cores. Let stem:[t_s] be the average time to execute
the sequential program and stem:[t_c] the average time to execute
the concurrent program. Sometimes we will use the notation
stem:[t_c^k] to refer to the execution time of a concurrent
program with stem:[k] threads. So that the comparison is
...
To determine speedup, we need to measure stem:[t_s] and
stem:[t_c]. Time in a Java program can easily be measured with
...
these values for stem:[t_c] and stem:[t_s], we can find
the speedup.

stem:[\mbox{speedup}=\frac{t_s}{t_c} = \frac{4,100}{1,660,000}= 0.00246] +
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=1..14]
----
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=16..21]
----
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=23..39]
----
...
[source, java]
----
include::{programsdir}/SumThread.java[lines=41..53]
----
...
two square matrices stem:[A] and stem:[B] and compute the
resultant matrix stem:[C]. If these matrices have stem:[n]
rows and stem:[n] columns, the value at the stem:[i]^th^
row and stem:[j]^th^ column of stem:[C] is

stem:[C_{ij} = \sum_{k = 1}^n A_{ik}B_{kj} = A_{i1}B_{1j} + A_{i2}B_{2j} + \ldots +
A_{in}B_{nj}]
...
corresponding to matrices stem:[A], stem:[B], and
stem:[C] as well as lower and upper bounds on the rows of
stem:[C] to compute. The body of the `run()` method is identical
...
was executed for matrices of different sizes (stem:[n \times n]).
...
|=======================================================================
|Size (stem:[n]) |stem:[t_s] (s) |stem:[t_c] (s)
|Speedup
|100 |0.013 |0.9 |0.014
...
Only with stem:[1000\times1000] matrices did we see improved
...
we will discuss in <<ch14-synchronization#ch14-synchronization>>.
...
.  [[extendingThreadExercise]] What is the difference between
...
.  [[quadcoreExercise]] Suppose that a computer has one
quadcore processor. Can the tasks in <<videoGameTasksExample>> and <<mathExpressionTasksExample>> be further subdivided to
...
parallel program is stem:[\frac{1}{1 - P}] where stem:[P]
...
that height stem:[y] of a circle centered at the origin at any
distance stem:[x] is given by stem:[y = \sqrt{r^2 - x^2}]
where stem:[r] is the radius of the circle.
+
...
the results after they all finish. By setting stem:[r = 2], you
need only sum one quadrant of a circle to get stem:[\pi]. You
...
..  Given a machine with stem:[k>1] cores, what is the maximum
...
stem:[k] threads working at the first stage,
stem:[\frac{k}{2}] working at the second stage,
stem:[\frac{k}{4}] working at the third, and so on, until a
