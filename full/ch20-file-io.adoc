...
1. [[ex20-1]] What is the difference between volatile and non-volatile memory?
Which is often associated with files and why?
2. [[ex20-1]] What is the difference between text and binary files? What are the
pros and cons of using each?
3. [[ex20-3]] Define compression ratio to be the size of the uncompressed data in
...
4. [[ex20-4]] What is serialization in Java? What do you have to do to serialize
an object?
5. [[ex20-5]] What kinds of objects cannot be serialized?
+
*Programming Practice*
6. [[ex20-6]] Implement the RLE bitmap compression program using only
`FileInputStream` and `FileOutputStream` for file input and output.
7.  [[ex20-7]] Re-implement the maze solving program from <<Solution: Maze of doom>> to ask the user for a file instead of reading from standard
input.
8. [[ex20-8]] An HTML file contains many tags such as `<p>`, which marks the
...
9.  [[ex20-9]] Write a program that prompts the user for an
...
10. [[ex20-10]] Expand the program from <<ex20-9, Exercise 20.9>> so that it
...
11. [[ex20-11]] Files can become corrupted when they are transferred over a network.
...
+
*Experiments*
12. [[ex20-12]] Write the RLE bitmap compression program in parallel so that a file
