...
example, `&amp;pi;` is the entity for the Greek letter stem:[\pi].
...
variable
..  Set stem:[sum = sum\mod 2^{32}]
..  Let stem:[r = (sum\mod 2^{16}) + (sum \div 2^{16})]
..  Let stem:[s = (r\mod 2^{16}) + (r \div 2^{16})]
..  The final checksum is stem:[s]
+
