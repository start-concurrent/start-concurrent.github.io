...
command-line I/O. When reading and writing text files, it's easy to verify
that what you wanted to write is written and the contents of the file are what
you read. Binary files, however, are more powerful. Data can often be stored
more compactly, as in the example with the integer 2,127,480,645. Even better,
Java provides facilities for easily dumping (and later retrieving) primitive
data types, objects, and even complex data structures to binary files.
...
[source, java]
----
ObjectInputStream in = new ObjectInputStream(new FileInputStream("baseball.bin"));
----
...
Using `ObjectInputStream` and `ObjectOutputStream` in this way isn't
too difficult, but it seems cumbersome. These file I/O objects provide methods
...
in `Serializable`. It's just a tag for a class which can be packed up
...
like a `Thread` or a `FileInputStream` object cannot be serialized. If you need to serialize
...
It's worth noting that `DataInputStream` and `DataOutputStream` objects
