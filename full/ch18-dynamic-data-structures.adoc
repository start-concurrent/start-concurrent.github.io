...
1. [[ex18-1]] Explain the difference between static data structures and dynamic
data structures.
2.  [[ex18-2]] In which situations is it better to use a dynamic array? In which
situations is it better to use a linked list? Explain why in each case.
3.  [[ex18-3]] On which line in
...
loop?
5.  [[ex18-5]] Explain why the `names` array in
<<UseDynamicArrayProgram>> is, on average, only three-quarters
full.
6. [[ex18-6]] Based on the stack implementation in
...
----
    ...
    stack.push("world");
----
7.  [[ex18-7]] Implement the methods `top()` and
`isEmpty()` for the dynamic array implementation of the stack in
<<DynamicArrayStackProgram>>.
8. [[ex18-8]] Based on queue implementation in
...
9.  [[ex18-9]] Implement a version of
...
10. [[ex18-10]] Consider <<LinkedListWithTailProgram>> which defines the
...
11. [[ex18-11]] In <<Linked lists>>, we use two kinds of linked
...
12. [[ex18-12]] Create JUnit test cases to
...
13. [[ex18-13]] To make an infix calculator that can handle floating point values or
...
14. [[ex18-14]] Re-implement the solution to the infix conversion problem given in
<<Solution: Infix conversion>> so that it uses `GenericStack`
with a type parameter of `Term` instead of `TermStack`.
15. [[ex18-15]] Interfaces can also be generic. Consider the following generic
version of `Queue`.
+
[source,java]
----
