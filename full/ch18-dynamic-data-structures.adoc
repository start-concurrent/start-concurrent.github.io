...
<<testUnsynchronizedDynamicArrayExercise>> explores the need to
...
is empty. Other more elaborate error handling is possible by throwing an exception.
...
interface can be used in the enhanced `for` loops described in
<<Enhanced `for` loops>>. The `ArrayDeque`, `ArrayList`,
`HashSet`, `TreeSet`, and `Vector`, classes all implement `Iterable`. In
our examples, a `Vector` object and a `Set` (returned by the
`entrySet()` method of a `HashMap`) are used as targest of enhanced `for`
loops.
...
----
<1> The program creates a queue and stores a reference
to it in a static (class) variable `queue`.
<2> It then creates and starts 10 threads.
<3> During the adding phase (indicated by `adding` being
`true`), each thread adds its thread ID number to the queue and prints
it to standard output.
<4> Then, the program joins the threads until each
has finished.
<5> The program ends the adding phase (by setting the
boolean variable `adding` to `false`) and starts 10 more threads.
<6> These
...
loop?
.  [[threeQuarterFullArrayExercise]] Explain why the `names` array in
...
. In <<Linked lists>>, we use two kinds of linked
