...
. [[ex11-1]] Give three advantages of using inheritance instead of copying and
pasting code from a parent class. Are there any disadvantages to using
inheritance?
. [[ex11-2]] Consider classes `Radish` and `Carrot` which both extend class
...
. [[ex11-3]] In the context of inheritance, the keyword `super` can be used for
two different purposes. What are they?
. [[ex11-4]] Consider the following class definitions.
+
[source,java]
----
public class A {
...
----
. [[ex11-5]] Beginning Java programmers often confuse package-private access (no
...
. [[ex11-6]] What are the similarities and differences between overloading a
method and overriding a method?
. [[ex11-7]] What is field hiding? How can software bugs arise from this
functionality in Java?
. [[ex11-8]] Give reasons why the designers of Java decided not to allow multiple
inheritance. Would you have made the same decision? Why or why not?
. [[ex11-9]] Draw a class hierarchy establishing a sensible relationship between
...
[start=10]
. [[ex11-10]] Create an `InternationalStudent` class that extends `Student`. It
...
fields.
. [[ex11-11]] Add `Pentagon` and `Hexagon` classes that extend the `Polygon`
...
. [[ex11-12]] The inheritance design of our solution to the Boolean circuits
...
extra classes.
. [[ex11-13]] Re-implement the object hierarchy in the solution to the sort it out
...
. [[ex11-14]] Design a celestial body simulator. You will
...
*Experiments*
[start=15]
. [[ex11-15]] Inheritance is a powerful technique, but it comes with some overhead
