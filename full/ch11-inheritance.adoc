...
The `Student` class we create in <<ch09-classes#ch09-classes>> is useful
...
stem:[y] values *increase* going downward, not upward.
...
stem:[x] and stem:[y] coordinates of the three vertices of
...
. What are the similarities and differences between overloading a
...
. Draw a class hierarchy establishing a sensible relationship between
...
. Create an `InternationalStudent` class that extends `Student`. It
...
fields.
. Add `Pentagon` and `Hexagon` classes that extend the `Polygon`
class. The constructor for each class should take an stem:[x],
stem:[y] and radius value, each of `int` type. Both classes
should be implemented to create _regular_ polygons, that is, polygons in
which all 5 or 6 sides have the same length. The stem:[x] and
stem:[y] values should give the center of the polygon, and each
...
`Point` values, the stem:[x] and stem:[y] coordinates of
the points must be `int` values. This requirement will force you to
round these stem:[x] and stem:[y] coordinates after using
...
. The inheritance design of our solution to the Boolean circuits
...
problem from <<ch10-interfaces#ch10-interfaces>>. This time, let the `Cat`,
...
. [[celestialSimulatorExercise]] Design a celestial body simulator. You will
need to create a class containing fields for the stem:[x],
stem:[y], and stem:[z] locations, stem:[x],
stem:[y], and stem:[z] velocities, radii, and masses of
each object. For each time step of length stem:[t], you must do
the following.
..  Compute the sum of forces exerted on each body by every other body.
The equation for gravitational force on body stem:[b] exerted by
body stem:[a] is
stem:[\mathbf{F}_{ab} = - G \frac{m_a m_b}{{\vert \mathbf{r}_{ab} \vert}^2} \, \mathbf{\hat{r}}_{ab}]
where
stem:[G = 6.673\times 10^{-11} \mbox{N} \mbox{m}^2 \mbox{kg}^{-2}],
stem:[\vert \mathbf{r}_{ab} \vert] is the distance between the
centers of objects stem:[a] and stem:[b], and
stem:[\mathbf{\hat{r}}_{ab} = \frac{\mathbf{r}_b - \mathbf{r}_a}{\vert\mathbf{r}_b - \mathbf{r}_a\vert}],
the unit vector between the centers of the two objects.
..  Compute the stem:[x], stem:[y], and stem:[z]
components of the acceleration vector stem:[{\mathbf a}] for each
object using the equation, stem:[{\mathbf F} = m {\mathbf a}],
once the sum of forces has been calculated.
..  Update the stem:[x], stem:[y], and stem:[z]
components of the velocity vector stem:[{\mathbf v}] for each
object using the equation
stem:[{\mathbf v}_{new} = {\mathbf v}_{old} + {\mathbf a}t].
