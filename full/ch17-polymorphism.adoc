...
1. [[ex17-1]] Consider the following two classes:
+
[source, java, linenums]
[[SaleProgram]]
----
...
System.out.println( sale.discount );
----
2. [[ex17-2]] What are the differences and similarities between abstract classes
and interfaces?
3. [[ex17-3]] Assume that the `Corn`, `Carrot`, and `Potato` classes are all
...
----
4. [[ex17-4]] How many different meanings of the keyword `final` are there in
Java, and what does each mean?
5. [[ex17-5]]  Assume that `Quicksand` is a subclass of `Danger`.
+
...
6. [[ex17-6]] Implement a program to assess income tax on normal employees,
...
7. [[ex17-7]] Refer to the sort given in <<Solution: Sort it out>> as the
...
functionality.
8.  [[ex17-8]] Again refer to the sort from
...
9. [[ex17-9]] Re-implement the original
...
+
*Experiments*
10. [[ex17-10]] Once you have implemented the sort in parallel from
<<ex17-8, Exercise 17.8>>, time it against the
...
expect?
11. [[ex17-11]] Take <<NoRaceConditionProgram>>
