...
. [[ex17-1]] Consider the following two classes:
+
[source, java, linenums]
[[SaleProgram]]
----
...
System.out.println( sale.discount );
----
. [[ex17-2]] What are the differences and similarities between abstract classes
and interfaces?
. [[ex17-3]] Assume that the `Corn`, `Carrot`, and `Potato` classes are all
...
----
. [[ex17-4]] How many different meanings of the keyword `final` are there in
Java, and what does each mean?
. [[ex17-5]]  Assume that `Quicksand` is a subclass of `Danger`.
+
...
[start=6]
. [[ex17-6]] Implement a program to assess income tax on normal employees,
...
. [[ex17-7]] Refer to the sort given in <<Solution: Sort it out>> as the
...
functionality.
.  [[ex17-8]] Again refer to the sort from
...
. [[ex17-9]] Re-implement the original
...
*Experiments*
[start=10]
. [[ex17-10]] Once you have implemented the sort in parallel from
...
expect?
. [[ex17-11]] Take <<NoRaceConditionProgram>>
