...
. [[ex10-1]] What is the purpose of an interface?
. [[ex10-2]] Why implement an interface when it puts additional requirements on a
class yet adds no functionality?
. [[ex10-3]] Is it legal to have methods marked `private` or `protected` in an
interface? Why did the designers of Java make this choice?
. [[ex10-4]] What is the `instanceof` keyword used for? Why is it useful in the
context of interfaces?
. [[ex10-5]] What kind of programming error causes a `ClassCastException`?
. [[ex10-6]] Create an interface called `ColorWavelengths` that only contains
...
|Violet |415
|======================
. [[ex10-7]] Write an interface called `Clock` that specifies the functionality a
...
that you specify.
. [[ex10-8]] There are four compiler errors in the following interface. Name each
...
----
. [[ex10-9]] Consider the interface defined below.
+
[source,java]
----
...
----
. [[ex10-10]] Write a single class that correctly implements the following three
interfaces.
+
[source,java]
----
...
----
. [[ex10-11]] If you are sorting a list of items latexmath:[n] elements long
...
[start=12]
. [[ex10-12]] Add client code that randomly creates the objects needing sorting in
...
. [[ex10-13]] Refer to the sort given as a solution in <<Solution: Sort it out>>. Add another `boolean` to the parameters of the sort which
...
. [[ex10-14]] After learning about threads in
...
*Experiments*
[start=15]
. [[ex10-15]] Once you have implemented the sort in parallel from
