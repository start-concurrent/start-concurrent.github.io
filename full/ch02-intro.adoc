...
stem:[h]. What is the maximum height the ball will reach after
the stem:[k^{\mathrm{th}}] bounce? We will discuss the steps
...
area of a triangle as stem:[base \times height] when the real
formula is stem:[\frac{1}{2} base
...
+
If stem:[c] is the coefficient of restitution, then the ball will
bounce back the first time to a height of stem:[h\times c]. Then,
...
stem:[h\times c^2] the second time. By examining this pattern for
the third and fourth bounce, it becomes clear that the ball will bounce
to a height of stem:[h\times c^k] on the
stem:[k^{\mathrm{th}}] time. See <<bouncingBallFigure>>
for a graphic description of this process.
+
We are assuming that stem:[k > 0] and that stem:[c < 1].
Note that stem:[c] depends on many factors, such as the
elasticity of the ball and the properties of the floor on which the ball
is dropped. However, if we know that we will be given stem:[c],
we don't need to worry about any other details.
+

[[bouncingBallFigure]]
[.text-center]
.A ball is dropped from height stem:[h]. The ball rises to height stem:[h c] after the first bounce and to stem:[hc^2] after the second.
...
+
Subproblem 1:::
  Get the values of stem:[h], stem:[c], and
  stem:[k] from the user.
Subproblem 2:::
  Compute the height of the ball after the stem:[k^{\mathrm{th}}]
  bounce.
Subproblem 3:::
...
stem:[h], stem:[c], and stem:[k]. It sends these
values to the next box, which represents a solution to subproblem 2.
This second box computes the height of the ball after stem:[k]
...
version (e.g., Jave SE 8 or 11) of the Java Platform,
...
was based on even older languages. Its syntax inherits ideas from C, C++,
and other languages.
...
should be stem:[15\times 0.3^{10} = 0.0000885735]. The result
...
.(a) Translation by one translator. Time stem:[t_s] gives the sequential time taken. (b) Translation by translators A and B. Times stem:[t_1], stem:[t_2], stem:[t_3], and stem:[t_4] give the times needed to do each component of the concurrent approach.
...
running times. We let stem:[t_s] be the time for one person to
complete the translation. The times stem:[t_1] through
stem:[t_4] mark the times needed to complete tasks 1 through 4,
...
..  Compute stem:[t_s], the time for you to translate the entire
...
speedup is stem:[t_s/t_c]. Using the values you have computed in
...
of a singlef-loor home. Suggest how two robots could be programmed to
...
Pythagorean formula stem:[a^2 + b^2 = c^2] to solve it?
