...
.  [[ex2-1]] When solving a problem using a
...
.  [[ex2-2]] In <<DistanceProgram>>, we declared all variables to be of type
...
[start=13]
. [[ex2-13]] Write a program that prompts the user
...
`System.out.println()`.
. [[ex2-14]] Expand the program from
...
. Rewrite your solution to <<ex2-14,Exercise 2.14>> so
that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
. [[ex2-16]] Copy and paste
...
. Rewrite your solution to <<ex2-16,Exercise 2.16>>
so that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
. [[ex2-18]] In <<inputForDistanceComputationExample>>, we assumed that the speed is given in miles
...
. [[ex2-19]] A program can use both a command
