...
.  [[ex7-1]] In which situations would it be better to use a command-line
interface instead of a GUI? When is it better to use a GUI over a
command-line interface?
.  [[ex7-2]] Explain the difference
...
[start=5]
.  [[ex7-5]] Modify the program in <<yes-NoDialogExample>> such that it
...
.  [[ex7-6]] Modify the program in <<yes-NoDialogExample>> such that it
...
dialog.
.  [[ex7-9]] Modify <<CapitalQuizProgram>> so that the button labeled "`Baku`" has focus when the program begins.
. [[ex7-10]] <<Solution: Three card poker>> gives a method called `shuffle()` that is used to randomize an
...
correct answer.
. [[ex7-11]] Re-implement the solution to the college cost calculator problem
given in <<Solution: College cost calculator>> so that it uses
GUIs constructed with `JOptionPane` for input and output.
. [[ex7-12]] Re-implement the solution to the Monty Hall problem given in
<<Solution: Monty Hall>> so that it uses GUIs constructed with
`JOptionPane` for input and output.
. [[ex7-13]] Re-implement the solution to the DNA searching problem given in
<<Solution: DNA searching>> so that it uses GUIs constructed with
`JOptionPane` for input and output.
. [[ex7-14]] Write a program that creates an input dialog that prompts and reads
...
. [[ex7-15]] *Note: You should attempt
