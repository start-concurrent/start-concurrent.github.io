...
integer stem:[x], presents it to the user, and asks the question,
"`Is stem:[sum_(i=1)^n i^3=((n(n+1))/2)^2] an odd integer?`" The answer given by the user is
...
<<integerFormatExceptionExercise>> asks you to modify
...
. [[shuffleCapitalsExercise]] <<Solution: Three card poker>> gives a method called `shuffle()` that is used to randomize an
...
. [[integerFormatExceptionExercise]] Use the `try-catch` block and modify
...
question asked).
+
*Note:* You should attempt this exercise only if you are familiar with exceptions in Java. Exceptions are covered in <<ch12-exceptions#ch12-exceptions>>.
