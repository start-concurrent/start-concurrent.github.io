...
Let's write a GUI program that will allow young math students to practice
...
A frame is the Java terminology for a window. GUI components in Java will usually be found on a frame. In
...
types of listeners, some of which are introduced here. Next we show you how to handle action
...
----
URL chirpURL =
    new URL("http://faculty.otterbein.edu/wittman1/chirp.wav");
----
...
✓ sign appears to its left and an `ItemEvent` is
...
`getStateChange()` method, it's possible to tell whether the widget
...
the square root of 2). When the other button is pressed, it increments
