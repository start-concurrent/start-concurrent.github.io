...
. [[ex9-1]] Explain the relationship between a class and an object.
. [[ex9-2]] What is the difference between a static method and an instance
method?
. [[ex9-3]] What is the purpose of a constructor? Why is it impossible for a
constructor to return a value? Why is it impossible for a constructor to
be called multiple times on the same object?
. [[ex9-4]] A static method can be called directly from a instance method, but
an instance method cannot be called directly from a static method. Why?
. [[ex9-5]] Describe the uses of accessor and mutator methods. Is it possible to
create a method that is both an accessor and a mutator? Why or why not?
. [[ex9-6]] Why do we usually mark fields with the `private` keyword when it
would be easier to make all fields `public`?
.  [[ex9-7]] What is the meaning of the `this` keyword? When is it necessary to
use it? When can it be ignored?
. [[ex9-8]] Consider the following class definitions.
+
[source,java]
----
public class A {
...
. [[ex9-9]] In <<Solution: Nested expressions>>, we gave a definition of
...
. [[ex9-10]] Consider the following class definition.
+
[source,java]
----
...
[start=11]
. [[ex9-11]] OOP is often used when the data inside the object must maintain
...
. [[ex9-12]] Draw on any of your hobbies to come up with a collection of items,
...
. [[ex9-13]] The `java.awt` package defines a class called `Point` that can be
...
. [[ex9-14]] Re-implement the solution from <<Solution: Nested expressions>> so that it performs its input and output with GUIs created
using `JOptionPane`.

*Experiments*
[start=15]
. [[ex9-15]] Objects are great tools for solving problems, but there is some
