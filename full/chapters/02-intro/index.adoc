...
.  [[exercise2.1]] When solving a problem using a
...
.  [[exercise2.2]] In <<DistanceProgram>>, we declared all variables to be of type
...
..  Compute latexmath:[t_s], the time for you to translate the entire
...
[start=13]
. [[exercise2.13]] Write a program that prompts the user
...
`System.out.println()`.
. [[exercise2.14]] Expand the program from
...
. Rewrite your solution to <<exercise2.14,Exercise 2.14>> so
that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
. [[exercise2.16]] Copy and paste
...
. Rewrite your solution to <<exercise2.16,Exercise 2.16>>
so that it uses a `JOptionPane`-based GUI instead of `Scanner` and
`System.out.print()`.
. [[exercise2.18]] In <<inputForDistanceComputationExample>>, we assumed that the speed is given in miles
...
. [[exercise2.19]] A program can use both a command
