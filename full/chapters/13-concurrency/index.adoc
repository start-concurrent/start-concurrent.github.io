...
.  [[exercise13.5]] Suppose that a computer has one
...
.  [[exercise13.6]] Consider the definition of speedup from
...
.  [[exercise13.9]] Consider the following table of
tasks:
+
...
. [[exercise13.10]] Consider the following code snippet.
+
[source,java]
----
x = 13;
...
[start=11]
. [[exercise13.11]] Re-implement the array summing problem from
...
with the technique.
. [[exercise13.12]] Composers often work with multiple tracks
...
. [[exercise13.13]] Write a program which takes a number of
...
*Experiments*
[start=15]
. [[exercise13.15]] Use the `currentTimeMillis()` method
...
. [[exercise13.16]] Thread creation overhead is an
...
. [[exercise13.17]] Create serial and concurrent
...
..  Given a machine with latexmath:[k>1] cores, what is the maximum
speedup you can expect to obtain?
. [[exercise13.18]] Repeatedly run the code in
...
. [[exercise13.19]] Create serial and parallel implementations of
...
first?
. [[exercise13.20]] The solution to the array summing problem in
