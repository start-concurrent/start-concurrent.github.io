...
.  [[exercise15.2]] Note that both `ActionListener`
...
.  [[exercise15.7]] Write a program that creates a
...
. [[exercise15.10]] Write a Java program that
creates a GUI containing a label with a picture of yourself.
. [[exercise15.11]] Extend the `SoundGame` class
...
. [[exercise15.12]] In <<soundGameExample>> we stopped both the chirp and the bark sounds because the action
...
the Internet.
. [[exercise15.14]] Modify the applet from
...
the GUI.
. [[exercise15.15]] Remove the `actionPerformed()` and
...
panel is visible?
. [[exercise15.18]] Remove the line `frame.setResizable(false);` from <<FlowLayoutExampleProgram>>. Run the modified program and
resize the frame to various sizes. How does the placement of the buttons
change?
. [[exercise15.19]] Modify
...
`pack()` method?
. [[exercise15.20]] Create a GUI with a frame that uses
...
. [[exercise15.21]] Write a Java program that creates
...
bar to a `JFrame`?
. [[exercise15.22]] Use the example from the book to
...
. [[exercise15.24]] <<UnresponsiveGUIProgram>> is
...
. [[exercise15.25]] Take the computationally expensive loop
