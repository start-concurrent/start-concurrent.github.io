...
. [[exercise10.1]] What is the purpose of an interface?
. [[exercise10.2]] Why implement an interface when it puts additional requirements on a
class yet adds no functionality?
. [[exercise10.3]] Is it legal to have methods marked `private` or `protected` in an
interface? Why did the designers of Java make this choice?
. [[exercise10.4]] What is the `instanceof` keyword used for? Why is it useful in the
context of interfaces?
. [[exercise10.5]] What kind of programming error causes a `ClassCastException`?
. [[exercise10.6]] Create an interface called `ColorWavelengths` that only contains
...
|Violet |415
|======================
. [[exercise10.7]] Write an interface called `Clock` that specifies the functionality a
...
that you specify.
. [[exercise10.8]] There are four compiler errors in the following interface. Name each
...
----
. [[exercise10.9]] Consider the interface defined below.
+
[source,java]
----
...
----
. [[exercise10.10]] Write a single class that correctly implements the following three
interfaces.
+
[source,java]
----
...
----
. [[exercise10.11]] If you are sorting a list of items latexmath:[n] elements long
...
[start=12]
. [[exercise10.12]] Add client code that randomly creates the objects needing sorting in
...
. [[exercise10.13]] Refer to the sort given as a solution in <<Solution: Sort it out>>. Add another `boolean` to the parameters of the sort which
...
. [[exercise10.14]] After learning about threads in
...
*Experiments*
[start=15]
. [[exercise10.15]] Once you have implemented the sort in parallel from
