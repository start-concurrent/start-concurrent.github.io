...
.  [[exercise5.1]] If you have a `String` containing a long text and you want to count
the number of words in the text that begin with the letter `'m'`, which
of the three kinds of loops would you use, and why?
.  [[exercise5.2]] In <<primalityTestingExample>>,
...
----
. [[exercise5.3]] How many different DNA sequences of length latexmath:[n] are
there?
. [[exercise5.4]] There are three different errors in the following loop intended to
...
----
. [[exercise5.5]]  Consider the following code containing nested `for` loops.
+
[source,java]
----
Scanner in = new Scanner(System.in);
...
[start=6]
.  [[exercise5.6]] Write a program that converts base 10 numbers into base 3 numbers.
...
.  [[exercise5.7]] The greatest common divisor (GCD) of two integers is the largest
...
.  [[exercise5.8]] In the solution to the DNA searching
...
.  [[exercise5.9]] Write a program that reads a number latexmath:[n] from a user and
...
. [[exercise5.10]] Re-implement the solution to the DNA searching program given in
<<Solution: DNA searching>> using `JOptionPane` to generate GUIs
for input and output.

*Experiments*
[start=11]
. [[exercise5.11]] Using a `for` loop, record the Monty Hall simulation so that you can
...
. [[exercise5.12]] Write three nested `for` loops, each of which run 1,000 times.
...
. [[exercise5.13]] In <<Common pitfalls>>, one of the common loop
...
an `int`?
. [[exercise5.14]] In <<primalityTestingExample>>, we gave three programs to
...
`PrimalityTester2`?
. [[exercise5.15]] In <<parallelismWithoutThreadsExample>>, we ran four programs at the same time to solve a
