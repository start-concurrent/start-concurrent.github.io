...
.  [[exercise19.1]] <<multiplicationDefinedRecursivelyExample>> gave a mathematical recursive definition for
...
.  [[exercise19.2]] In principle, every problem that can be solved with an iterative
...
.  [[exercise19.3]] Consider the first (non-memoized)
...
.  [[exercise19.4]] In the recursive `solve()` method in the `MazeSolver` program given
...
[start=5]
.  [[exercise19.5]] <<exercise8.11, Exercise 8.11>>
...
----
.  [[exercise19.6]] The efficient implementation of
...
.  [[exercise19.7]] Write an implementation of
...
\end{cases}]
.  [[exercise19.8]] <<fibonacciImplementedRecursivelyExample>> shows two implementations that can be used to
...
.  [[exercise19.9]] <<recursiveLinkedListSizeExample>> shows a way to calculate the size of a linked list
...
. [[exercise19.10]] Expand the previous exercise
...
written.
. [[exercise19.11]] Create a recursive `find()` method (and a
...
. [[exercise19.12]] The height of a binary tree is defined as
...
its two subtrees.
. [[exercise19.13]] Create an `interface` that
describes a tree ADT. Modify the programs in <<binarySearchTreeExample>> to use this `interface`.

*Experiments*
[start=14]
. [[exercise19.14]] Write an iterative version of the factorial function and compare its
...
. [[exercise19.15]] Write a program that generates four arrays
...
. [[exercise19.16]] Investigate the performance of using recursion to compute Fibonacci
...
recursive solution.)
. [[exercise19.17]] <<exercise6.9, Exercise 6.9>> from <<Arrays>> explains how binary search can be used to search for a value in a sorted
