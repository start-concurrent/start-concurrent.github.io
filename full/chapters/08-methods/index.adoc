...
. [[exercise8.1]] Describe three advantages of dividing long segments of code into
static methods.
. [[exercise8.2]] Can you think of any disadvantages of dividing code into methods?
Are there situations where using a method is unwise?
. [[exercise8.3]] If you wanted to declare a static method that would compute the
mean, median, and standard deviation of an input array of `double`
values, how would you return those three answers?
. [[exercise8.4]] Consider the following method definition.
+
[source,java]
----
...
while( x < 128 )
    twice(x);
----
. [[exercise8.5]] Consider the following signatures of two overloaded methods.
+
[source,java]
----
...
. [[exercise8.6]] The following class generates a sequence of even numbers. Each time
...
[start=7]
. [[exercise8.7]] Write a static method called `cube()` that takes a single `double`
value as a parameter and returns its value cubed. Do not use the
`Math.pow()` method.
. [[exercise8.8]] Implement a static method that takes a single `int` value as a
...
.  [[exercise8.9]] Write a static method that takes an array of `int` values as a
...
. [[exercise8.10]] Write a static method that finds the
...
|10 |3 |1024 |10
|=============================
. [[exercise8.11]] Write a method that tests palindromes like
...
. [[exercise8.12]] Re-implement the solution from <<Solution: Three card poker>>
so that it uses a GUI constructed with `JOptionPane` to display the hand
and the winnings.
. [[exercise8.13]] Five card poker is a much more common version of poker than the
...
*Experiments*
[start=14]
. [[exercise8.14]] In terms of time, there is a small overhead associated with calling
