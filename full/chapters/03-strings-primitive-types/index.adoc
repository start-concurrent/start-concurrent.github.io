...
.  [[exercise3.1]] What is the difference
between the set of integers from mathematics and the sets defined by
`int` and `long`?
.  [[exercise3.2]] In <<downcastErrorExample>>, the sum
...
.  [[exercise3.3]] The following three
...
----
.  [[exercise3.4]] Consider the following variable
declarations.
+
[source, java]
----
...
..  `5 << 2`
..  `5 >>> 2`
.  [[exercise3.6]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
..  `Byte.MIN_VALUE - 1`
..  `Byte.MAX_VALUE + 1`
..  `Integer.MIN_VALUE - 1`
.  [[exercise3.7]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
..  `Float.MAX_VALUE + 1`
..  `Double.MAX_VALUE - 1`
..  `-Double.MAX_VALUE - 1`
..  `-Double.MIN_VALUE - 1`
..  `-Double.MIN_VALUE + 1`
.  [[exercise3.8]] When evaluated in Java, the
...
.  [[exercise3.9]] Explain what is printed when the
...
..  `f(a * g(b))`
..  `f(g(a)) + f(b) + f(c)`
. [[exercise3.12]] Answer the following questions about
...
Java.
. [[exercise3.13]] Consider the following
...
. [[exercise3.14]] Characters `'a'` and `'A'` have Unicode values
`\u0061` and `\u0041`, respectively. Give the representation of these
two characters as 16-bit unsigned binary integers.
. [[exercise3.15]] Assuming that each character occupies 16
...
. [[exercise3.16]] What is the output from the following
...
----
. [[exercise3.17]] What is the output from the
...
[start=18]
. [[exercise3.18]] Try compiling the
