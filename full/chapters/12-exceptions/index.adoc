...
.  [[exercise12.1]] What are the advantages of
using exceptions instead of returning error codes?
.  [[exercise12.2]] The keywords `final` and `finally`,
as well as the `Object` method `finalize()`, are sometimes confused.
What is the purpose of each one?
.  [[exercise12.3]] What is the difference between the
`throw` keyword and the `throws` keyword?
.  [[exercise12.4]] Explain the catch or specify
requirement of Java.
.  [[exercise12.5]] What must be done differently
...
.  [[exercise12.6]] For every program you write, you
...
.  [[exercise12.7]] Why did the designers of Java choose to make `NullPointerException`
and +
...
.  [[exercise12.8]] Consider the following two classes.
+
[source,java]
----
...
.  [[exercise12.9]] What value will the following method
...
----
. [[exercise12.10]] Why will the following
...
----
. [[exercise12.11]] Consider the following fragment of
Java.
+
[source,java]
----
...
[start=12]
. [[exercise12.12]] The `NumberFormatException`
...
. [[exercise12.14]] Users often log onto systems by
...
*Experiments*
[start=15]
. [[exercise12.15]] Throwing and catching exceptions
