...
If the output doesn't match our expectations, we may not have enough
...
In Eclipse, you can set set breakpoints either by right clicking on the shaded bar
immediately to the left of the line you're interested in or by
...
statement, but it skips over method calls. The Step Return command
...
execution reaches the specified line.
...
corresponding to the common bugs listed in <<Fixing bugs>>.
...
[source,java]
----
red = (byte)Math.min(255, 2*(red & 0xFF)); // Bitwise AND automatically upcasts to int
...
discussed in <<ch18-dynamic-data-structures#ch18-dynamic-data-structures>>, can reduce the
...
iterate _n_ times, start at 0 and go up to but not including
_n_. Alternatively, if you have a reason not to be
...
the method to be error free, the addition of error checking is rarely a bad idea.
...
Thus, the first call to `brag()` will print out `My strength is 10!`, but the second call will print out `My strength is 20!`.
====
...
you should download the latest jar file from the http://www.junit.org[JUnit site^] and
