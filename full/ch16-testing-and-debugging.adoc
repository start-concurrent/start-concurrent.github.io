...
. [[ex16-1]] What is the purpose of the `assert` keyword in Java? What steps must
be taken for it to be active?
. [[ex16-2]] What is the value of `j` after the following statements are
executed?
+
[source,java]
----
...
. [[ex16-3]] The following loop is intended to print out all possible `byte`
...
----
. [[ex16-4]] What are all the possible run-time errors that could occur in this
...
. [[ex16-5]] Consider the following definition of a stack that holds `int`
values.
+
[source,java]
----
...
be thrown?
. [[ex16-6]] This question sometimes comes up in job interviews. Imagine that you
...
execution?
. [[ex16-7]] What is the difference between black box testing and white box
testing? What kinds of bugs are more likely to be caught by black box
testing? By white box testing?
. [[ex16-8]] The Microsoft Zune is a portable media player in competition with
...
[start=9]
.  [[ex16-9]] Apply JUnit testing to the last major assignment you did in class.
What bugs did you uncover?

*Experiments*
[start=10]
. [[ex16-10]] James Gosling's original specification for Java contained
...
. [[ex16-11]] Take another look at your last programming assignment. Calculate the
...
. [[ex16-12]] Take a concurrent program you have written that relies on explicit
