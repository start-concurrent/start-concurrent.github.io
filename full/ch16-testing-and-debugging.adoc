...
bugs in software. For this book, this chapter is unique in that it's not based
on clearly stated concrete problems with straightforward solutions. Finding
and fixing bugs in software, especially concurrent
software, is a problem no one has solved completely. The first
...
testing for bugs hidden in your code.
...
chapters. We won't discuss syntax errors or any other errors which
can be caught at compile time. Instead, all bugs discussed in this
chapter are run-time bugs.

There are an infinite number of possible bugs, so they can't all be
...
confirms that our program is doing what we imagine it should be doing.
...
in the field for VM arguments. There are other options allowing you to
...
can use essentially the same tools to advance execution as you can in Eclipse.
...
Precision can cause subtle errors, especially with floating-point types.
...
express incorrect code. We're going to mention a few of the most common loop errors.
...
your ending condition that you forget to change it. You only get 49
questions printed out. If your purpose was making an exam, you can
...
causes no ouput, in this case) will scour the body of the loop without
...
numbers input by the user. This time we've intialized `count` to be -1
...
situations and fail in others, if it's dependent on the `==` operator.
...
never is in Java. Although negative indexes occur in code, the error is
usually accessing an index slightly larger than the bounds of an array,
particularly with a loop.
...
want to pick the list based on the first letter of the word, we could have
...
each element of the array is just a reference to `null` until it's
initialized.
...
This kind of mistake is also common in constructors, since it's
...
is itself a reference, it is possible to change the values that it references.
...
We'll only briefly discuss parallel bugs here because we've already
spoken in depth about the dangers of parallel programming in
<<ch14-synchronization#ch14-synchronization>>. Except in the case of deadlocks and
...
to be aware that a race condition is occurring.
...
of the program and watching for errors. Two sets of eyes is always beneficial
...
a program, it's generally recognized that it's useful to have the
individuals who test the software be independent from those who develop
it. By keeping the testers separate, they're not infected by the
assumptions and biases that the developers made while writing the
software. Some communication between the two groups is necessary, but
there's value in black-box testing, which we'll explain in the next subsection.
...
Equivalence Partitioning:::
  ...
  For this reason equilvalence partitioning is also referred to as _subdomain_
  testing. +
{empty} +
...
Fuzz Testing:::
  ...
  stress testing, in which the program's ability to process a large amount
...
because it's open-source. First, we'll explain how to use JUnit, and
then we'll discuss tools for testing concurrent software.
...
but it also relies on _annotations_. Annotations are additional information
...
`@Deprecated`, it's deprecated and included only for backwards compatibility.
...
`@Deprecated`. The `@Override` annotation marks a method that's
...
including an delta threshold in case the values don't match exactly.
...
what you want your code to do, a way to know when you're done writing your
code, a way to document your code, and a suite of tests that grows along with
your program.
...
allow you to disable tests or run them only on particular platforms.
...
bugs might not manifest even after thousands of runs with ConTest
instrumented code.
// This site no longer works...
// For more information about ConTest, visit the IBM
...
maintained at the +
http://www.concutest.org[Concutest site^]. Concutest includes
...
products from Intel, like many concurrency tools, are focused on C/{cpp}
...
physics that the force _F_ between two charges
_q_~1~ and _q_~2~ is given by:
...
appropriate values into the equation for _F_.
...
. [[assertExercise]] What is the purpose of the `assert` keyword in Java? What steps must
be taken for it to be active?
. What is the value of `j` after the following statements are
executed?
+
[source,java]
----
...
. Consider the following definition of a stack that holds `int`
values.
+
[source,java]
----
...
<<ch18-dynamic-data-structures#ch18-dynamic-data-structures>>. What if there is a loop in
...
execution?
. [[blackBoxTestingExercise]] What is the difference between black box testing and white box
testing? What kinds of bugs are more likely to be caught by black box
testing? By white box testing?
. The Microsoft Zune was a portable media player in competition with
