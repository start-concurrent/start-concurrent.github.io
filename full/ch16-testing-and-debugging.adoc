...
looking. Certain data structures such as the `list` template in {cpp} may
...
iterate stem:[n] times, start at 0 and go up to but not including
stem:[n]. Alternatively, if you have a reason not to be
...
fact, a program with stem:[n] independent conditionals would
require stem:[2^n] runs to test them all. Because of the large
...
arbitrary stem:[x], stem:[y], and stem:[z]
...
physics that the force stem:[F] between two charges
stem:[q_1] and stem:[q_2] is
stem:[k_\mathrm{e} \frac{q_1q_2}{r^2}] where
stem:[k_\mathrm{e}] is the proportionality constant
stem:[8.9875517873681764 \times 10^9 \ \mathrm{N  \cdot m^2 \cdot C^{-2}}]
and stem:[r] is the distance between the charges. Likewise the
electric field stem:[E] at a given location due to a charge
stem:[q] is stem:[k_\mathrm{e} \frac{q}{r^2}].
...
stem:[\sqrt{2}]. Yet, when we run this test with JUnit, the test
fails. We get:
...
appropriate values into the equation for stem:[F].
