...
1.  [[ex3-1]] What is the difference
between the set of integers from mathematics and the sets defined by
`int` and `long`?
2.  [[ex3-2]] In <<downcastErrorExample>>, the sum
...
----
4.  [[ex3-4]] Consider the following variable
declarations.
+
[source, java]
----
...
5.  [[ex3-5]] Evaluate the following
...
f.  `5 << 2`
g.  `5 >>> 2`
6.  [[ex3-6]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Byte.MIN_VALUE - 1`
b.  `Byte.MAX_VALUE + 1`
c.  `Integer.MIN_VALUE - 1`
7.  [[ex3-7]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
a.  `Float.MAX_VALUE + 1`
...
8.  [[ex3-8]] When evaluated in Java, the
...
9.  [[ex3-9]] Explain what is printed when the
...
b.  `f(a * g(b))`
c.  `f(g(a)) + f(b) + f(c)`
12. [[ex3-12]] Answer the following questions about
...
Java.
13. [[ex3-13]] Consider the following
...
16. [[ex3-16]] What is the output from the following
...
----
17. [[ex3-17]] What is the output from the
