...
As you have seen in <<ch02-intro#ch02-intro>>,
...
how to work with exceptions in <<ch12-exceptions#ch12-exceptions>>.
...
<4> We start the second thread running. Every object whose type is `Thread` (or a child of `Thread`, which we will discuss in <<ch11-inheritance#ch11-inheritance>>) has a `start()` method which begins its execution as a separate thread.
...
design, and inheritance that we will cover in <<ch08-methods#ch08-methods>>, <<ch09-classes#ch09-classes>>, and <<ch11-inheritance#ch11-inheritance>>.
