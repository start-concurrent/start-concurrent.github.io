...
----
.  [[ex3-4]] Consider the following variable
declarations.
+
[source, java]
----
...
..  `5 << 2`
..  `5 >>> 2`
.  [[ex3-6]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
..  `Byte.MIN_VALUE - 1`
..  `Byte.MAX_VALUE + 1`
..  `Integer.MIN_VALUE - 1`
.  [[ex3-7]] Evaluate the following
expressions by hand and then check the results with a Java compiler.
..  `Float.MAX_VALUE + 1`
..  `Double.MAX_VALUE - 1`
..  `-Double.MAX_VALUE - 1`
..  `-Double.MIN_VALUE - 1`
..  `-Double.MIN_VALUE + 1`
.  [[ex3-8]] When evaluated in Java, the
...
.  [[ex3-9]] Explain what is printed when the
...
..  `f(a * g(b))`
..  `f(g(a)) + f(b) + f(c)`
. [[ex3-12]] Answer the following questions about
...
. [[ex3-16]] What is the output from the following
...
----
. [[ex3-17]] What is the output from the
