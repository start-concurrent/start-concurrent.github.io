...
stem:[P] be the amount of the loan (the principal). Let
stem:[J] be the monthly interest rate (the annual interest rate
divided by 12). Let stem:[N] be the number of months to pay back
the loan (years of the loan times 12). Let stem:[M] be the
monthly payment we want to calculate, then:

stem:[M = P \cdot \frac{J}{1 - (1 + J)^{-N}}]
...
In this equation, stem:[x] has the value stem:[4], and it
always will. You can set up another equation in which stem:[x]
...
The `+=` operator gets the contents of the variable, in this case `value`,
...
the name of the variable twice. And `+=` is not the only shortcut. It's
...
|exp()|double population = 250 * Math.exp(0.03 * time);|Compute stem:[e^x], where stem:[x] is the
argument.
|log()|double digits = Math.log(1000000);|Compute the natural logarithm of the argument.
|pow()|double money = principal * Math.pow(1.0 + rate, time);|Compute stem:[a^b], where stem:[a] and stem:[b]
are the first and second arguments.
3+^.^d|Miscellaneous
|random()|double percent = Math.random();|Generate a random number stem:[x] where
stem:[0.0 \leq x < 1.0].
...
including Euler's number stem:[e] and stem:[\pi]. These
