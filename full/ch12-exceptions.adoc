...
.  [[ex12-1]] What are the advantages of
using exceptions instead of returning error codes?
.  [[ex12-2]] The keywords `final` and `finally`,
as well as the `Object` method `finalize()`, are sometimes confused.
What is the purpose of each one?
.  [[ex12-3]] What is the difference between the
`throw` keyword and the `throws` keyword?
.  [[ex12-4]] Explain the catch or specify
requirement of Java.
.  [[ex12-5]] What must be done differently
...
.  [[ex12-6]] For every program you write, you
...
.  [[ex12-7]] Why did the designers of Java choose to make `NullPointerException`
and +
...
.  [[ex12-8]] Consider the following two classes.
+
[source,java]
----
...
.  [[ex12-9]] What value will the following method
...
----
. [[ex12-10]] Why will the following
...
----
. [[ex12-11]] Consider the following fragment of
Java.
+
[source,java]
----
...
[start=12]
. [[ex12-12]] The `NumberFormatException`
...
. [[ex12-14]] Users often log onto systems by
...
*Experiments*
[start=15]
. [[ex12-15]] Throwing and catching exceptions
