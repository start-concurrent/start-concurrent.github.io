...
exactly stem:[\frac{n}{k}] items to work on. For example, the first
...
of the array: Each thread gets stem:[\lfloor \frac{n}{k} \rfloor] (using integer
...
with 10 threads and 28 pieces of data. stem:[\lfloor \frac{28}{10} \rfloor = 2],
...
stem:[\lfloor \frac{n}{k} \rfloor] (rounded down because of integer division) elements,
...
to find an element in a list of stem:[n] items. In contrast, looking through the list one
...
function of stem:[n], how many times total does the body of the inner
