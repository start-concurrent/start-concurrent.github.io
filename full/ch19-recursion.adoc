...
of itself. It's common to define functions and sequences (and also
sets) recursively. Functions, such as _f_(_n_), are usually
defined in relation to the same function with a smaller input, such as
_f_(_n_ - 1). Sequences, such as _s_~_n_~, are
...
repeatedly adding the _x_ a total of _y_ times.
...
Recursive Case:::
  ...
  the _n_^th^ disk to the destination pole, then move the
...
_a_^_n_^ = (_a_^_n_/2^)^2^, but what does that
buy us? If we structure our method correctly, we cut the size of
_n_ in half at each recursive step instead of only reducing
_n_ by 1.
...
point in the recursion, the value of _n_ is odd unless you
...
_x_ ⋅ _y_. Give a similar recursive definition for
_x_ + _y_. The structure is similar to the recursion to
...
array is is set to a blank character (`'{nbsp}'`) after no solution had been
...
find the _n_^th^ Fibonacci number. With a little bit of
math, it is possible to show that there is a closed-form equation that
gives the _n_^th^ Fibonacci number _F_~_n_~ where
_F_~0~ = _F_~1~ = 1.
+
//[stem]
//++++
...
equation and discover the value of _F_~_n_~ quickly, provided
that you have an efficient way to raise values to the
_n_^th^ power. Use the recursive algorithm for fast
exponentation from <<fastExponentiationExercise>> to
make an implementation that finds the _n_^th^ Fibonacci
number *very* quickly.
+
...
each of the sorts. Note that both selection sort and bubble sort may take
...
_n_ log _n_ (multiplied by some constant) for merge sort. For
...
`System.nanoTime()` method to time the computations for large values of `n`.
+
...
search and then recursive binary search. Use the `System.nanoTime()` method to time each
