...
to futures, but these tools are enough to get you started.
...
<.> Its `call()` method sums the square roots of all the `int` values greater
...
to handle two unrelated kinds of exceptions with the same `catch` block.
