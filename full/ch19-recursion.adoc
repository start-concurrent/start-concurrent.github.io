...
given year. Here stem:[age(year)] is a function that gives the
age you were during year stem:[year].
...
sets) recursively. Functions (such as stem:[f(n)]) are usually
defined in relation to the same function with a smaller input (such as
stem:[f(n - 1)]). Sequences (such as stem:[s_n]) are
...
multiplication stem:[x \times y]. This multiplication means
repeatedly adding the stem:[x] a total of stem:[y] times.
If stem:[y] is a positive integer, we can describe this
...
factorial function, often written stem:[n!]. The factorial
...
and sequences precisely without using ellipses (stem:[\ldots]).
...
Note that the base case gives stem:[1] as the answer when
stem:[n = 0]. By convention stem:[0! = 1]. Thus, this
definition correctly gives stem:[0! = 1],
stem:[1! = 1\cdot 0!
= 1], stem:[2! = 2\cdot 1! = 2],
stem:[3! = 3\cdot 2! = 6], and so on.
...
1, 1, 2, 3, 5, 8, 13, 21, stem:[\ldots]. Each term after the two
...
stem:[n]^th^ Fibonacci number stem:[F_n] as follows,
starting with stem:[n=0]. stem:[F_n=
...
Since the stem:[F_n] depends on the two previous terms, it is
...
the stack abstract data type in <<ch18-dynamic-data-structures#ch18-dynamic-data-structures>>. A similar structure is used to control the flow of control
...
This change makes the computation of the stem:[n]^th^ Fibonacci
...
Base Case:::
  ...
  destination pole.
Recursive Case:::
  In order to move stem:[n > 1] disks from one pole to another,
  we can move stem:[n - 1] disks to an intermediate pole, move
  the stem:[n]^th^ disk to the destination pole, then move the
  stem:[n - 1] disks from the intermediate pole to the
  destination pole.
...
trivial: Given a rational number stem:[a] and a positive integer
stem:[n], find the value of stem:[a^n].
...
stem:[n]. Ignoring that limitation, what can we say about its
efficiency? For any legal value of stem:[n], the method is called
stem:[n] times. If stem:[n] has a small value, like 2 or
3, the process is quick. But if stem:[n] is 1,000,000 or so, the
...
If we limit stem:[n] to a power of 2, we can do something clever
...
stem:[a^n = \left(a^{\frac{n}{2}}\right)^2], but what does that
buy us? If we structure our method correctly, we cut the size of
stem:[n] in half at each recursive step instead of only reducing
stem:[n] by 1.
...
the previous method. That version took stem:[n] recursive calls.
How efficient is this version? The answer is the number of times you
have to cut stem:[n] in half before you get 1. Let's call that
value stem:[x]. Recall that stem:[n] is a power of 2,
meaning that stem:[n = 2^k] for some integer
stem:[k \geq 0].
...
In other words, the number of times you have to divide stem:[n]
in half to get 1 is the logarithm base 2 of stem:[n], written
stem:[\log_2 n]. The logarithm function is the inverse of
...
example, stem:[2^{20} = 1,048,576]. Thus,
stem:[\log_2 1,048,576 = 20]. The original version of `power()`
would have to make 1,048,576 calls to raise a number to that power. This
second version would only have to make 20 calls.

It is critical that stem:[n] is a power of 2 (1, 2, 4, 8, …),
otherwise the process of repeatedly cutting stem:[n] in half
loses some data due to integer division. The problem is that, at some
point in the recursion, the value of stem:[n] is odd unless you
start with a power of 2. There is a way to extend this clever approach
to all values of stem:[n], even and odd, but we leave it as an
exercise.
====
...
The linked list examples from <<ch18-dynamic-data-structures#ch18-dynamic-data-structures>> are recursive data structures, since a linked list node is
...
and print them out in alphabetical order. <<TreeProgram>> shows
...
.Class that implements a simple binary search tree ADT for creating a sorted list of strings.
[source, java]
[[TreeProgram]]
----
...
in <<ch18-dynamic-data-structures#ch18-dynamic-data-structures>>, we want our data
...
[source, java]
----
include::{programsdir}/MazeSolver.java[lines=16..29]
----
...
[source, java]
----
include::{programsdir}/MazeSolver.java[lines=31..37]
----
...
[source, java]
----
include::{programsdir}/MazeSolver.java[lines=39..57]
----
...
location to each of the four neighboring cells by recursively calling `solve()`. If any of those four neighbors returns `true`, then `solve()` knows it has found a completed path and returns `true` to its caller.
...
[source, java]
----
include::{programsdir}/RootFutures.java[lines=1..14]
----
...
[source, java]
----
include::{programsdir}/RootFutures.java[lines=16..22]
----
...
[source, java]
----
include::{programsdir}/RootFutures.java[lines=24..40]
----


All that remains is to collect the values from each future. We iterate
through the list of futures with an enhanced `for` loop and add the return
value of each future's `get()` method to our running total `sum`. Because
...
.  [[recursiveAdditionExercise]] <<multiplicationDefinedRecursivelyExample>> gave a mathematical recursive definition for
stem:[x \times y]. Give a similar recursive definition for
stem:[x + y]. The structure is similar to the recursion to
determine your current age given in <<What is recursion?>>.
...
`fibonacci()` called with argument stem:[1] to compute
`fibonacci(n)`? Instrument your program and count the number of calls
for stem:[n = 2, 3, 4 \ldots 20].
...
fast exponentiation that works for even and odd stem:[n]. This
implementation is exactly the same as the one given at the end of
<<exponentiationExample>> except when stem:[n] is odd.
...
\end{cases}]
.  [[fastFibonacciExercise]] <<fibonacciImplementedRecursivelyExample>> shows two implementations that can be used to
find the stem:[n]^th^ Fibonacci number. With a little bit of
math, it is possible to show that there is a closed-form equation that
gives the stem:[n]^th^ Fibonacci number stem:[F_n] where
stem:[F_0 = F_1 = 1].
+
stem:[F_n = \frac{\left(\frac{1 + \sqrt{5}}{2}\right)^{n+1}  -  \left(\frac{1 -
\sqrt{5}}{2}\right)^{n+1}}{\sqrt{5}}]
+
Although this math is a bit ugly, you can plug numbers into this
equation and discover the value of stem:[F_n] quickly, provided
that you have an efficient way to raise values to the
stem:[n]^th^ power. Use the recursive algorithm for fast
exponentation from <<fastExponentiationExercise>> to
make an implementation that finds the stem:[n]^th^ Fibonacci
number *very* quickly.
+
Note that this approach uses real numbers (including
stem:[\sqrt{5}]) that need to be represented as `double` values.
...
.  [[recursivePrintExercise]] <<recursiveLinkedListSizeExample>> shows a way to calculate the size of a linked list
...
. [[reversePrintExercise]] Expand the previous exercise
...
its two subtrees.
. [[treeInterfaceExercise]] Create an `interface` that
describes a tree ADT. Modify the programs in <<binarySearchTreeExample>> to use this `interface`.
...
<<mergeSortExample>>, respectively. Use the
...
an array length of stem:[n] should take time proportional to
stem:[n^2] (multiplied by some constant) for selection sort and
bubble sort, but it should take time proportional to
stem:[n \log n] (multiplied by some constant) for merge sort. For
...
computations for large values of `n`.
+
*Warning:* It may take a very long time to compute the `n`^th^ Fibonacci number with the naive
recursive solution.
