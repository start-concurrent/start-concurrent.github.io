...
.  [[ex19-1]] <<multiplicationDefinedRecursivelyExample>> gave a mathematical recursive definition for
...
.  [[ex19-2]] In principle, every problem that can be solved with an iterative
...
.  [[ex19-3]] Consider the first (non-memoized)
...
.  [[ex19-4]] In the recursive `solve()` method in the `MazeSolver` program given
...
----
.  [[ex19-6]] The efficient implementation of
...
.  [[ex19-7]] Write an implementation of
...
\end{cases}]
.  [[ex19-8]] <<fibonacciImplementedRecursivelyExample>> shows two implementations that can be used to
...
.  [[ex19-9]] <<recursiveLinkedListSizeExample>> shows a way to calculate the size of a linked list
...
. [[ex19-10]] Expand the previous exercise
...
written.
. [[ex19-11]] Create a recursive `find()` method (and a
...
. [[ex19-12]] The height of a binary tree is defined as
...
its two subtrees.
. [[ex19-13]] Create an `interface` that
describes a tree ADT. Modify the programs in <<binarySearchTreeExample>> to use this `interface`.

*Experiments*
[start=14]
. [[ex19-14]] Write an iterative version of the factorial function and compare its
...
. [[ex19-15]] Write a program that generates four arrays
...
. [[ex19-16]] Investigate the performance of using recursion to compute Fibonacci
...
recursive solution.)
. [[ex19-17]] <<ex6-9, Exercise 6.9>> from <<ch06-arrays#ch06-arrays>> explains how binary search can be used to search for a value in a sorted
