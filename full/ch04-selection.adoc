...
. [[ex4-1]] Given that latexmath:[x], latexmath:[y], and latexmath:[z] are
propositions in Boolean logic, make a truth table for the expression
latexmath:[(\lnot(x \land \lnot y)
\oplus \lnot z)].
. [[ex4-2]] What is the value of the Boolean expression latexmath:[\lnot((T
\oplus F) \land \lnot(F \lor T))]?
. [[ex4-3]] The calculation to determine the leap year given in
...
and `&&`.
.  [[ex4-4]] The XOR operator (`^`) is useful for combining `boolean` values, but
it can be replaced with a more commonly used *relational* operator in
Java. Which one?
. [[ex4-5]] De Morgan's laws are the following, which show that the process of
...
. [[ex4-6]] Use De Morgan's laws given above to rewrite the following statement
...
----
. [[ex4-7]] Consider the following fragment of code.
+
[source,java]
----
int x = 5;
...
`y > 10 & (x = 10) > 5`? Why?
. [[ex4-8]] Consider the following fragment of code.
+
[source,java]
----
int a = 7;
...
[start=9]
. [[ex4-9]] This problem has two parts.
...
. [[ex4-11]] Re-implement both parts from <<ex4-10, Exercise 4.10>> using `switch` statements instead of `if` statements.
. [[ex4-12]] Expand the program given in
...
. [[ex4-14]] At the bottom of <<`switch` statements>>, we use a
...
statements instead.
. [[ex4-14]] Every member of your secret club has an ID number. These ID numbers
...
correctly.
. [[ex4-15]] According to the North American Numbering Plan (NANP) used by the
...
large. (23 or more!)
. [[ex4-16]] Re-implement the solution to the Monty Hall program given in
