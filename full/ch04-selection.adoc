...
keep your initial choice, you'll win the gold with probability
stem:[\frac{1}{3}]. However, if you switch doors, you will win
with probability stem:[\frac{2}{3}]. The problem is
...
if the gold was behind either one of them, you'd win. Clearly, your
probability of winning would be stem:[\frac{2}{3}]. Monty allows
...
_If it's raining outside, then I'll take my umbrella._
...
_raining outside_ is a condition stem:[p] and that _taking my
umbrella_ is an action stem:[a]. In other words, if
stem:[p] is true, then do stem:[a]. We haven't specified
what is to be done if stem:[p] is not true, although we can 
assume that the actor in this drama will *not* take an umbrella.

If we want to view stem:[p] as a decision to make, we can specify
what happens if it's not true. For example, we could formulate another
choice:

_If I have at least $50 in my pocket, then I'll eat a lobster dinner;
otherwise, I'll eat fast food._

In this case, we let _having at least $50_ be condition stem:[q],
_eating a lobster dinner_ be action stem:[b], and _eating fast
food_ be action stem:[c]. Now we've created a decision. If
stem:[q] is true, the person will do action stem:[b], but
if it's false, she'll do action stem:[c].
...
_If I'm bored, or it's late and I can't sleep, then I'll watch
television._
...
three sub-conditions: _I'm bored_ is condition stem:[x], _it's
late_ is condition stem:[y], and _I can't sleep_ is condition
stem:[z]. We have connected these three conditions together using
...
stem:[\land] to represent the concept of AND and the symbol
stem:[\lor] to represent the concept of OR. We'll also
abbreviate true to T and false to F.

Given a condition stem:[x], a condition stem:[y], and the condition made by stem:[x \land y], this truth table shows all possible values. As stipulated, stem:[x \land y] is true only when both stem:[x] and stem:[y] are true.

[.center%autowidth,cols="^,^,^",options="header",]
|=========================================================
|stem:[x] |stem:[y] |stem:[x \land y]
|T |T |T
|T |F |F
|F |T |F
|F |F |F
...
This truth table gives all the values for stem:[x \lor y]. As you can see, stem:[x \lor y] is true if stem:[x] or stem:[y] are true.

[.center%autowidth,cols="^,^,^",options="header",]
|========================================================
|stem:[x] |stem:[y] |stem:[x \lor y]
|T |T |T
|T |F |T
|F |T |T
|F |F |F
...
stem:[\oplus] to represent the XOR operation in the truth table
below.

This truth table gives all the values for stem:[x \oplus y].

[.text-center]
[.center%autowidth,cols="^,^,^",options="header",]
|==========================================================
|stem:[x] |stem:[y] |stem:[x \oplus y]
|T |T |F
|T |F |T
|F |T |T
|F |F |F
...
Here's a truth table for NOT, using the symbol stem:[\lnot] to represent the NOT operation.
...
first place. Recall that stem:[x] is _I'm bored_,
stem:[y] is _it's late_, and stem:[z] is _I can't sleep_.
Let stem:[d] be the action _I'll watch television_. We can
express the choice in this way: If stem:[x \lor (y \land z)],
then do stem:[d]. Using this notation, we've expressed
...
_If I want seafood, then I'll eat at Sharky's; otherwise, I'll eat at
the Golden Calf. When dining at Sharky's, if I have at least $50, I'll
order the lobster; otherwise, I'll order the shrimp. When dining at the
Golden Calf, if I have at least $30, I'll order the filet mignon;
otherwise, I'll order the pork chops._
...
Operator
<.>|Description
|AND |stem:[\land] |`&&` |Returns `true` if both values are `true`
|OR |stem:[\lor] |`\|\|` |Returns `true` if either value is `true`
|XOR |stem:[\oplus] |`^` |Returns `true` if values are different
|NOT |stem:[\lnot] |`!` |Returns the opposite of the value
|=======================================================================
...
Operator
<.>|Description
|Equals |stem:[=] |`==` |`true` if the two values are equal
|Not Equals |stem:[\neq] |`!=` |`true` if the two values are not
equal
|Less Than |stem:[<] |`<` |`true` if the first value is strictly
less than the second
|Less Than or Equals |stem:[\leq] |`\<=` |`true` if the first
value is less than or equal to the second
|Greater Than |stem:[>] |`>` |`true` if the first value is
strictly greater than the second
|Greater Than or Equals |stem:[\geq] |`>=` |`true` if the first
...
symbols are used to make a statement: stem:[x < y] is an
announcement or a discovery that the value contained in stem:[x]
is, in fact, smaller than the value contained in stem:[y]. In the
...
Then, that value would be given to the `if` statement. Here, the `makeRoommates()` method will be called only if `genderB`
...
formula, you can solve equations of the form stem:[ax^2 + bx
+ c = 0]. As you might recall, the quadratic equation that gives the
solutions is: stem:[\frac{-b \pm \sqrt{b^2 - 4ac}}{2a}]

The stem:[b^2 - 4ac] part of the formula is called the
...
<4> Next, if the discriminant is negative, we compute the real and complex parts separately and output the
two answers.
<5> Finally, if the discriminant is positive, we find the two real
...
possible to differentiate stem:[2^{20} = 1,048,576] items. If you
...
example, if you were born in 1979, stem:[1979 \mod 12 \equiv 11],
...
in great detail in <<ch14-synchronization#ch14-synchronization>>. Remember, one of
...
. Given that stem:[x], stem:[y], and stem:[z] are
propositions in Boolean logic, make a truth table for the expression
stem:[(\lnot(x \land \lnot y)
\oplus \lnot z)].
. What is the value of the Boolean expression stem:[\lnot((T
...
+
stem:[\lnot(x \land y) = \lnot x \lor \lnot y]
stem:[\lnot(x \lor y) = \lnot x \land \lnot y]
+
