...
latexmath:[p] is true, then do latexmath:[a]. We haven't specified
what is to be done if latexmath:[p] is not true, although we can 
...
food_ be action latexmath:[c]. Now we've created a decision. If
latexmath:[q] is true, the person will do action latexmath:[b], but
if it's false, she'll do action latexmath:[c].
...
three sub-conditions: _I'm bored_ is condition latexmath:[x], _it's
...
latexmath:[\lor] to represent the concept of OR. We'll also
...
Here's a truth table for NOT, using the symbol latexmath:[\lnot] to represent the NOT operation.
...
first place. Recall that latexmath:[x] is _I'm bored_,
latexmath:[y] is _it's late_, and latexmath:[z] is _I can't sleep_.
Let latexmath:[d] be the action _I'll watch television_. We can
express the choice in this way: If latexmath:[x \lor (y \land z)],
then do latexmath:[d]. Using this notation, we've expressed
