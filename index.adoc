...
__Start Concurrent__ is available for download in a number of formats. Click a link below to access the appropriate file.

The HTML version is available as a single document (which allows for easy searching) and chunked into a separate document for each chapter (which reduces loading time).

* link:full[HTML (single document)]
* link:chunked[HTML (chunked)]

[.text-center]
image::multicore.svg[scaledwidth=25%,pdfwidth=25%,width=25%]

== About

Welcome to __Start Concurrent__!
This book is intended as an entry point into the multicore revolution that's
now in full swing. It's designed to introduce students to concurrent
programming at the same time they are learning the basics of sequential
...
Multicore processors are omnipresent. Whether you use a desktop or a laptop,
chances are that your computer has a multicore chip at its heart. Desktop
parallel computers have been prophesied for years. That time has come. Parallel
computers sit on our desks and our laps. This progress in microprocessor
...
computing. Now that parallel computers are on our desks, should we consider
introducing the fundamentals  of concurrent programming in beginner classes in
programming? Of course, there are many opinions about this question.
...
the kitchen and you will see concurrency in action. Watch a movie and you will
see concurrency in action as various subplots, scenes, and flashbacks weave the
plot together. Parents use concurrent solutions to solve day-to-day problems as
they juggle caring for their children, a career, and a social life.
...
of a problem to an artificial language such as Java. It is an activity
...
mapping process. What is the best way to transform a sequential solution to an
...
concepts from the very beginning, it is difficult for students with no
...
Chapters 1 through 12 are intended for the absolute beginner. If you do not want
to introduce concurrent programming in a first course, these chapters should
prove adequate. The concurrency material and exercises in these chapters
are well-marked and can be ignored without negatively impacting the other
...
an instructor to decide what to cover and what to leave out. Often there is a
tendency to cover more material than less. We have noticed that today's student
...
description of Java. Where appropriate we direct the student to websites where
...
do not have the space to cover. For example, the Swing package for
...
Quest for the ideal:: For many of us, teaching programming is a constant
challenge. While some students, inspired by their interactions with computers,
immediately  love the subject, others find it tedious and uninteresting. Perhaps no combination of pedagogy and teaching excellence will get all students in a
large freshman programming class to like programming. Nevertheless, some of us
keep trying to excite and motivate every single student in a class full of
curious minds. Perhaps this is the ideal that keeps many of us engaged in, and
never bored of, teaching first year programming.
Java IDE:: It is important that the students be introduced to a Java IDE
...
could use simple problems from areas such as mathematics or physics or even day-to-day life that lead to sequential and concurrent solutions. Early exposure to solutions these problems, programmed in Java, can be beneficial students even if they do not understand all the syntax.
...
this book we have decided to follow a flexible approach. We begin by discussing the use of `System.out.print()` and `Scanner` and the
...
Special thanks to Azfar Khandoker who not only attended CS190M, but also worked on an independent study project to create exercises using Lego robots to help students learn programming. Azfar's work has led to the use of robots in two freshman programming classes taught at Purdue. 
